...
"""
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        ...
        if post_type == 'telegram':
            ...
            # 3. Проверяем наличие вопроса - УЛУЧШЕННАЯ ПРОВЕРКА
            has_question = any('?' in line for line in lines)
            question_ends_with_q = any(line.endswith('?') for line in lines)
            
            # Если вопроса нет или он обрезан (не заканчивается на ?)
            if not has_question or not question_ends_with_q:
                logger.warning(f"⚠️ Telegram пост не содержит полного вопроса! Проверяю...")
                
                # Проверяем, есть ли обрезанный вопрос
                partial_question_found = False
                for i, line in enumerate(lines):
                    # Ищем строку, которая похожа на обрезанный вопрос
                    if line and not line.endswith('?') and len(line) > 10:
                        # Проверяем, не обрезан ли вопрос в конце строки
                        if '?' not in line and any(word in line.lower() for word in ['готовы', 'считаете', 'думаете', 'как', 'что']):
                            logger.warning(f"⚠️ Найден возможный обрезанный вопрос: '{line}'")
                            # Пробуем восстановить вопрос
                            if self.current_theme:
                                theme_questions = {
                                    "HR и управление персоналом": "Готовы ли компании инвестировать в развитие HR не как в затраты, а как в стратегический актив?",
                                    "PR и коммуникации": "Готовы ли компании отказаться от иллюзии идеальности ради долгосрочных отношений с аудиторией?",
                                    "ремонт и строительство": "Готовы ли вы платить за качество сейчас, чтобы экономить на ремонтах в будущем?"
                                }
                                full_question = theme_questions.get(self.current_theme, "Что думаете об этом?")
                                lines[i] = full_question
                                partial_question_found = True
                                break
                
                if not partial_question_found:
                                ...
                                for j in range(i + 1, len(lines)):
                                    if lines[j] == '' or j == len(lines) - 1:
                                        if self.current_theme:
                                            ...
                                            full_question = theme_questions.get(self.current_theme, "Что думаете об этом?")
                                            lines.insert(j + 1, full_question)
                                        target_found = True
                                        break
                                if target_found:
                                    break
                            ...
                            for i in range(len(lines)-1, -1, -1):
                                if lines[i] != '':
                                    if self.current_theme:
                                        ...
                                        full_question = theme_questions.get(self.current_theme, "Что думаете об этом?")
                                        lines.insert(i+1, full_question)
                                    break
                
                text = '\n'.join(lines)
                logger.info(f"✅ Добавлен/исправлен вопрос в Telegram пост")
            ...
            if len(non_empty_lines) < 4:
                ...
                if len(non_empty_lines) == 0:
                    if self.current_theme:
                        ...
                        default_question = theme_questions.get(self.current_theme, "Что думаете об этом?")
                    else:
                        default_question = "Что думаете об этом?"
    ...
    def check_post_complete(self, text: str, post_type: str, slot_style: Dict = None) -> bool:
        ...
        if post_type == 'telegram':
            ...
            has_complete_question = any(line.endswith('?') for line in lines)
            has_hashtags = any(line.startswith('#') for line in lines)
            
            if not has_question or not has_complete_question:
                logger.warning(f"⚠️ Telegram пост не содержит полного вопроса!")
                return False
