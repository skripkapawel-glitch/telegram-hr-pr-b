...
ТРЕБОВАНИЯ К ДЛИНЕ СИМВОЛОВ (ЭТО САМОЕ ВАЖНОЕ):
• Telegram пост должен содержать от {tg_min} до {tg_max} символов включительно (вместе с хештегами)
• Дзен пост должен содержать от {zen_min} до {zen_max} символов включительно (вместе с хештегами)

АЛГОРИТМ ТВОИХ ДЕЙСТВИЙ:
1. Сначала написать текст
2. Потом проверить общую длину всего текста (с хештегами)
3. Если текст длиннее максимального лимита ({tg_max} символов для Telegram или {zen_max} символов для Дзен) — СРАЗУ СОКРАТИТЬ
4. Убрать лишние слова, повторы, второстепенные детали
5. Сохранить основную мысль и структуру
...
ФИНАЛЬНАЯ ПРОВЕРКА:
ПОСЛЕ написания текста ОБЯЗАТЕЛЬНО убедись, что:
• Telegram пост содержит от {tg_min} до {tg_max} символов
• Дзен пост содержит от {zen_min} до {zen_max} символов
            ...
            try:
                ...
                if '---' in text:
                    ...
                    if tg_min <= tg_len <= tg_max and zen_min <= zen_len <= zen_max:
                        ...
                        logger.warning(f"Attempt {attempt+1}: Character limits not met: TG={tg_len}(от {tg_min} до {tg_max}), ZEN={zen_len}(от {zen_min} до {zen_max})")
                        if attempt < 2:
