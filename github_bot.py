...
# ============== АБСОЛЮТНО ЖЁСТКОЕ ПРАВИЛО ОБЪЁМА ТЕКСТА ==============

# (БЕЗ ПРАВА НА ОШИБКУ)

Ты обязан СТРОГО, БЕЗУКОРИЗНЕННО и БЕЗ МАЛЕЙШЕГО ОТКЛОНЕНИЯ соблюдать объём текста.
Отклонение даже на 1 символ — КРИТИЧЕСКАЯ ОШИБКА, полностью недопустимая.

# КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО:

1. Писать меньше или больше указанного диапазона хотя бы на 1 символ.
2. Урезать текст — запрещено полностью.
3. «Сжимать» текст, сокращать, удалять смысловые блоки, обрезать абзацы.
4. Выходить за рамки лимита даже на один символ ради “красоты фразы”.

# ТРЕБОВАНИЕ:

Ты обязан создавать текст ИЗНАЧАЛЬНО сразу в нужном диапазоне символов.
Не меньше.
Не больше.
Никаких погрешностей, допусков, предположений.

# ЕСЛИ ОБЪЁМ НЕСООТВЕТСТВУЕТ:

Ты обязан самостоятельно переписать, перестроить, переформулировать материал так, чтобы итоговый вариант
идеально попадал в заданный диапазон символов без урезаний.

# ЖЁСТКАЯ ФОРМУЛА:

Если лимит {tg_min}–{tg_max}, значит итоговый текст обязан быть не меньше {tg_min} и не больше {tg_max}.
Если лимит {zen_min}–{zen_max}, значит итоговый текст обязан быть не меньше {zen_min} и не больше {zen_max}.
Любое отклонение — нарушение задания.

# КАЖДЫЙ ПОСТ = ТОЧНО В ДИАПАЗОНЕ

Не "около", не "примерно", не "в среднем",
а ТОЧНО в рамках указанного диапазона.
Без исключений.
Без оправданий.
Без автоматических сокращений.

# ❗️ ❗️ ❗️ ЕСЛИ ТЫ НЕ СОБЛЮДЕШЬ ЭТИ ЛИМИТЫ - ТЕКСТ БУДЕТ УДАЛЕН ❗️ ❗️ ❗️
...
Если Telegram должен быть {tg_min}-{tg_max} символов:
• ХОРОШО: {tg_ideal}, {tg_ideal-20}, {tg_ideal+20} символов
• ПЛОХО: {tg_min-1} символов (мало), {tg_max+1} символов (много)

Если Дзен должен быть {zen_min}-{zen_max} символов:
• ХОРОШО: {zen_ideal}, {zen_ideal-20}, {zen_ideal+20} символов  
• ПЛОХО: {zen_min-1} символов (мало), {zen_max+1} символов (много)
...
Текст должен быть ИЗНАЧАЛЬНО создан в нужном диапазоне.
"""
