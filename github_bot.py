...
class TelegramBot:
    ...
    QUESTION_TYPES = [
        ...
        "Как вы находите баланс между скорость и качеством?",
...
[2] АБЗАЦ 1: {approach}. Свободный вход в мысль и небольшое развитие. 2-3 предложения. Используй разнообразные подходы.

[3] 🎯 КЛЮЧЕВАЯ МЫСЛЬ: {key_thought}. Начни с 🎯. 1-2 предложение. Сформулируй по-новому, избегая шаблонных фраз.
...
4. Обязательно используй 🎯 для ключевой мысли
...
12. УБЕДИСЬ, ЧТО ВСЕ 5 БЛОКОВ УМЕЩАЮТСЯ В {tg_token_min}-{tg_token_max} ТОКЕНОВ
13. ЕСЛИ НЕ ХВАТАЕТ ТОКЕНОВ - СОКРАТИ ПРЕДЫДУЩИЕ БЛОКИ, НО НЕ ОБРЕЗАЙ ПОСЛЕДНИЙ
14. ПОСТ ДОЛЖЕН БЫТЬ ПОЛНЫМ И ЗАКОНЧЕННЫМ
...
"""
    ...
    def create_zen_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        """Создает промпт для Zen поста - СТРОГАЯ СТРУКТУРА"""
        # Добавляем случайный seed для вариативности
        random_seed = random.randint(1, 10000)
...
АБЗАЦ 1: {approach}. Разверни тему подробно. 4-6 предложений. Добавь конкретный пример или кейс.

АБЗАЦ 2: Продолжи развитие мысли, добавь ещё один пример или анализ. 3-5 предложений.

КЛЮЧЕВАЯ МЫСЛЬ: {key_thought}. Разверни её подробно, объясни почему это важно. 2-3 предложения.
...
3. Между всеми 6 блоками ДОЛЖНЫ быть пустые строки (но НЕ пиши слова "ПУСТАЯ СТРОКА" - просто оставь пустую строку)
...
7. Обязательно добавь конкретные примеры, цифры, кейсы из реальной практики
8. Избегай повторения предыдущих текстов
9. ОБЯЗАТЕЛЬНО достигни минимальной длины в {zen_min_chars} символов
10. НИ В КОЕМ СЛУЧАЕ не превышай максимальную длину в {zen_max_chars} символов
11. СТРОГО следуй структуре: Заголовок, пустая строка, Абзац 1, пустая строка, Абзац 2, пустая строка, Ключевая мысль, пустая строка, Вопрос, пустая строка, Хештеги
12. Каждый блок должен быть полным и законченным - НИКАКИХ ОБРЫВОВ В СЕРЕДИНЕ ПРЕДЛОЖЕНИЯ
...
16. УБЕДИСЬ, ЧТО ВСЕ АБЗАЦЫ ЗАКОНЧЕНЫ - НЕ ОБРЫВАЙ МЫСЛЬ ПОСЛЕДНИМ СЛОВОМ
17. Абзацы должны быть полными мыслями с логическим завершением
18. Каждый абзац должен содержать минимум 3 предложения и заканчиваться точкой
19. Если используешь пример - доведи его до логического завершения
20. НЕ оставляй предложения недописанными, особенно перед точкой, запятой или знаком вопроса
"""
        return prompt.strip()
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        ...
        if post_type == 'telegram':
            ...
            if not target_found:
                ...
                if len(lines) > 1:
                    ...
                    lines.append('')  # Пустая строку после ключевой мысли
            ...
            # 6. Проверяем, что заголовок - одно предложение и есть пустая строка после него
            if lines and slot_style and 'emoji' in slot_style:
                if lines[0].startswith(slot_style['emoji']):
                    # Находим конец заголовка (первая точка, вопросительный или восклицательный знак)
                    header_line = lines[0]
                    # Если заголовок слишком длинный (содержит точку), обрезаем до первого предложения
                    if '.' in header_line or '?' in header_line or '!' in header_line:
                        # Находим конец первого предложения
                        for end_char in ['.', '?', '!']:
                            if end_char in header_line:
                                end_index = header_line.find(end_char) + 1
                                lines[0] = header_line[:end_index].strip()
                                # Если после заголовка нет пустой строки, добавляем её
                                if len(lines) > 1 and lines[1] != '':
                                    lines.insert(1, '')
                                break
            
            # 7. Проверяем и добавляем пустые строки между блоками
            final_lines = []
        ...
        # Zen проверка - СТРОГАЯ ВАЛИДАЦИЯ С ПРОВЕРКОЙ ДЛИНЫ И СТРУКТУРЫ
        elif post_type == 'zen':
            ...
            # 3. Проверяем структуру: должны быть пустые строки между блоками
            lines = text.split('\n')
            
            # Проверяем наличие пустых строк между основными блоками
            # Должно быть минимум 5 основных блоков с пустыми строками между ними
            non_empty_lines = [line.strip() for line in lines if line.strip()]
            
            # 4. Удаляем дублирующиеся вопросы после хештегов
            # Находим хештеги
            ...
            # 5. Проверяем незавершенные предложения и дополняем их
            new_lines = []
            for line in lines:
                line_stripped = line.strip()
                if line_stripped and not line_stripped.startswith('#'):
                    # Проверяем, заканчивается ли строка знаком завершения предложения
                    if not line_stripped.endswith(('.', '?', '!', ':', ';')):
                        # Если строка не заканчивается знаком препинания, это может быть обрыв
                        # Проверяем, обрывается ли на последнем слове
                        words = line_stripped.split()
                        if words:
                            last_word = words[-1]
                            # Если последнее слово выглядит как начало или середина предложения
                            if not last_word.endswith(('.', '?', '!', ':', ';', ',', '»', '"', "'")):
                                # Дополняем предложение в зависимости от темы
                                if self.current_theme:
                                    completions = {
                                        "HR и управление персоналом": "что положительно влияет на корпоративную культуру.",
                                        "PR и коммуникации": "что способствует восстановлению доверия к бренду.",
                                        "ремонт и строительство": "что обеспечивает долговечность конструкции."
                                    }
                                    completion = completions.get(self.current_theme, "что подтверждает важность правильного подхода.")
                                    line_stripped += " " + completion
                new_lines.append(line_stripped)
            lines = new_lines
            
            # 6. Если текст слишком короткий, добавляем недостающий контент
            current_length = len(text)
            if current_length < zen_min:
                logger.warning(f"⚠️ Zen пост слишком короткий: {current_length} < {zen_min}. Увеличиваю...")
                
                # Добавляем дополнительные абзацы или расширяем существующие
                if len(non_empty_lines) < 6:  # Меньше 6 блоков (заголовок + 2 абзаца + ключевая мысль + вопрос + хештеги)
                    # Добавляем недостающие блоки
                    if self.current_theme:
                        theme_content = {
                            "HR и управление персоналом": {
                                "доп_абзац": "Современные исследования показывают, что компании с развитой корпоративной культурой показывают на 20-30% более высокую продуктивность. Важно не только нанимать талантливых сотрудников, но и создавать среду, где они могут расти и развиваться.",
                                "пример": "Компания Google известна своими программами развития сотрудников, которые включают тренинги, менторство и возможности для карьерного роста."
                            },
                            "PR и коммуникации": {
                                "доп_абзац": "В 2023 году исследование Edelman Trust Barometer показало, что доверие к бизнесу падает, и только прозрачная коммуникация может восстановить его. Компании, которые честно рассказывают о своих проблемах и достижениях, получают больше лояльности от клиентов.",
                        ...
                        content = theme_content.get(self.current_theme, {})
                        if content:
                            # Добавляем дополнительный абзац перед ключевой мыслью
                            new_lines = []
                            for line in lines:
                                new_lines.append(line)
                                # Если находим место для вставки (перед ключевой мыслью или вопросом)
                                if '?' in line and not line.strip().startswith('#'):
                                    new_lines.append('')
                                    new_lines.append(content.get("доп_абзац", ""))
                                    new_lines.append('')
                                    new_lines.append(content.get("пример", ""))
                                    break
            ...
            for i, line in enumerate(lines):
                ...
                if line_stripped:
                    # Если предыдущая строка была непустой и мы начинаем новый блок
                    if in_block and i > 0 and lines[i-1].strip():
                        final_lines.append('')
            ...
            # 8. Проверяем наличие всех необходимых элементов
            # Ищем хештеги
            hashtag_pattern = r'#\w{2,}'
            hashtags = re.findall(hashtag_pattern, text)
            ...
            # Ищем вопросы
            question_lines = [line for line in text.split('\n') if '?' in line and not line.startswith('#')]
            if len(question_lines) < 2:  # Должен быть заголовок-вопрос и вопрос в конце
                if self.current_theme:
                    theme_questions = {
                        "HR и управление персоналом": "Как вы считаете, что важнее для удержания сотрудников: высокая зарплата или возможности для развития?",
                        "PR и коммуникации": "Насколько важна прозрачность коммуникаций для построения доверия с аудиторией?",
                        "ремонт и строительство": "Стоит ли экономить на материалах, если это может повлиять на качество результата?"
                    }
                    additional_question = theme_questions.get(self.current_theme, "Что вы думаете по этому поводу?")
            ...
            # 9. ФИНАЛЬНАЯ ПРОВЕРКА ДЛИНЫ - САМОЕ ВАЖНОЕ
            ...
            if final_length < zen_min:
                ...
                completion_text = "\n\nЭтот пример показывает, как важно учитывать все аспекты при принятии решений в профессиональной сфере. Что вы думаете по этому поводу?"
                text += completion_text
            
            if final_length > zen_max:
                logger.error(f"❌ Zen пост превышает лимит: {final_length} > {zen_max}")
                # Сокращаем до максимальной длины, но сохраняем структуру
                text = text[:zen_max]
                # Убедимся, что не обрезали посередине предложения
                last_period = max(text.rfind('.'), text.rfind('?'), text.rfind('!'))
                if last_period > zen_max * 0.8:  # Если есть законченное предложение в последних 20%
    ...
    def check_post_complete(self, text: str, post_type: str, slot_style: Dict = None) -> bool:
        ...
        if post_type == 'telegram':
            ...
            # Проверяем наличие хотя бы двух вопросов (в заголовке и в теле)
            ...
            # Проверяем, что нет незавершенных предложений
            has_complete_sentences = True
            for line in non_hashtag_lines:
                if line and not line.startswith('#') and '?' not in line:
                    # Проверяем, заканчивается ли строка знаком завершения предложения
                    if not line.endswith(('.', '?', '!', ':', ';')):
                        # Проверяем, не обрывается ли на последнем слове
                        words = line.split()
                        if words:
                            last_word = words[-1]
                            if not last_word.endswith(('.', '?', '!', ':', ';', ',', '»', '"', "'")):
                                logger.warning(f"⚠️ Zen пост содержит незавершенное предложение: {line}")
                                has_complete_sentences = False
                                break
            
            return has_no_emoji and len(question_lines) >= 1 and hashtag_lines and len(non_hashtag_lines) >= 4 and has_no_extra_questions_after_hashtags and has_no_empty_string_words and has_complete_sentences
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        """Генерация постов с повторными попытками и валидацией - УСИЛЕННАЯ ЛОГИКА"""
