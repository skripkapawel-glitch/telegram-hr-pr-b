...
class TelegramBot:
    ...
    def create_telegram_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        """Создает промпт для Telegram поста - НЕ ТРОГАЕМ"""
...
"""
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        ...
        # Telegram проверка - СТАРАЯ ЛОГИКА
        if post_type == 'telegram':
        ...
        # Zen проверка - БЕЗ ИЗМЕНЕНИЙ ДЛЯ ТГ
        elif post_type == 'zen':
            # 1. Удаляем эмодзи
            ...
            # ФИНАЛЬНАЯ ПРОВЕРКА: если текст все еще превышает лимит, ВОЗВРАЩАЕМ ОШИБКУ
            if len(text) > zen_max:
                logger.error(f"❌ Zen пост превышает лимит: {len(text)} > {zen_max}")
                return False, text
        
        # Telegram проверка хештегов - СТАРАЯ ЛОГИКА
        if post_type == 'telegram':
            hashtag_pattern = r'#\w{2,}'
