...
class TextPostProcessor:
    ...
    def process(self, raw_text: str) -> str:
        ...
        # 2. Удаляем звездочки из начала Telegram поста
        if self.post_type == 'telegram':
        ...
        # 5. Гарантируем эмодзи в начале Telegram поста
        ...
        # 6. Удаляем все эмодзи из Zen поста
        if self.post_type == 'zen':
        ...
        # 7. Проверяем и исправляем недостающие структурные элементы
        if self.post_type == 'telegram':
            # Удаляем описания блоков
            raw_text = re.sub(r'^1\. Заголовок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^2\. Первый блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^3\. Второй блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^4\. Третий блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^5\. Структурирующие элементы.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^6\. Вопрос.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^7\. Хештеги.*?\n', '', raw_text, flags=re.IGNORECASE)
            
            # Удаляем фразу "фиксация сути"
            raw_text = re.sub(r'фиксация сути', '', raw_text, flags=re.IGNORECASE)
            ...
            structural_elements = [
                ...
                ('📋 Шаги:', ''),
                ('🔧 Практика:', ''),
                ('ВОПРОС ДЛЯ АУДИТОРИИ:', 'вопрос')
            ]
            
            for element, default_content in structural_elements:
                # Удаляем элементы без содержания
                pattern_empty = re.compile(f'{re.escape(element)}\\s*$', re.MULTILINE)
                if pattern_empty.search(raw_text):
                    raw_text = pattern_empty.sub('', raw_text)
                    logger.warning(f"⚠️ Удален пустой элемент: {element}")
                
                pattern_template = re.compile(f'{re.escape(element)}\\s*\\[.*?\\]', re.IGNORECASE)
                if pattern_template.search(raw_text):
                    # Удаляем элемент с шаблоном
                    raw_text = pattern_template.sub('', raw_text)
                    logger.warning(f"⚠️ Удален элемент с шаблоном: {element}")
        
        elif self.post_type == 'zen':
            # Удаляем описания блоков
            raw_text = re.sub(r'^1\. Шапка.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^2\. Первый блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^3\. Второй блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^4\. Третий блок.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^5\. Смысловой якорь.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^6\. Вопрос для обсуждения.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'^7\. Хештеги.*?\n', '', raw_text, flags=re.IGNORECASE)
            
            # Удаляем варианты смыслового якоря
            raw_text = re.sub(r'Один из вариантов:.*?\n', '', raw_text, flags=re.IGNORECASE | re.DOTALL)
            raw_text = re.sub(r'Почему это важно.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'Что из этого следует.*?\n', '', raw_text, flags=re.IGNORECASE)
            raw_text = re.sub(r'Мнение экспертов.*?\n', '', raw_text, flags=re.IGNORECASE)
        ...
        # 8. Форматирование переносов строк
        ...
        # 9. Проверяем, что пост заканчивается хештегами
        if self.post_type == 'telegram':
        ...
        # 10. Удаляем лишние пустые строки
        result = re.sub(r'\n\s*\n\s*\n+', '\n\n', result)
        
        # 11. Проверяем, что пост не обрывается на середине предложения
...
class TelegramBot:
    ...
    def create_telegram_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        prompt = f"""
Создай Telegram пост по теме "{theme}".

Ограничения:
- Длина: {tg_min}-{tg_max} символов
- Начинай с: {slot_style['emoji']}
- Завершай: 2-5 хештегов по теме

Структура поста (строго соблюдать):

1. {slot_style['emoji']} [Заголовок]
   Формулирует тему или позицию.
   Не описывает формат, не обещает пользы, не объясняет.

2. [Первый блок]
   Начинает мысль.
   Это может быть утверждение, наблюдение, сомнение или тезис.
   Блок обязан быть логически завершённым.

3. [Второй блок]
   Развивает предыдущую мысль.
   Добавляет глубину, контекст или усложнение.
   Не повторяет первый блок другими словами.

4. [Третий блок] (опционально)
   Уточняет границы мысли, добавляет поворот или контраст.
   Если блока нет — второй блок обязан быть самодостаточным.

5. [Структурирующие элементы] (опционально)
   🎯 Важно — формулируется как завершённое утверждение.
   📋 Шаги — только если есть действия.
   🔧 Практика — только если есть прикладное применение.
   ❗ Запрещено писать названия блоков без содержания.

6. [Вопрос]
   Продолжает тему.
   Не подводит итог.
   Не формулируется как «что вы думаете».

7. [Хештеги]
   {hashtags}

Важные правила:
- НЕ начинай с "Доброе утро", "Добрый день", "Добрый вечер"
- НЕ используй фразу "фиксация сути"
- НЕ оставляй пустые блоки или шаблоны [содержимое]
- Каждый блок должен содержать реальный текст
- Пост должен читаться как готовый контент, а не как инструкция

Картинка для поста: {image_description}

Создай чистый пост без технических комментариев, нумерации блоков или пояснений."""
    ...
    def create_zen_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        prompt = f"""
Создай Яндекс Дзен пост по теме "{theme}".

Ограничения:
- Длина: {zen_min}-{zen_max} символов
- Без эмодзи во всем посте
- Завершай: 2-5 хештегов по теме
...
1. [Шапка]
   ...
   Не объясняет и не повторяется дальше.

   [Пустая строка]

2. [Первый блок]
   Разворачивает направление мысли.
   Без выводов, без советов, без фиксации.

3. [Второй блок]
   Двигает мысль дальше:
   через анализ, сравнение, противопоставление или смену масштаба.
...
5. [Смысловой якорь]
   ...
   - Что из этого следует
   - Мнение экспертов
   Формулируется как вывод, а не как описание блока.

6. [Вопрос для обсуждения]
   Расширяет тему, а не закрывает её.
...
- НЕ оставляй пустые блоки или шаблоны [содержимое]
- Каждый блок должен содержать реальный текст
- Не пиши названия блоков (Почему это важно: и т.д.)
- Пост должен читаться как готовый контент, а не как инструкция
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_tg:
                ...
                # Проверяем, что пост чистый
                if 'ТЕКУЩИЙ СЛОТ' in tg_processed or 'СТРУКТУРА' in tg_processed or 'ЛИМИТ' in tg_processed:
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                # Проверяем, что пост чистый
                if 'ТЕКУЩИЙ СЛОТ' in zen_processed or 'СТРУКТУРА' in zen_processed or 'ЛИМИТ' in zen_processed:
