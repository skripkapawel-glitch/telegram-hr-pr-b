...
ТВОЙ ПОСТ:"""
    ...
    def create_zen_prompt(self, theme, slot_info, text_format):
        """Создает промпт для Дзен поста"""
        try:
            zen_min, zen_max = slot_info['zen_chars']
            
            # Целевая длина - чуть ниже максимума
            target_length = zen_max - 50
...
ВАЖНЕЙШИЕ ПРАВИЛА:
• МАКСИМАЛЬНАЯ ДЛИНА: {zen_max} символов (АБСОЛЮТНЫЙ ЛИМИТ)
• Лучше сделать короче, чем длиннее
• Если приближаешься к {zen_max} символов - ОСТАНОВИСЬ
• Никаких эмодзи в тексте
• Короткие предложения, лаконично
• Сфокусируйся на одной главной мысли

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
СОЗДАЙ ПОСТ О ТЕМЕ "{theme}" В ФОРМАТЕ "{text_format}".
СТРОГО НЕ БОЛЕЕ {zen_max} СИМВОЛОВ! ЛУЧШЕ {target_length}!
...
ТВОЙ ПОСТ:"""
    ...
    def _manual_shorten_text(self, text, target_max):
        """Вручную сокращает текст если он слишком длинный"""
        lines = text.split('\n')
        shortened_lines = []
        current_length = 0
        ...
        for line in lines:
            ...
            if current_length + line_length + 1 <= target_max:
                shortened_lines.append(line)
                current_length += line_length + 1  # +1 за перенос строки
            else:
                # Попробуем сократить эту строку
                words = line.split()
                shortened_line = []
                for word in words:
                    if current_length + len(word) + 1 <= target_max:
                        shortened_line.append(word)
                        current_length += len(word) + 1
                    else:
                        break
                
                if shortened_line:
                    shortened_lines.append(' '.join(shortened_line))
                break
        
        result = '\n'.join(shortened_lines).strip()
        
        # Обрезаем до target_max на всякий случай
        if len(result) > target_max:
            result = result[:target_max].rsplit(' ', 1)[0]  # Обрезаем до последнего целого слова
        
        return result

    def _aggressive_rewrite_zen(self, text, target_max):
        """Агрессивно переписывает Дзен пост для сильного сокращения"""
        rewrite_prompt = f"""СДЕЛАЙ ЭТОТ ТЕКСТ В 2 РАЗА КОРОЧЕ. СОКРАТИ ДО МАКСИМУМА {target_max} СИМВОЛОВ.

ИСХОДНЫЙ ТЕКСТ:
{text}

ЗАДАЧА:
• Сделай текст ВДВОЕ короче
• Оставь только самую суть
• Удали все примеры, оставь только главную мысль
• Сделай предложения максимально короткими
• Сохрани структуру: заголовок, введение, основная часть, заключение, вопрос, хэштеги
• Удали все лишние детали и описания

ПРАВИЛА СОКРАЩЕНИЯ:
1. Одно предложение = одна мысль
2. Без прилагательных и наречий
3. Только факты и выводы
4. Объедини абзацы если можно
5. Удали все вводные фразы

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
СОКРАЩЕННЫЙ ТЕКСТ (максимум {target_max} символов, лучше меньше):"""
        ...
        try:
            ...
            data = {
                ...
                "generationConfig": {
                    ...
                    "topP": 0.2,
                    "topK": 10,
                    "maxOutputTokens": target_max + 100,
                }
            }
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in result and result['candidates']:
                    ...
                    # Если всё еще слишком длинный, делаем ручное сокращение
                    if len(rewritten_text) > target_max:
                        logger.info(f"🔥 Все еще слишком длинный, применяем ручное сокращение")
                        rewritten_text = self._manual_shorten_text(rewritten_text, target_max)
    ...
    def generate_with_retry(self, prompt, target_min, target_max, post_type, max_attempts=3):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if "Дзен" in post_type:
                    temperature = 0.2  # Низкая для более предсказуемой длины
                    max_tokens = target_max + 200  # Жесткое ограничение
                else:
                    temperature = 0.3
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        if "Дзен" in post_type and length > target_max and attempt < max_attempts - 1:
                            ...
                            rewritten = self._aggressive_rewrite_zen(generated_text, target_max)
                            
                            if rewritten:
                                rewritten_len = len(rewritten)
                                logger.info(f"📊 После агрессивного сокращения: {rewritten_len} символов")
