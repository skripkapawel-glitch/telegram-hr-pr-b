...
print("=" * 80)
print("🚀 GITHUB BOT: ГЕНЕРАЦИЯ КОРОТКИХ ПОСТОВ (структурированных)")
print("=" * 80)
    ...
    def __init__(self):
        ...
        self.time_slots = {
            "09:00": {
                ...
                "zen_chars": (1000, 1500),   # Дзен: 1000-1500 (увеличен утром)
            ...
            "14:00": {
                ...
                "zen_chars": (700, 850),     # Дзен: 700-850
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
            ...
            "19:00": {
                ...
                "zen_chars": (800, 900),     # Дзен: 800-900
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
...
Яндекс Дзен:
• 09:00 — 1000–1500 символов
• 14:00 — 700–850 символов
• 19:00 — 800–900 символов
Стиль: {time_slot_info['zen_style']}

⸻

2) Как должен выглядеть идеальный пост:

• Сильный хук.
Сразу интрига или провокационный факт, чтобы читатель не пролистнул.

• Живая подача.
Короткие фразы, эмоции, структурность, лёгкая динамика. Телеграм — ярко, Дзен — глубоко.

• Ясная логика:
факт → мини-кейс / наблюдение → вывод → вопрос.
Работает и в ТГ, и в Дзене, просто глубина разная.

• Экспертность через реальные ситуации.
Без фантазий. Если опыт — через 3-е лицо («знакомый из сферы рассказал»).
Если аналитика — от 1 лица.

⸻

3) Что отличает Дзен от ТГ:
...
4) Закрывашка (обязательная):

В конце должен быть мягкий вовлекающий финал:
...
5) Конкретные требования к этому посту:

Telegram ({tg_chars_min}-{tg_chars_max} символов):
• Начинай СРАЗУ с хука (без заголовков)
• Используй эмодзи для эмоций
• Короткие абзацы с отступом
• 3-4 хештега в конце
...
• Чёткая структура с отступами
• Структура: ХУК → Проблема → Анализ → Решение → Вопрос → Подпись

⸻

6) После написания текста проанализируй пост и сформируй точный поисковый запрос для изображения.
...
Telegram-пост:
[здесь текст для Telegram с эмодзи, отступами и 3-4 хештегами в конце]
(Начинай сразу с хука!)

Дзен-пост:
[здесь текст для Дзен без эмодзи, с четкой структурой и отступами]
(Начинай сразу с хука!)
...
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС. Создай по-настоящему цепляющие тексты!"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = {
                    ...
                    "generationConfig": {
                        "temperature": 0.9,
                        "maxOutputTokens": 2000,  # Увеличено для Дзен
                        "topP": 0.95,
                        "topK": 40
                    }
                }
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        if "Telegram-пост:" in generated_text and "Дзен-пост:" in generated_text:
                            ...
                            # Проверяем длину
                            if total_length > 2500:  # Если слишком длинно для Дзена
    ...
    def add_telegram_hashtags(self, text, theme):
        ...
        general_hashtags = ["#инсайты", "#лайфхак", "#профессия", "#развитие", "#успех"]
        random.shuffle(general_hashtags)
        
        all_hashtags = base_hashtags[:3] + general_hashtags[:2]
        hashtags_to_add = random.sample(all_hashtags, min(4, len(all_hashtags)))
    ...
    def send_single_post(self, chat_id, text, image_url, is_telegram=True):
        """Отправляет пост"""
        try:
            # Для Telegram и Дзен уже отформатированы
            formatted_text = text
    ...
    def generate_and_send_posts(self):
        ...
        try:
            ...
            if not self.has_hook_at_start(tg_text):
                logger.warning("⚠️ В Telegram посте нет хука в начале")
                tg_text = "🔥 " + tg_text
            
            if not self.has_hook_at_start(zen_text):
                logger.warning("⚠️ В Дзен посте нет хука в начале")
                hook_starters = ["Важный момент:", "Интересный факт:", "Знаете ли вы:"]
                tg_text = random.choice(hook_starters) + " " + zen_text
...
def main():
    """Главная функция"""
    print("\n" + "=" * 80)
    print("🤖 GITHUB BOT: СТРУКТУРИРОВАННЫЕ ПОСТЫ")
    print("=" * 80)
    ...
    print("\n   ДЗЕН:")
    print("   • 09:00 — 1000–1500 символов")
    print("   • 14:00 — 700–850 символов")
    print("   • 19:00 — 800–900 символов")
    print("=" * 80)
    print("⚡ Особенности:")
    print("   • Сильный хук в начале")
    print("   • Чёткая структура с отступами")
    print("   • Живая подача для Telegram")
    print("   • Глубокая аналитика для Дзена")
    print("   • Обязательная закрывашка")
    print("=" * 80)
    ...
    if success:
        ...
        print("   Структурированные посты отправлены")
        print("=" * 50)
        sys.exit(0)
    else:
        print("\n" + "=" * 50)
