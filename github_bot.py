...
DZEN: [текст Дзен-поста]"""
    ...
    def get_post_image(self, theme):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"🖼️ Найдена картинка")
                return image_url
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            # Проверяем, не был ли уже отправлен этот слот
    ...
    def run_now_mode(self):
        ...
        slot_info = self.schedule[slot_time]
        print(f"📝 Автовыбор: {slot_time} - {slot_info['name']}")
...
def main():
    ...
    parser.add_argument('--now', '-n', action='store_true', help='Немедленная отправка поста')
