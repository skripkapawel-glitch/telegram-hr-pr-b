...
ЛИМИТ СИМВОЛОВ: {tg_min}-{tg_max} символов (ВКЛЮЧАЯ ВСЕ: текст, хештеги, эмодзи)
...
5. 3-5 релевантных хештегов в конце поста (включены в общий лимит символов!)
...
ЛИМИТ СИМВОЛОВ: {zen_min}-{zen_max} символов (ВКЛЮЧАЯ ВСЕ: текст и хештеги)
...
5. 3-5 релевантных хештегов в конце (включены в общий лимит символов!)
        ...
        for attempt in range(max_attempts):
            ...
            if generated_tg:
                ...
                if tg_min <= len(tg_processed) <= tg_max:
                    ...
                    logger.info(f"✅ Telegram успех! {len(tg_text)} символов")
                    break
                else:
                    logger.warning(f"⚠️ Telegram не прошел валидацию: {len(tg_processed)} символов, нужно {tg_min}-{tg_max}")
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                if zen_min <= len(zen_processed) <= zen_max:
                    ...
                    logger.info(f"✅ Zen успех! {len(zen_text)} символов")
                    break
                else:
                    logger.warning(f"⚠️ Zen не прошел валидацию: {len(zen_processed)} символов, нужно {zen_min}-{zen_max}")
