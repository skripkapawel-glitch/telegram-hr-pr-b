...
3. Обязательно добавь смысловой якорь
...
Смысловой якорь — отдельный абзац, фиксирующий значение или последствия без морали и клише.
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                # Ищем признаки смыслового якоря
                anchor_markers = [
                    ...
                    'Отсюда вывод', 'Это значит', 'Это приводит к',
                ...
                has_hashtags = 2 <= hashtag_count <= 5
                
                # Проверяем пустые строки между абзацами
                paragraphs = [p.strip() for p in generated_zen.split('\n') if p.strip()]
                has_paragraphs = len(paragraphs) >= 4  # Минимум: шапка, 2 абзаца, вопрос, хештеги
                
                if has_question and has_anchor and has_hashtags and has_paragraphs and zen_min <= zen_length <= zen_max:
                    zen_text = generated_zen
                    logger.info(f"✅ Zen успех! {zen_length} символов (нужно {zen_min}-{zen_max})")
                    break
                else:
                    logger.warning(f"⚠️ Zen не прошел проверку: вопрос={has_question}, якорь={has_anchor}, хештеги={has_hashtags}({hashtag_count}), абзацы={has_paragraphs}, длина={zen_length}")
