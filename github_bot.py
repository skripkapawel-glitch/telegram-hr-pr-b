...
{slot_style['emoji']} Шапка — провокационный вопрос или утверждение.

Первый абзац. Свободный вход в мысль. Любой тип начала: тезис, наблюдение, сомнение, идея, утверждение, абстракция, конкретика — без ограничений.

Второй абзац. Любое развитие мысли. Углубление, расширение, уточнение, противопоставление, логическое продолжение.

🎯 Важно: одна ключевая мысль, сформулированная явно и конкретно.

Вопрос к читателю.

#хештег1 #хештег2 #хештег3 (от 2 до 5 хештегов)

Тема: "{theme}"
Картинка: {image_description}

Правила:
1. Начинай СРАЗУ с {slot_style['emoji']} и шапки
2. Обязательно добавь ВОПРОС к читателю
3. Обязательно выдели ключевую мысль с 🎯
4. Между абзацами оставляй пустую строку
5. Хештеги: от 2 до 5 штук
6. Пиши естественно, но точно следуй структуре
        """
        return prompt.strip()
...
Шапка — провокационный вопрос или утверждение.

Первый абзац. Развёртывание темы в любом формате: аналитика, размышление, наблюдение, логика, тенденция, идея.

Второй абзац. Развитие или усложнение мысли. Допускается смена угла, масштаба или акцента.

ВАЖНО: Смысловой якорь (один отдельный абзац, фиксирующий значение или последствия без морали и клише)
...
Правила:
1. ЗАПРЕЩЕНО использовать эмодзи, смайлики, символы (кроме текста)
2. Вопрос должен быть отдельной строкой и заканчиваться знаком ?
3. Обязательно добавь смысловой якорь как отдельный абзац
4. Между всеми абзацами оставляй пустую строку
5. Хештеги: от 2 до 5 штук
6. Пиши как человек, который хочет дискуссии

КРИТИЧЕСКОЕ ПРАВИЛО: Не используй символ 🎯 в тексте! Вместо этого начинай абзац с "Важно:", "Вывод:", "Смысл:" или подобного.
        """
        return prompt.strip()
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_tg:
                ...
                # УПРОЩЕННАЯ ПРОВЕРКА для Telegram:
                ...
                starts_with_emoji = generated_tg.strip().startswith(slot_style['emoji'])
                
                if starts_with_emoji and has_question and tg_min <= tg_length <= tg_max:
                    tg_text = generated_tg
                    logger.info(f"✅ Telegram успех! {tg_length} символов (нужно {tg_min}-{tg_max})")
                    break
                else:
                    logger.warning(f"⚠️ Telegram не прошел проверку: эмодзи={starts_with_emoji}, вопрос={has_question}, длина={tg_length}")
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                # Проверяем на эмодзи (но разрешаем маркер 🎯, если он используется как указатель в промпте)
                ...
                # Специальная проверка: разрешаем только маркер 🎯 если он есть, но проверяем другие эмодзи
                if '🎯' in generated_zen:
                    # Убираем маркер 🎯 для проверки других эмодзи
                    text_without_target = generated_zen.replace('🎯', '')
                    has_other_emoji = bool(emoji_pattern.search(text_without_target))
                    if has_other_emoji:
                        logger.warning(f"⚠️ Zen содержит запрещенные эмодзи (кроме разрешенного 🎯)")
                        continue
                    else:
                        logger.info("✅ Zen содержит только разрешенный маркер 🎯 для якоря")
                else:
                    # Проверяем любые эмодзи
                ...
                # Ищем признаки смыслового якоря с маркерами (БЕЗ эмодзи в списке маркеров)
                anchor_markers = [
                    # Русские формулировки
                    'Важно:', 'Смысловой якорь', 'Значит', 'Поэтому', 'Таким образом', 'Следовательно', 
                    'В итоге', 'Итак', 'Важно понимать', 'Ключевой момент', 'Суть в том',
                    'Это означает', 'Последствия', 'Итог', 'Вывод', 'Что это значит',
                    'Фиксируем', 'Закрепляем', 'Подведём итог', 'Резюмируя',
                    'Смысл в том', 'Основная мысль', 'Главный вывод', 'Из этого следует',
                    'Следовательно', 'Отсюда вывод', 'Это значит', 'Это приводит к',
                    'В результате', 'Как следствие', 'Подводя черту', 'Обобщая',
                    # Английские варианты (Gemini может иногда использовать английский)
                    'Important:', 'Key takeaway:', 'The bottom line:', 'What this means:',
                    'The significance:', 'The implication:', 'In essence:', 'To summarize:',
                    'Therefore:', 'Thus:', 'Hence:', 'As a result:', 'Consequently:',
                    'In conclusion:', 'Ultimately:', 'The key point:', 'The main idea:',
                    'So what does this mean?', "What's the takeaway?", 'The core message:'
                ]
                
                # Сначала ищем явные маркеры
                has_anchor = any(marker in generated_zen for marker in anchor_markers)
                
                # Если не нашли явных маркеров, ищем смысловой якорь по содержанию
                if not has_anchor and zen_length > 500:
                    paragraphs = [p.strip() for p in generated_zen.split('\n') if p.strip()]
                    
                    # Ищем возможное расположение якоря
                    anchor_candidates = []
                    
                    # 1. Проверяем абзацы перед хештегами
                    hashtag_index = -1
                    ...
                    if hashtag_index > 0:
                        # Берем 2-3 абзаца перед хештегами
                        start_idx = max(0, hashtag_index - 3)
                        anchor_candidates.extend(paragraphs[start_idx:hashtag_index])
                    
                    # 2. Проверяем абзацы после вопроса (если есть)
                    question_indices = [i for i, p in enumerate(paragraphs) if p.strip().endswith('?')]
                    if question_indices:
                        last_question_idx = question_indices[-1]
                        if last_question_idx + 1 < len(paragraphs):
                            # Берем абзацы после вопроса
                            start_idx = last_question_idx + 1
                            end_idx = min(start_idx + 2, len(paragraphs))
                            anchor_candidates.extend(paragraphs[start_idx:end_idx])
                    
                    # 3. Проверяем последние абзацы перед концом
                    if len(paragraphs) > 2:
                        anchor_candidates.extend(paragraphs[-3:])
                    
                    # Проверяем кандидатов на содержание
                    anchor_candidates = list(dict.fromkeys(anchor_candidates))  # Убираем дубли
                    
                    for candidate in anchor_candidates:
                        candidate_lower = candidate.lower()
                        
                        # Проверяем признаки итогового/резюмирующего текста
                        is_summary = (
                            len(candidate) > 50 and  # Достаточная длина
                            not candidate.startswith('#') and  # Не хештег
                            not candidate_lower.startswith('вопрос') and  # Не вопрос
                            '?' not in candidate and  # Не содержит вопроса
                            any(word in candidate_lower for word in [
                                'значит', 'поэтому', 'итак', 'в итоге', 'следовательно',
                                'вывод', 'итог', 'резюме', 'суть', 'смысл', 'значение',
                                'последствия', 'результат', 'заключение', 'обобщение'
                            ])
                        )
                        
                        if is_summary:
                            has_anchor = True
                            logger.info(f"✅ Найден смысловой якорь по содержанию: {candidate[:100]}...")
                            break
                
                if has_question and has_anchor and zen_min <= zen_length <= zen_max:
                    zen_text = generated_zen
                    logger.info(f"✅ Zen успех! {zen_length} символов (нужно {zen_min}-{zen_max})")
                    break
                else:
                    logger.warning(f"⚠️ Zen не прошел проверку: вопрос={has_question}, якорь={has_anchor}, длина={zen_length}")
