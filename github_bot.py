...
ADMIN_CHAT_ID = os.environ.get("ADMIN_CHAT_ID")  # НОВОЕ: ID администратора
...
print(f"👑 ADMIN_CHAT_ID: {'✅ Установлен' if ADMIN_CHAT_ID else '❌ Отсутствует'}")
            ...
            if APPROVAL_AVAILABLE and ADMIN_CHAT_ID:
                ...
                try:
                    success = send_for_approval(
                        tg_text=tg_text,
                        zen_text=zen_text,
                        tg_image=tg_image_url,
                        zen_image=zen_image_url,
                        theme=self.current_theme,
                        time_slot=f"{time_key} ({time_slot_info['name']})"
                    )
                    
                    if success:
                        logger.info("✅ Посты отправлены администратору на согласование")
                        ...
                        logger.info("\n" + "=" * 60)
                        logger.info(f"📨 Посты отправлены на согласование!")
                        logger.info("=" * 60)
                        ...
                        logger.info("=" * 60)
                        logger.info("👆 Проверьте Telegram для согласования постов")
                        logger.info("=" * 60)
                        
                        return True
                    else:
                        logger.error("❌ Не удалось отправить на согласование, публикую сразу")
...
def main():
    """Главная функция"""
    print("\n" + "=" * 80)
    print("🤖 GITHUB BOT: ГЕНЕРАЦИЯ ПОСТОВ с системой согласования")
    print("=" * 80)
    print("📋 РЕЖИМ РАБОТЫ:")
    
    if APPROVAL_AVAILABLE and ADMIN_CHAT_ID:
        print("   ✅ СИСТЕМА СОГЛАСОВАНИЯ ВКЛЮЧЕНА")
        print("   👑 Администратор получит посты для утверждения")
    else:
        print("   ⚠️ СИСТЕМА СОГЛАСОВАНИЯ ОТКЛЮЧЕНА")
        print("   📤 Посты публикуются автоматически")
    ...
    print("  1. Проверяем Gemini AI...")
    if bot.test_gemini_access():
        print("     ✅ Gemini доступен")
    else:
        print("     ❌ Gemini недоступен")
        print("     Проверьте GEMINI_API_KEY в GitHub Secrets")
        sys.exit(1)
    ...
    if success:
        ...
        if APPROVAL_AVAILABLE and ADMIN_CHAT_ID:
            print("✅ ПОСТЫ ОТПРАВЛЕНЫ НА СОГЛАСОВАНИЕ!")
            print("   📲 Проверьте Telegram для утверждения")
        else:
