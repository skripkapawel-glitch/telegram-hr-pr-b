...
ТЕМА: {theme}
ФОРМАТ: {text_format}
ЦЕЛЕВАЯ ДЛИНА: {target_length} символов (СТРОГО не менее {tg_min} символов и не более {tg_max} символов)
...
ВАЖНЫЕ ПРАВИЛА:
• МИНИМУМ {tg_min} символов, МАКСИМУМ {tg_max} символов
• Если меньше {tg_min} символов - ДОБАВЬ подробностей
• Если больше {tg_max} символов - СОКРАТИ текст
...
ТВОЙ ПОСТ:"""
    ...
    def create_zen_prompt(self, theme, slot_info, text_format):
        ...
        try:
            ...
            # Для Дзена ставим цель - среднее значение
...
ТЕМА: {theme}
ФОРМАТ: {text_format}
СТРОГОЕ ТРЕБОВАНИЕ: МИНИМУМ {zen_min} символов, МАКСИМУМ {zen_max} символов
ИДЕАЛЬНАЯ ДЛИНА: {target_length} символов

СТРУКТУРА (сокращенная):
1. Заголовок: 1 короткая строка
2. Введение: 2-3 предложения
3. Основная часть: 3-4 предложения (развернуто)
4. Заключение: 2-3 предложения
5. Вопрос для обсуждения: 1 короткий вопрос
6. {closing}
7. Хэштеги: {hashtags_str}

ВАЖНЕЙШИЕ ПРАВИЛА:
• АБСОЛЮТНЫЙ МИНИМУМ: {zen_min} символов
• АБСОЛЮТНЫЙ МАКСИМУМ: {zen_max} символов
• Если меньше {zen_min} - ДОБАВЬ подробностей и примеров
• Если больше {zen_max} - СОКРАТИ текст
• Никаких эмодзи в тексте
• Никаких "Длина:" или счетчиков
• Никаких ** вокруг текста
• Короткие предложения, но развернутые мысли

ПРИМЕР КОРОТКОГО ПОСТА (примерно {target_length} символов):
Стратегии развития персонала в современном бизнесе

Эффективное развитие персонала - ключ к успеху компании. Современный бизнес требует постоянного обучения сотрудников.

HR используют различные методы: тренинги, менторство, онлайн-курсы. Важно персонализировать программы под каждого сотрудника и учитывать его потребности.

Инвестиции в развитие окупаются производительностью и лояльностью персонала. Компании с сильной системой обучения привлекают лучших специалистов.

Какие методы развития работают у вас? Какие подходы вы считаете наиболее эффективными?
...
МИНИМУМ {zen_min} СИМВОЛОВ! МАКСИМУМ {zen_max} СИМВОЛОВ! НИКАКИХ ЭМОДЗИ!
...
ТВОЙ КОРОТКИЙ ПОСТ:"""
    ...
    def _expand_text(self, text, target_min, target_max, text_type, current_length):
        """
        Расширяет текст, если он слишком короткий
        """
        logger.info(f"📈 Расширяем {text_type}: {current_length} → {target_min}-{target_max}")
        
        deficit = target_min - current_length
        target_expand = min(deficit + 100, target_max - current_length)
        
        expand_prompt = f"""РАСШИРЬ ЭТОТ ТЕКСТ. ДОБАВЬ {target_expand} СИМВОЛОВ.

ТЕКСТ:
{text}

ЗАДАЧА:
• Сделай текст длиннее на {target_expand} символов
• Добавь примеры, детали, объяснения
• Сохрани основную мысль и структуру
• Добавь больше подробностей
• Не трогай заголовок, вопрос и хэштеги
• Итог: не менее {target_min} символов

ПРАВИЛА РАСШИРЕНИЯ:
1. Добавь конкретные примеры
2. Раскрой основные мысли подробнее
3. Добавь практические рекомендации
4. Объясни причинно-следственные связи
5. Оставь структуру неизменной

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
РАСШИРЕННЫЙ ТЕКСТ (не менее {target_min} символов):"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in result and result['candidates']:
                    expanded_text = result['candidates'][0]['content']['parts'][0]['text'].strip()
                    expanded_text = self.clean_generated_text(expanded_text)
                    new_len = len(expanded_text)
                    
                    logger.info(f"📊 После расширения: {new_len} символов")
                    
                    if target_min <= new_len <= target_max:
                        logger.info(f"✅ {text_type} успешно расширен")
                        return expanded_text
            
            return None
            
        except Exception as e:
            logger.error(f"❌ Ошибка при расширении текста: {str(e)[:100]}")
            return None

    def _aggressive_rewrite(self, text, target_min, target_max, text_type, current_length):
        """
        Агрессивная перезапись для слишком длинных текстов
        """
...
СОКРАЩЕННЫЙ ТЕКСТ (максимум {target_max} символов):"""
    ...
    def generate_with_retry(self, prompt, target_min, target_max, post_type, max_attempts=3):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Для Дзена используем более высокую температуру для более подробного текста
                temperature = 0.4 if "Дзен" in post_type else 0.3
                max_tokens = int(target_max * 1.5)  # Больше токенов для более длинных текстов
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        if length < target_min and attempt < max_attempts - 1:
                            logger.info(f"📈 {post_type} слишком короткий, расширяем...")
                            expanded = self._expand_text(
                                generated_text,
                                target_min,
                                target_max,
                                post_type,
                            ...
                            if expanded:
                                ...
                                if target_min <= expanded_len <= target_max:
                                    logger.info(f"✅ {post_type} успешно расширен: {expanded_len} символов")
                                    return expanded
                        
                        # Если текст слишком длинный - сокращаем
                        if length > target_max and attempt < max_attempts - 1:
                            logger.info(f"🔥 {post_type} слишком длинный, сокращаем...")
                            ...
                            if rewritten:
                                ...
                                if target_min <= rewritten_len <= target_max:
                                    logger.info(f"✅ {post_type} успешно сокращен: {rewritten_len} символов")
                                    return rewritten
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            zen_text = self.generate_with_retry(zen_prompt, zen_min, zen_max, "Дзен пост", max_attempts=5)  # Больше попыток для Дзен
