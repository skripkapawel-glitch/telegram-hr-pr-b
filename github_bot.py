...
⚠️ ⚠️ ⚠️ ВАЖНЕЙШЕЕ ТРЕБОВАНИЕ: СТРОГОЕ СОБЛЮДЕНИЕ ЛИМИТОВ СИМВОЛОВ ⚠️ ⚠️ ⚠️

ТОЧНЫЕ ЛИМИТЫ СИМВОЛОВ ДЛЯ ЭТОГО ПОСТА (НЕ ПРИБЛИЗИТЕЛЬНО, А ТОЧНО!):

Telegram (@da4a_hr): ТОЧНО {tg_min}-{tg_max} символов
• НИ МЕНЬШЕ {tg_min} символов
• НИ БОЛЬШЕ {tg_max} символов
• Идеально: около {(tg_min + tg_max) // 2} символов

Дзен (@tehdzenm): ТОЧНО {zen_min}-{zen_max} символов
• НИ МЕНЬШЕ {zen_min} символов
• НИ БОЛЬШЕ {zen_max} символов
• Идеально: около {(zen_min + zen_max) // 2} символов

❗️ КРИТИЧЕСКИ ВАЖНО: После создания текста проверь его длину!
Используй len(текст) в Python. Результат ДОЛЖЕН быть:
• Telegram: len(текст) >= {tg_min} и len(текст) <= {tg_max}
• Дзен: len(текст) >= {zen_min} и len(текст) <= {zen_max}

Если текст не соответствует этим лимитам — перепиши его заново!

AI обязательно:

1. СТРОГО соблюдает структуру
2. СТРОГО соблюдает лимиты символов (ПОВТОРЯЮ: TG {tg_min}-{tg_max}, Дзен {zen_min}-{zen_max})
...
📏 📏 📏 ТОЧНЫЕ ЛИМИТЫ СИМВОЛОВ (ПОВТОРЯЮ ЕЩЕ РАЗ):

ДЛЯ TELEGRAM: ТОЧНО {tg_min}-{tg_max} СИМВОЛОВ
• МИНИМУМ: {tg_min}
• МАКСИМУМ: {tg_max}
• Идеальная длина: {(tg_min + tg_max) // 2}

ДЛЯ ДЗЕН: ТОЧНО {zen_min}-{zen_max} СИМВОЛОВ
• МИНИМУМ: {zen_min}
• МАКСИМУМ: {zen_max}
• Идеальная длина: {(zen_min + zen_max) // 2}

❗️ ЕСЛИ ТЕКСТ НЕ СООТВЕТСТВУЕТ ЭТИМ ЛИМИТАМ — ОН БУДЕТ ОТБРАКОВАН!

ФОРМАТ ВЫВОДА:

TG:
[Текст для Telegram, ТОЧНО {tg_min}-{tg_max} символов, готовый к публикации]
---
DZEN:
[Текст для Дзен, ТОЧНО {zen_min}-{zen_max} символов, готовый к публикации]

⚠️ ОКОНЧАТЕЛЬНЫЕ ТРЕБОВАНИЯ:

1. Telegram текст: {tg_min}-{tg_max} символов (len() должен показать это)
2. Дзен текст: {zen_min}-{zen_max} символов (len() должен показать это)
3. ДВА РАЗНЫХ текста для разных платформ
4. Telegram: с эмодзи, Дзен: без эмодзи
5. Мягкий вовлекающий финал в конце
6. Учесть время: {slot_time} МСК ({time_style})
7. Использовать формат: {text_format}
8. НИКАКИХ вводных фраз
9. Списки через •
10. Текст готов к публикации

ПРОВЕРЬ ДЛИНУ ТЕКСТОВ ПЕРЕД ОТПРАВКОЙ!"""

        logger.info(f"📝 Создан СТРОГИЙ промпт для Gemini")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'candidates' in result and result['candidates']:
                            ...
                            logger.info(f"📊 Длина всего ответа: {len(generated_text)} символов")
    ...
    def validate_and_fix_structure(self, text, is_telegram=True):
        ...
        text = re.sub(r'^(Вот|Держи|Пожалуйста|Смотри|Вот тебе|Я создал|Я подготовил|Как тебе|Привет|Здравствуйте|Хорошо|Так|Итак).+?\n', '', text, flags=re.IGNORECASE)
    ...
    def format_telegram_text(self, text, slot_info):
        ...
        if not is_valid:
            # НЕ исправляем, просто возвращаем ошибку
            logger.error(f"❌ Telegram текст не соответствует лимитам: {length} символов")
            return None  # Возвращаем None вместо исправленного текста
    ...
    def format_zen_text(self, text, slot_info):
        ...
        if not is_valid:
            # НЕ исправляем, просто возвращаем ошибку
            logger.error(f"❌ Дзен текст не соответствует лимитам: {length} символов")
            return None  # Возвращаем None вместо исправленного текста
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            logger.info(f"📊 Длина текстов после минимальной обработки:")
            ...
            if tg_length < tg_min or tg_length > tg_max:
                logger.error(f"❌ КРИТИЧНО: Telegram текст все еще не соответствует лимитам: {tg_length}")
                return False
            
            if zen_length < zen_min or zen_length > zen_max:
                logger.error(f"❌ КРИТИЧНО: Дзен текст все еще не соответствует лимитам: {zen_length}")
                return False
