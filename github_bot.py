...
class TelegramBot:
    ...
    def create_detailed_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        total_tokens = slot_style['total_tokens']
...
📊 РАСПРЕДЕЛЕНИЕ ТОКЕНОВ ПО СЛОТАМ:

11:00 Утренний слот:
Telegram: 260 токенов
Дзен: 310 токенов
Итог токенов: 570 на 2 поста

15:00 Дневной слот:
Telegram: 400 токенов
Дзен: 400 токенов
Итог токенов: 800 токенов на 2 поста

20:00 Вечерний слот:
Telegram: 400 токенов
Дзен: 350 токенов
Итог токенов: 750 токенов на 2 поста

Текущий слот: {slot_style['name']} ({slot_style['emoji']})
Лимиты для этого слота:
Telegram: {tg_min}-{tg_max} символов (максимум {tg_tokens} токенов)
Дзен: {zen_min}-{zen_max} символов (максимум {zen_tokens} токенов)
Общий лимит токенов для двух постов: {total_tokens} токенов
...
Telegram пост ДОЛЖЕН быть строго {tg_min}-{tg_max} символов (максимум {tg_tokens} токенов).
Дзен пост ДОЛЖЕН быть строго {zen_min}-{zen_max} символов (максимум {zen_tokens} токенов).
Общий лимит токенов для двух постов: {total_tokens} токенов.
...
• Объём: {tg_min}-{tg_max} символов (максимум {tg_tokens} токенов) (ОБЯЗАТЕЛЬНО!)
...
• 3-5 релевантных хештегов в конце
• Объём: {zen_min}-{zen_max} символов (максимум {zen_tokens} токенов) (ОБЯЗАТЕЛЬНО!)
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя всем правилам выше и соблюдая лимиты токенов для каждого поста отдельно и общий лимит для двух постов."""
