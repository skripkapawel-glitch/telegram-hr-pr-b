...
СОЗДАЙ ДВА ТЕКСТА:
1. Telegram пост: {tg_min}-{tg_max} символов
2. Дзен пост: {zen_min}-{zen_max} символов
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔥 КРИТИЧЕСКОЕ ТРЕБОВАНИЕ №1: ЛИМИТ СИМВОЛОВ — НЕ ПРЕВЫШАТЬ И НЕ ЗАНИЖАТЬ 🔥

⚠️  Telegram пост ДОЛЖЕН БЫТЬ ТОЧНО {tg_min}-{tg_max} символов. НЕ {tg_min-1}, НЕ {tg_max+1}.
⚠️  Дзен пост ДОЛЖЕН БЫТЬ ТОЧНО {zen_min}-{zen_max} символов. НЕ {zen_min-1}, НЕ {zen_max+1}.
⚠️  Символы считаются ВКЛЮЧАЯ пробелы и знаки препинания.
⚠️  Текст, не соответствующий этим лимитам, будет ОТБРАКОВАН и НЕ будет использован.

🔥 КРИТИЧЕСКОЕ ТРЕБОВАНИЕ №2: ПРОВЕРЬ ДЛИНУ КАЖДОГО ТЕКСТА 🔥

Перед отправкой ответа:
1. Посчитай символы в Telegram тексте. Результат должен быть между {tg_min} и {tg_max}.
2. Посчитай символы в Дзен тексте. Результат должен быть между {zen_min} и {zen_max}.
3. Если текст не соответствует — ПЕРЕПИШИ ЕГО.
4. Не отправляй текст, пока он не будет соответствовать.

СТРУКТУРА TELEGRAM ({tg_min}-{tg_max} символов):
• Эмодзи обязательны (один в начале)
...
СТРУКТУРА ДЗЕН ({zen_min}-{zen_max} символов):
• Эмодзи ЗАПРЕЩЕНЫ
• Заголовок
• 2-4 абзаца
• Мини-вывод
...
ВНИМАНИЕ: В тексте НЕ должно быть меток вида [TG: 550 символов]. Просто текст.

ФОРМАТ ОТВЕТА:
TG:
[Твой Telegram пост здесь, {tg_min}-{tg_max} символов]
---
DZEN:
[Твой Дзен пост здесь, {zen_min}-{zen_max} символов]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 ПОВТОРЯЮ: ЕСЛИ len(текст) < {tg_min} ИЛИ len(текст) > {tg_max} ДЛЯ TELEGRAM — ПЕРЕПИШИ!
🚨 ПОВТОРЯЮ: ЕСЛИ len(текст) < {zen_min} ИЛИ len(текст) > {zen_max} ДЛЯ ДЗЕН — ПЕРЕПИШИ!"""

        logger.info(f"📝 Создан ЖЁСТКИЙ строгий промпт для Gemini")
    ...
    def generate_with_gemini(self, prompt, tg_max=None, zen_max=None, temperature=0.0, top_p=0.6, max_retries=3):
        ...
        try:
            ...
            available_models = [
                "gemma-3-27b-it",  # Эта модель точно работает
                "gemini-1.5-flash",  # Резервная модель
    ...
    def strict_length_validation(self, text, min_chars, max_chars, text_type):
        """СТРОГАЯ валидация длины БЕЗ ИСПРАВЛЕНИЙ - только проверка"""
        if not text:
    ...
    def format_telegram_text(self, text, slot_info):
        """Форматирует текст для Telegram - МИНИМАЛЬНАЯ обработка"""
        if not text:
            return None
        ...
        # Добавляем эмодзи слота только если его нет
    ...
    def format_zen_text(self, text, slot_info):
        """Форматирует текст для Дзен - МИНИМАЛЬНАЯ обработка"""
        if not text:
            return None
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            # Финальная проверка после форматирования (на всякий случай)
            ...
            # Финальная проверка (двойная гарантия)
    ...
    def run_once_mode(self):
        ...
        print(f"📏 ЖЁСТКИЕ лимиты: Telegram {slot_info['tg_chars'][0]}-{slot_info['tg_chars'][1]} символов")
        print(f"📏 ЖЁСТКИЕ лимиты: Дзен {slot_info['zen_chars'][0]}-{slot_info['zen_chars'][1]} символов")
        print(f"🎯 Система ротации тем: одинаковые темы не будут идти подряд")
        print(f"🚨 Текст будет НЕМЕДЛЕННО ОТБРАКОВАН, если не соответствует лимитам!")
