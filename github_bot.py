    ...
    def process(self, raw_text: str) -> str:
        ...
        if self.post_type == 'telegram':
            ...
            structural_elements = [
                ...
                ('ВОПРОС ДЛЯ АУДИТОРИИ:', 'вопрос для вовлечения')
            ]
            ...
            for element, default_content in structural_elements:
                ...
                # Проверяем, есть ли вообще элемент
                if element not in raw_text:
                    # Добавляем недостающий элемент
            ...
            zen_elements = [
                ...
                ('ВОПРОС ДЛЯ ОБСУЖДЕНИИ:', 'вопрос для дискуссии')
            ]
            ...
            for element, default_content in zen_elements:
                ...
                if pattern.search(raw_text):
                    ...
                    if re.search(f'{re.escape(element)}\\s*\\[(ключевые выводы|практические следствия|обобщающая мысль|вопрос для дискуссии)\\]', raw_text, re.IGNORECASE):
                        raw_text = pattern.sub(f'{element} {default_content}', raw_text)
                        logger.warning(f"⚠️ Заменен шаблон {element}")
                
                # Проверяем, есть ли вообще элемент (только для заключительных)
                if element in ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']:
                    if element not in raw_text:
                        # Добавляем первый заключительный элемент, если нет ни одного
                        if not any(e in raw_text for e in ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']):
                            raw_text += f"\n\n{element} {default_content}"
                            logger.warning(f"⚠️ Добавлен заключительный элемент: {element}")
            
            # Проверяем ВОПРОС ДЛЯ ОБСУЖДЕНИЯ
            if 'ВОПРОС ДЛЯ ОБСУЖДЕНИИ:' not in raw_text and 'ВОПРОС ДЛЯ ОБСУЖДЕНИЯ:' not in raw_text:
                raw_text += f"\n\nВОПРОС ДЛЯ ОБСУЖДЕНИИ: вопрос для дискуссии"
...
1. НАЧАЛО: {slot_style['emoji']} [цепляющий заголовок без лишних символов]
   ЗАПРЕЩЕНО начинать с "Доброе утро", "Добрый день", "Добрый вечер", "Начало дня", "Старт утра" и любых других приветствий!
   Начинай сразу с эмодзи и заголовка.
2. ОСНОВНАЯ ЧАСТЬ: 2-3 абзаца с анализом и примерами
3. ПРАКТИЧЕСКИЙ БЛОК:
   🎯 Важно: [конкретная ключевая мысль, а не "содержимое"]
   📋 Шаги: [конкретные действия, а не "содержимое"]
   🔧 Практика: [конкретные рекомендации, а не "содержимое"]
4. ВОПРОС ДЛЯ АУДИТОРИИ: [конкретный вопрос для вовлечения, а не "содержимое"]
5. ХЕШТЕГИ: {hashtags}
...
• ЗАПРЕЩЕНО оставлять [содержимое] - заменяй конкретным текстом
• Каждый структурный элемент должен содержать реальный текст, а не шаблон
...
• Приветствия типа "Доброе утро", "Добрый день", "Добрый вечер"
...
3. ЗАКЛЮЧЕНИЕ (выбери ОДИН вариант):
   Почему это важно: [конкретные ключевые выводы, а не "ключевые выводы"]
   или
   Что из этого следует: [конкретные практические следствия, а не "практические следствия"]
   или
   Мнение экспертов: [конкретная обобщающая мысль, а не "обобщающая мысль"]
4. ВОПРОС ДЛЯ ОБСУЖДЕНИЯ: [конкретный вопрос для дискуссии, а не "вопрос для дискуссии"]
5. ХЕШТЕГИ: {hashtags}
...
• Хештеги ВКЛЮЧЕНЫ в общий лимит символов
• Пост должен быть ЗАКОНЧЕННЫМ, не обрываться на полуслове
• Обязательно включи ОДИН из заключительных блоков (Почему это важно, Что из этого следует, Мнение экспертов) с КОНКРЕТНЫМ текстом
• ЗАПРЕЩЕНО оставлять шаблоны типа [ключевые выводы], [практические следствия], [обобщающая мысль], [вопрос для дискуссии]
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_tg:
                ...
                # Дополнительная проверка на шаблоны
