...
— Выход за пределы диапазона считается ошибкой.

СТРУКТУРА И ОБЪЁМ TELEGRAM-ПОСТА:
1. Вступление (с эмодзи {slot_style['emoji']}) — 80-120 символов
2. Основная часть — {tg_min-150}-{tg_max-150} символов
3. Практический блок — 100-150 символов
4. Хештеги — 50-80 символов

Дзен-пост:
— Общий объём: {zen_min}-{zen_max} символов.
— Выход за пределы диапазона считается ошибкой.

СТРУКТУРА И ОБЪЁМ ДЗЕН-ПОСТА:
1. Крючок-убийца (без эмодзи) — 100-150 символов
2. Основная часть — {zen_min-250}-{zen_max-250} символов
3. Блок завершения — 150-200 символов
4. Хештеги — 50-80 символов

ПРАВИЛА:
— Не добавлять новые блоки
— Не объединять абзацы
— Не писать дисклеймеры
— Не комментировать процесс

АЛГОРИТМ (ОБЯЗАТЕЛЬНО СОБЛЮДАТЬ):
1. Мысленно распределить объём по блокам для каждого поста
2. Написать текст строго по структуре
3. Проверить, что суммарный объём находится в диапазоне
4. Если итоговый объём выходит за диапазон, результат считается неверным
5. В этом случае текст должен быть переписан заново
6. Вывести ТОЛЬКО итоговый текст

ВАЖНО:
Gemma НЕ понимает «символы» как абстракцию. Она мыслит токенами.
Точные значения (например, {tg_max} символов) она не считает, она оценивает.
Поэтому работай только с диапазонами и структурными якорями.
"""
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше."""
    ...
    def validate_parsed_texts(self, tg_text, zen_text, tg_min, tg_max, zen_min, zen_max):
        ...
        try:
            ...
            if len(tg_text) > tg_max * 1.5:
                tg_text = self._force_cut_text(tg_text, tg_max * 1.2)
                logger.info(f"⚔️ Telegram текст обрезан до {len(tg_text)} символов")
            
            if len(zen_text) > zen_max * 1.5:
                zen_text = self._force_cut_text(zen_text, zen_max * 1.2)
