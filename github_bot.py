...
class TelegramBot:
        ...
        self.zen_conclusions = {
            ...
            'expert_insights': {
                ...
                'structure': ['По данным исследования...', 
                            'Эксперты отмечают...', 
                            'Тренд показывает...'],
                'probability': 0.2  # 20% постов
            },
            'no_special_section': {
                'title': None,  # Без специального блока
                'probability': 0.1  # 10% постов - чистое завершение
            }
        }
    ...
    def select_conclusion_type(self, post_type='zen'):
        ...
        return list(conclusions.values())[0]  # Fallback
    ...
    def generate_conclusion_block(self, conclusion_type, theme):
        ...
        if not conclusion_type or conclusion_type.get('title') is None:
            return ""  # Пост без специального блока завершения
...
3. "Мнение экспертов:" с цитатами/данными (20% постов)
4. Простое завершение без специального блока (10% постов)
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше."""
    ...
    def format_zen_text(self, text, slot_style):
        ...
        # Теперь проверяем наличие ЛЮБОГО блока завершения
        ...
        if not has_conclusion_block:
            ...
            for i, line in enumerate(lines):
                if '?' in line or '!' in line:
                    if i + 2 < len(lines):
                        lines.insert(i + 2, '')
                        # Выбираем случайный тип завершения
                        conclusion_type = self.select_conclusion_type('zen')
                        if conclusion_type and conclusion_type.get('title'):
                            lines.insert(i + 3, conclusion_type['title'])
                        break
...
<b>📢 После истечения времени посты будут автоматически отклонены</b>
    ...
    def validate_post_structure(self, text, post_type):
        ...
        if post_type == 'telegram':
            ...
            # Обновленная проверка 2: Есть ли ЛЮБОЙ блок завершения (не обязательно "Почему это важно")
            has_conclusion = any(
                marker in text for marker in [
                    'Почему это важно:', 
                    'Что из этого следует:', 
            ...
            if not has_conclusion:
                return False, "❌ Дзен пост должен содержать любой блок завершения (Почему это важно/Что из этого следует/Мнение экспертов)"
