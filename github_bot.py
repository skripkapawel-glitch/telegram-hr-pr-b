...
📝 Создай два разных текста по одной теме. Оба текста должны быть уникальными по структуре, но об одном смысле."""
    ...
    def parse_generated_texts(self, text, tg_min, tg_max, zen_min, zen_max):
        ...
        try:
            ...
            # УБРАНО: Проверка наличия хештегов в середине процесса
            # Все проверки хештегов теперь только в конце
    ...
    def format_telegram_text(self, text, slot_style):
        ...
        # УБРАНО: Проверка наличия хештегов в процессе форматирования
        # ЕДИНСТВЕННАЯ ПРОВЕРКА ХЕШТЕГОВ В КОНЦЕ
        if not re.findall(r'#\w+', text):
            logger.warning("⚠️ Хештеги не найдены в тексте. Добавляю...")
            hashtags = self.get_relevant_hashtags(self.current_theme or "HR и управление персоналом", 5)
            text = f"{text}\n\n{' '.join(hashtags)}"
