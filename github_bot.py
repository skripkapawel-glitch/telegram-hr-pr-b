...
ВАЖНО: Увеличь объем токенов, чтобы посты писались полноценно! Иногда посты обрезаются, поэтому сделай текст достаточно длинным и насыщенным.
...
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС. УЧТИ ВСЕ ТРЕБОВАНИЯ ВЫШЕ."""
    ...
    def generate_with_gemini(self, prompt, max_retries=3):
        """Генерирует текст через Gemini с увеличенными токенами"""
        for attempt in range(max_retries):
            try:
                ...
                data = {
                    ...
                    "generationConfig": {
                        "temperature": 0.9,  # Немного выше для креативности
                        "maxOutputTokens": 4000,  # УВЕЛИЧЕНО для полноценных постов
                        "topP": 0.95,
                        "topK": 40
                    }
                }
                ...
                response = session.post(url, json=data, timeout=45)  # Увеличиваем таймаут
    ...
    def add_telegram_hashtags(self, text, theme):
        ...
        all_hashtags = base_hashtags[:3] + general_hashtags[:3]
        hashtags_to_add = random.sample(all_hashtags, min(random.randint(3, 6), len(all_hashtags)))
