...
class SmartPostGenerator:
    ...
    def add_theme_to_history(self, channel_id, theme):
        ...
        # Добавляем тему и ограничиваем историю
    ...
    def get_smart_theme(self, channel_id):
        ...
        # Если все темы исключены, используем все
    ...
    def generate_zen_post(self, theme):
        ...
        prompt = f"""
        Напиши аналитический пост для Яндекс.Дзен на тему "{theme}" за 2024-2025 год.
...
Техническим навыкам можно научить, а мышление меняется долго.

Прозрачность процесса найма
Внедрите пробный день вместо многоэтапных собеседований.

Постоянная обратная связь
Давайте фидбек даже тем, кого не взяли.""",
...
Создавайте карты профессионального роста.

Баланс работы и личных интересов
Работа над персональными проектами снижает выгорание.

Открытая коммуникация
Встречи должны фокусироваться на целях и состоянии."""
            ],
...
Люди доверяют людям, а не компаниям.

Видеоконтент вместо текстового
Короткие видео собирают в 3 раза больше просмотров.

Диалог с аудиторией вместо монолога
Комментарии стали важнее контента.""",
                
                """Трансформация PR-коммуникаций

45% используют искусственный интеллект, аудитория доверяет микро-инфлюенсерам.
...
Демонстрация закулисья создает доверие.

Формирование сообщества
Ответы на комментарии создают активное сообщество."""
            ],
...
Использование местных материалов экологичнее.

Технологии как основа
Заложите умные системы на этапе отделки.

Гибкость пространства
Универсальная база дешевле нового ремонта.""",
...
Цифровые двойники помогают устранить ошибки.

Прозрачность для заказчика
Онлайн-дневник стройки информирует клиента.

Оптимизация через технологии
BIM-моделирование показывает проблемы заранее."""
            ]
        }
