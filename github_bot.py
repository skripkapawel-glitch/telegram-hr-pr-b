...
class TelegramBot:
    def __init__(self, target_slot=None, auto=False):
        ...
        self.trends_by_theme = {
            "HR и управление персоналом": [
                "HR1аналитика и data-driven решения",
            ...
            "ремонт и строительство": [
                ...
                "BIM1технологии на всех этапах",
                "Эко1материалы нового поколения",
...
• Вопрос для обсуждения с аудиторией
• 3-5 релевантных хештегов в конце
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше. Лаконично, без воды, строго в пределах лимита."""
    ...
    def format_post_text(self, text, slot_style, post_type):
        ...
        for i, line in enumerate(lines):
            ...
            # Добавляем пустые строки между логическими блоками:
            if line and i < len(lines) - 1:
                next_line = lines[i+1] if i+1 < len(lines) else ""
                
                # После шапки (эмодзи в начале)
                if i == 0 and any(e in line[:5] for e in ['🌅', '🌞', '🌙']):
                    formatted_lines.append('')
                
                # Перед практическим блоком
                elif any(marker in next_line.lower() for marker in 
                        ['🎯', 'важно:', '📋', 'действия:', '🔧', 'практич', 'совет', 'шаг']):
                    formatted_lines.append('')
                
                # Перед полезняшкой (статистика, исследование)
                elif any(marker in next_line.lower() for marker in 
                        ['исследовани', 'отчёт', 'данн', 'статистик', 'анализ', 'кейс']):
                    formatted_lines.append('')
                
                # Перед вопросами к аудитории
                elif any(marker in next_line for marker in ['?', 'Что думаете', 'Ваше мнение', 'Обсудим']):
                    formatted_lines.append('')
                
                # Перед выводом/заключением
                elif any(marker in next_line.lower() for marker in 
                        ['💡', 'инсайт', 'вывод', 'заключен', 'итог']):
                    formatted_lines.append('')
        
        # 3. Собираем обратно
        result_text = '\n'.join(formatted_lines)
        
        # 4. Обеспечиваем 2 пустые строки перед хештегами
        hashtag_pattern = r'(\n+)(#[\w\u0400-\u04FF]+(?:\s+#[\w\u0400-\u04FF]+)*\s*)$'
        ...
        # 5. Дополнительная обработка для Telegram
        if post_type == 'telegram':
        ...
        # 7. Финальная очистка (не более 2 пустых строк подряд)
...
"""
    ...
    def _finalize_post_structure(self, text, post_type, theme, min_chars, max_chars):
        ...
        # Удаляем шаблонные блоки завершения
        template_patterns = [
            r'Почему это важно:.*?(?=\n\n|$)',
