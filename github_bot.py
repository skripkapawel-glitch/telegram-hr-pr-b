...
1. ШАПКА: {slot_style['emoji']} + Вопрос/утверждение по теме (цепляющий, провокационный или интригующий).

2. Основная часть: Анализ явления, кейсы, данные, исследования.

3. Практический блок: Что делать с этой информацией, конкретные шаги.

4. БЛОК ПОЛЕЗНЯШКИ (ЕСЛИ ЕСТЬ РЕАЛЬНЫЙ ИСТОЧНИК):
   • Формат: [Фраза-мостик] [Название]. Организация: [Название]. Год: [Год]. Ссылка: [URL].
   • Фразы-мостики (использовать одну):
     · Это наблюдение подтверждается исследованием:
     · Похожий вывод встречается в отраслевом отчёте:
     · Данный тезис опирается на данные:
     · Аналогичный подход рассматривается в работе:
   • Если источника нет — этот блок ПРОПУСТИТЬ.

5. Мини-вывод или ключевая мысль (инсайт).

6. Мягкий финал: {soft_final}

7. Хештеги: {hashtags_str}

📝 СТРУКТУРА ДЗЕН (СТРУКТУРА «КРЮЧОК-УБИЙЦА»):
1. КРЮЧОК-УБИЙЦА (БЕЗ ЭМОДЗИ!)
Первая строка — провокационный вопрос, заявление или неочевидный факт, который ломает привычную картину. НИКАКИХ ЭМОДЗИ!

2. СУТЬ ЗА 15 СЕКУНД
Второй абзац — только сухие факты. Что произошло, что представлено или обнародовано, без оценок.

3. ПОЧЕМУ ЭТО ВАЖНО (2-3 пункта с маркерами •)
...
4. ПОЛЕЗНЯШКА (если есть реальный источник)
• Формат: [Фраза-мостик] [Название]. Организация: [Название]. Год: [Год]. Ссылка: [URL].
• Фразы-мостики (использовать одну):
  · Это наблюдение подтверждается исследованием:
  · Похожий вывод встречается в отраслевом отчёте:
...
5. ВОПРОС ВАМ
Открытый вопрос к аудитории, который превращает пассивное чтение в диалог.

6. ХЕШТЕГИ: {hashtags_str}
...
5. Дзен пост — КРЮЧОК-УБИЙЦА БЕЗ ЭМОДЗИ ВООБЩЕ (абсолютно никаких эмодзи!)
6. Хештеги ТОЛЬКО В КОНЦЕ каждого поста
7. Мягкий финал — вопрос к аудитории, приглашение к дискуссии
8. Сохранять ограничения по символам (помни: хештеги включаются в подсчет)
9. Оба поста должны быть РАЗНЫМИ по структуре, но об одном смысле

🔒 ДОПОЛНИТЕЛЬНЫЕ ПРАВИЛА СТРУКТУРЫ ДЗЕН:
1. Строгая структура «Крючок-убийца» с пустыми строками между блоками:
   - Крючок-убийца (первая строка, БЕЗ ЭМОДЗИ!)
   - ПУСТАЯ СТРОКА
   - Суть за 15 секунд
   - ПУСТАЯ СТРОКА
   - Почему это важно (с маркерами •)
   - ПУСТАЯ СТРОКА
   - Полезняшка (если есть источник)
   - ПУСТАЯ СТРОКА
   - Вопрос вам
   - ПУСТАЯ СТРОКА
   - Хештеги

🔒 ДОПОЛНИТЕЛЬНЫЕ ПРАВИЛА СТРУКТУРЫ TELEGRAM:
1. Структура с пустыми строками между блоками:
   - Шапка с эмодзи {slot_style['emoji']}
   - ПУСТАЯ СТРОКА
   - Основная часть
   - ПУСТАЯ СТРОКА
   - Практический блок
...
• Потом Дзен версия (с шапкой без эмодзи, строгая структура «Крючок-убийца»)
• Разделитель: три дефиса (---)
...
Создай два РАЗНЫХ текста по одной теме. Оба должны быть уникальными по структуре, но об одном смысле."""
    ...
    def parse_generated_texts(self, text, tg_min, tg_max, zen_min, zen_max):
        ...
        try:
            ...
            # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА СТРУКТУРЫ ДЗЕН
            # Проверяем наличие маркеров для списков
            if '•' not in zen_text and re.search(r'\d+\.', zen_text) is None:
                # Если нет маркеров, добавляем их к пунктам "Почему это важно"
                logger.warning("⚠️ В Дзен посте нет маркеров списка. Восстанавливаю структуру...")
                ...
                for i, line in enumerate(zen_lines):
                    stripped = line.strip()
                    if stripped and len(stripped) > 20 and not stripped.startswith('#') and '?' not in stripped and '!' not in stripped:
                        # Это может быть пункт списка
                        if i > 0 and zen_lines[i-1].strip() == '' and not found_important_section:
                            # Начинаем секцию "Почему это важно"
                            found_important_section = True
                            restored_lines.append("Почему это важно:")
                            restored_lines.append('')
                            restored_lines.append(f"• {stripped}")
                        elif found_important_section and stripped and not stripped.startswith('•'):
                            restored_lines.append(f"• {stripped}")
                        else:
    ...
    def format_zen_text(self, text, slot_style):
        ...
        for i, line in enumerate(lines):
            ...
            is_important = '•' in line_stripped or line_stripped.lower().startswith(('контекст:', 'сдвиг:', 'импликация:'))
            is_useful_source = any(keyword in line_stripped.lower() for keyword in [
            ...
            if is_important or (current_block == 2 and not is_useful_source and not is_question and not is_hashtag):
                if current_block < 2:
                    ...
                    structured_lines.append('')
                if '•' not in line_stripped and not line_stripped.lower().startswith(('контекст:', 'сдвиг:', 'импликация:')):
