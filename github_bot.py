...
class TelegramBot:
    ...
    def create_telegram_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        prompt = f"""
Создай пост для Telegram по теме "{theme}".

Формат поста:
1. {slot_style['emoji']} [Заголовок]
2. [Первый абзац: начало мысли]
3. [Второй абзац: развитие мысли]
4. [Третий абзац: углубление или поворот] (можно пропустить)
5. [Ключевая мысль] (выдели как важное)
6. [Вопрос к читателям]
7. [Хештеги по теме]
...
- Ключевая мысль должна быть в конце перед вопросом
- Хештеги: 2-3 штуки
...
Картинка: {image_description}
    ...
    def create_zen_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        prompt = f"""
Создай пост для Яндекс.Дзен по теме "{theme}".

Формат поста:
1. [Заголовок-провокация]
2. [Первый абзац: разворачивание темы]
3. [Второй абзац: анализ или сравнение]
4. [Третий абзац: смысловой поворот] (можно пропустить)
...
- Без эмодзи
- Естественный стиль
- После абзацев оставляй пустую строку
- Вывод должен подводить итог
- Хештеги: 2-3 штуки
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                has_conclusion = any(marker in generated_zen for marker in ['Вывод', 'Итог', 'В заключение', 'Таким образом', 'Поэтому'])
