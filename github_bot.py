...
1. ИСТОРИИ/РАССКАЗЫ (со словами: "коллега", "история", "случай", "знакомый", "делился"):
   • Формат: пункты БЕЗ отступов, с эмодзи в начале пункта
   • Стиль: живой, повествовательный
   • Пример:
     🔥 Коллега поделился историей...
     
     • 🎯 Знакомый HR устраивал "День открытых дверей"...
     • ⏰ Пришел парень, весь такой уверенный...
     • 🤔 HR спрашивает: "А что вы реально умеете делать?"...
     
     А у вас были смешные случаи? 👇
...
3. ОБЫЧНЫЕ ПОСТЫ (аналитика, наблюдения, мнения):
   • Формат: абзацы без пунктов
   • Стиль: аналитический, но живой
...
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС!"""
    ...
    def format_telegram_text(self, text):
        ...
        for i, line in enumerate(lines):
            ...
            # Если это история с пунктами - БЕЗ отступов
            if is_story_post and line.startswith('•'):
                # Добавляем случайный эмодзи если его нет
                if not any(emoji in line for emoji in ['🎯', '⏰', '🤔', '💡', '🔥', '🙈', '⭐', '📌', '👉', '❗', '⚠️']):
                    point_emojis = ['🎯 ', '⏰ ', '🤔 ', '💡 ', '🔥 ', '⭐ ', '📌 ', '👉 ', '❗ ', '⚠️ ']
                    emoji = random.choice(point_emojis)
                    line = "• " + emoji + line[1:].strip()
                formatted_lines.append(line)
            
            # Если это список/перечисление (не история) - С отступами
...
def main():
    ...
    print("=" * 80)
    print("📋 Особенности:")
    print("   • Умное форматирование Telegram (истории без отступов, списки с отступами)")
    print("   • Яндекс.Дзен: аналитический стиль без эмодзи")
    print("   • Обязательные хештеги и закрывашки в обоих каналах")
