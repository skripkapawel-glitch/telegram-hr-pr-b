...
class TextPostProcessor:
    """Класс для интеллектуальной пост-обработки текстов постов"""
    
    # Константы для структурного анализа
    PRACTICE_MARKERS = ['🎯 Важно:', '📋 Шаги:', '🔧 Практика:', 'Практический совет:', 'Что делать дальше:', 'Конкретные действия:']
    CONCLUSION_MARKERS = ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']
    
    # Словарь для исправления грамматических ошибок Gemini
    GRAMMAR_CORRECTIONS = {
        'привращаются': 'превращаются',
        'привращается': 'превращается',
        'методикующей': 'металлической',  # по контексту
        'свободного дома по 20%': 'экономии до 20%',
        'юг': 'юг',  # контекстно-зависимое, может быть нормальным
    }
    
    # Паттерны "воды" для удаления (избыточные выражения)
    WATER_PATTERNS = [
        r'очень\s+',
        r'крайне\s+',
        r'невероятно\s+',
        ...
        r'фактически\s+',
        r'в\s+реальности\s+',
        r'на\s+самом\s+деле\s+',
        r'как\s+известно\s*,?\s*',
        r'как\s+правило\s*,?\s*',
        r'обычно\s+',
        r'чаще\s+всего\s+',
        r'в\s+большинстве\s+случаев\s+',
        r'безусловно\s*,?\s*',
        r'несомненно\s*,?\s*',
        r'разумеется\s*,?\s*',
        r'конечно\s*же\s*,?\s*',
    ...
    def __init__(self, theme, slot_style, post_type):
        self.theme = theme
        self.slot_style = slot_style
        self.post_type = post_type
        self.min_chars, self.max_chars = self._get_char_limits()
        
    def _get_char_limits(self):
        """Получает лимиты символов для текущего типа поста и слота"""
        if self.post_type == 'telegram':
            return self.slot_style['tg_chars']
        else:  # 'zen'
            return self.slot_style['zen_chars']
    
    def process(self, raw_gemini_text):
        """
        Основной пайплайн обработки текста
        """
        if not raw_gemini_text or len(raw_gemini_text.strip()) < 50:
            return raw_gemini_text
            
        logger.info(f"🔧 Начинаю пост-обработку {self.post_type} поста ({len(raw_gemini_text)} символов)")
        
        # 1. Структурный анализ
        structure = self._analyze_structure(raw_gemini_text)
        
        # 2. Структурная коррекция
        corrected_text = self._correct_structure(raw_gemini_text, structure)
        
        # 3. Интеллектуальное сокращение
        shortened_text = self._intelligently_shorten(corrected_text)
        
        # 4. Грамматическая коррекция
        cleaned_text = self._fix_grammar_errors(shortened_text)
        
        # 5. Финальное форматирование
        final_text = self._apply_formatting(cleaned_text)
        
        # Валидация результата
        validation_result = self._validate(final_text)
        if validation_result['valid']:
            logger.info(f"✅ Пост-обработка завершена успешно: {len(final_text)} символов")
            return final_text
        else:
            logger.warning(f"⚠️ Пост прошел обработку с предупреждениями: {validation_result['warnings']}")
            # Все равно возвращаем результат, но логируем проблемы
            return final_text
    
    def _analyze_structure(self, text):
        """
        Анализирует текст и определяет:
        - Где вступление (первые 2-3 предложения)
        - Где основная часть  
        - Где практические блоки (TG) или блок завершения (ZEN)
        - Где хештеги (если есть)
        - Где вопросы к аудитории
        """
        structure = {
            'introduction': {'start': 0, 'end': 0},
            'main_content': {'start': 0, 'end': 0},
            'practical_block': None,
            'conclusion_block': None,
            'hashtags': None,
            'questions': [],
        ...
        # Определяем вступление (первые 2-3 предложения)
        intro_end = min(3, len(sentences))
        ...
        # Находим позиции блоков
        for marker in self.PRACTICE_MARKERS:
            if marker in text:
                start = text.find(marker)
                if start != -1:
                    # Ищем конец блока (следующий маркер или конец абзаца)
                    end = len(text)
                    for next_marker in self.PRACTICE_MARKERS:
        ...
        # Находим блок завершения для ZEN
        ...
        # Находим вопросы к аудитории
        question_pattern = r'[А-ЯA-Z][^.!?]*\?'
        for match in re.finditer(question_pattern, text):
            # Пропускаем вопросы в начале (это может быть крючок)
            if match.start() > 100:  # Не первые 100 символов
    ...
    def _correct_structure(self, text, structure):
        """
        ДОБАВЛЯЕТ недостающие структурные элементы:
        
        Для TG:
        1. Если нет эмодзи в начале - добавляет
        2. Если нет практического блока - генерирует и вставляет
        3. Если нет хештегов - добавляет
        
        Для ZEN:
        1. Если нет крючка-убийца - добавляет вопрос
        2. Если нет блока завершения - генерирует
        3. Если есть эмодзи - удаляет
        """
        result = text
        
        if self.post_type == 'telegram':
            # 1. Гарантируем эмодзи в начале
            ...
            # 2. Гарантируем практический блок
            if not structure['has_practice']:
                ...
                if practical_block:
                    ...
                    if structure['hashtags']:
                        hashtag_pos = structure['hashtags']['start']
                        result = result[:hashtag_pos].strip() + '\n\n' + practical_block + '\n\n' + result[hashtag_pos:].strip()
                    else:
                        result = result.strip() + '\n\n' + practical_block
                    logger.info("✅ Добавлен практический блок в Telegram пост")
            
            # 3. Удаляем эмодзи из середины текста (кроме начала)
            lines = result.split('\n')
            if len(lines) > 1:
                for i in range(1, len(lines)):
                    if any(e in lines[i][:5] for e in ['🌅', '🌞', '🌙']):
            ...
            # 2. Гарантируем крючок-убийца в начале (вопрос или восклицание)
            first_line = result.split('\n')[0] if '\n' in result else result
            if '?' not in first_line and '!' not in first_line and len(first_line) > 20:
                hook = self._generate_zen_hook()
                result = f"{hook}\n\n{result}"
                logger.info("✅ Добавлен крючок-убийца в начало Zen поста")
            
            # 3. Гарантируем блок завершения
    ...
    def _generate_practical_block(self):
        """Генерирует практический блок для Telegram поста"""
        templates = [
            ...
            "🔧 Практика: выделите 15 минут в день на отработку этого навыка."
        ]
        ...
        theme_templates = {
            "HR и управление персоналом": [
                ...
                "🔧 Практика: внедрите еженедельные 15-минутные встречи с командой."
            ],
            "PR и коммуникации": [
                ...
                "🔧 Практика: отвечайте на комментарии в соцсетях в течение 2 часов."
            ],
            "ремонт и строительство": [
                "🎯 Важно: качественная подготовка поверхностей экономит 40% времени на отделке.",
                "📋 Шаги: 1) составьте детальную смету, 2) закупите материалы с запасом 10%, 3) соблюдайте технологию работ.",
                "🔧 Практика: проверяйте уровень и плоскость после каждого этапа работ."
            ]
        }
        
        if self.theme in theme_templates:
            templates = theme_templates[self.theme]
        
        return random.choice(templates)
    
    def _generate_zen_hook(self):
        """Генерирует крючок-убийца для Zen поста"""
        hooks = {
            "HR и управление персоналом": [
                "Почему 70% сотрудников увольняются не из-за зарплаты?",
                "Как построить команду, которая работает как часы?",
                "Секрет удержания талантов, о котором молчат HR-директора."
            ],
            "PR и коммуникации": [
                "Почему даже идеальный продукт может провалиться без грамотного PR?",
                "Как завоевать доверие аудитории в эпоху информационного шума?",
                "Ошибки в коммуникациях, которые дорого обходятся брендам."
            ],
            "ремонт и строительство": [
                "Почему 90% ремонтов выходят за сроки и бюджет?",
                "Как избежать типичных ошибок при планировании ремонта?",
                "Секреты качественного ремонта, которые скрывают подрядчики."
            ]
        }
        
        if self.theme in hooks:
            return random.choice(hooks[self.theme])
        
        return "Что действительно важно в этой теме?"
    
    def _generate_conclusion_block(self):
        """Генерирует блок завершения для Zen поста"""
        markers = self.CONCLUSION_MARKERS
        
        conclusions = {
            "Почему это важно:": [
                "Понимание этой темы позволяет принимать более взвешенные решения и избегать типичных ошибок.",
                "Этот аспект часто недооценивают, хотя он напрямую влияет на результат.",
                "Без этого знания даже самые правильные действия могут не привести к успеху."
            ],
            "Что из этого следует:": [
                "Нужно пересмотреть текущие подходы и внести корректировки в свою стратегию.",
                "Стоит обратить внимание на эти моменты при планировании следующих шагов.",
                "Это открывает новые возможности для оптимизации и повышения эффективности."
            ],
            "Мнение экспертов:": [
                "Профессионалы в этой сфере сходятся во мнении, что ключ к успеху — в системном подходе.",
                "Опытные практики отмечают, что самое важное — не теория, а её адаптация к конкретным условиям.",
                "Эксперты подчеркивают: постоянное развитие и обучение — обязательное условие для роста."
            ]
        }
        
        marker = random.choice(markers)
        conclusion = random.choice(conclusions.get(marker, ["Это важно для достижения успеха в данной области."]))
        
        return f"{marker} {conclusion}"
    
    def _get_relevant_hashtags(self, count=3):
        """Возвращает релевантные хештеги для темы"""
        hashtags_by_theme = {
            "HR и управление персоналом": ["#HR", "#управлениеперсоналом", "#рекрутинг", "#команда", "#лидерство"],
            "PR и коммуникации": ["#PR", "#коммуникации", "#маркетинг", "#брендинг", "#соцсети"],
            "ремонт и строительство": ["#ремонт", "#строительство", "#дизайн", "#интерьер", "#ремонтквартир"]
        }
    ...
    def _intelligently_shorten(self, text):
        """
        СОКРАЩАЕТ текст до max_chars, НЕ ломая его
        
        Алгоритм (в порядке применения):
        
        1. Удалить "воду":
           - Повторяющиеся фразы
           - Избыточные прилагательные/наречия
           - Очевидные утверждения
        
        2. Сократить длинные описания:
           - "очень красивый современный дизайн" → "современный дизайн"
           - "невероятно важно и критически необходимо" → "важно"
        
        3. Упростить сложные предложения:
           - Разбить на 2 простых
           - Убрать сложные конструкции
        
        4. Удалить целые абзацы (с конца текста):
           - Сначала абзацы без структурных элементов
           - Потом абзацы в середине (кроме практических блоков)
           - НИКОГДА не удалять начало и структурные блоки
        
        5. Если все еще длиннее лимита:
           - Аккуратно обрезать по границам предложений
           - Обязательно оставить точку в конце
        """
        original_length = len(text)
        if original_length <= self.max_chars:
            return text
        
        logger.info(f"✂️ Начинаю интеллектуальное сокращение: {original_length} → {self.max_chars}")
        
        result = text
        
        # ШАГ 1: Удаление "воды" (избыточных выражений)
        water_removed = False
        ...
        if water_removed:
            logger.info("✅ Удалена 'вода' из текста")
        
        # Если после удаления воды текст уже в лимите - возвращаем
        if len(result) <= self.max_chars:
            logger.info(f"✅ Текст сокращен до {len(result)} символов (удаление воды)")
            return self._ensure_coherent_end(result)
        
        # ШАГ 2: Анализ структуры для сохранения ключевых блоков
        structure = self._analyze_structure(result)
        
        # ШАГ 3: Упрощение сложных предложений
        sentences = structure['sentences']
        if len(sentences) > 5:
            simplified_sentences = []
            for sentence in sentences:
                if len(sentence) > 150:  # Слишком длинное предложение
                    # Пытаемся разбить по союзам
                    parts = re.split(r'\s*(и|а|но|что|который|которая|которое|которые)\s+', sentence, maxsplit=1)
                    if len(parts) > 1:
            ...
            # Обновляем текст, если упростили
            if len(simplified_sentences) > len(sentences):
                result = ' '.join(simplified_sentences)
                logger.info("✅ Упрощены сложные предложения")
        
        # Если после упрощения текст уже в лимите
        if len(result) <= self.max_chars:
            logger.info(f"✅ Текст сокращен до {len(result)} символов (упрощение предложений)")
            return self._ensure_coherent_end(result)
        
        # ШАГ 4: Удаление абзацев с конца (кроме структурных блоков)
        lines = [line.strip() for line in result.split('\n') if line.strip()]
        
        # Определяем, какие абзацы можно удалять
        removable_indices = []
        for i in range(len(lines) - 1, -1, -1):
            ...
            # НЕ удаляем:
            # 1. Первые 2 строки (вступление)
            if i < 2:
                continue
                
            # 2. Строки со структурными маркерами
            is_structural = False
            if self.post_type == 'telegram':
                if any(marker in line for marker in self.PRACTICE_MARKERS):
                    is_structural = True
            else:  # zen
                if any(marker in line for marker in self.CONCLUSION_MARKERS):
                    is_structural = True
            
            # 3. Строки с хештегами
            if '#' in line and i == len(lines) - 1:
                is_structural = True
            
            if not is_structural and line:
                # Удаляем этот абзац
                line_length = len(line) + 1  # +1 за перенос строки
                if len(result) - line_length >= self.max_chars * 0.8:  # Оставляем минимум 80%
                    removable_indices.append(i)
                    # Обновляем предполагаемую длину
                    result_length = len('\n'.join([lines[j] for j in range(len(lines)) if j not in removable_indices]))
        
        # Удаляем выбранные абзацы
        if removable_indices:
            new_lines = [lines[i] for i in range(len(lines)) if i not in removable_indices]
            result = '\n'.join(new_lines)
            logger.info(f"✅ Удалено {len(removable_indices)} абзацев с конца")
        
        # Если после удаления абзацев текст уже в лимите
        if len(result) <= self.max_chars:
            logger.info(f"✅ Текст сокращен до {len(result)} символов (удаление абзацев)")
            return self._ensure_coherent_end(result)
        
        # ШАГ 5: Последнее средство - аккуратная обрезка по предложениям
        if len(result) > self.max_chars:
            result = self._cut_by_sentences(result, self.max_chars)
            logger.info(f"✅ Текст сокращен до {len(result)} символов (обрезка по предложениям)")
        
        return self._ensure_coherent_end(result)
    
    def _ensure_coherent_end(self, text):
        ...
        if last_end > 0 and last_end > len(text) * 0.8:  # Если точка в последних 80% текста
            text = text[:last_end + 1].strip()
        
        # Если текст не заканчивается знаком препинания - добавляем точку
        if text and text[-1] not in '.!?':
            text = text + '.'
        
        return text
    
    def _cut_by_sentences(self, text, max_chars):
        """Аккуратно обрезает текст по границам предложений"""
        sentences = re.split(r'(?<=[.!?])\s+', text)
    ...
    def _fix_grammar_errors(self, text):
        """
        ИСПРАВЛЯЕТ очевидные грамматические ошибки от Gemini
        ...
        for error, correction in self.GRAMMAR_CORRECTIONS.items():
            if error in result:
                result = result.replace(error, correction)
                logger.info(f"✅ Исправлена ошибка: '{error}' → '{correction}'")
        
        # Удаляем обрывы предложений (... в конце или середине)
        result = re.sub(r'\w+\.\.\.\s*$', '', result)  # Обрывы в конце
        result = re.sub(r'\s\.\.\.\s', ' ', result)    # Обрывы в середине
    ...
    def _apply_formatting(self, text):
        ...
        1. Правильные переносы строк:
           - Пустая строка после шапки
           - Пустая строка перед практическими блоками
           - 2 пустые строки перед хештегами
        
        2. Эмодзи (только для TG):
           - Гарантировать эмодзи в начале
           - Удалить лишние эмодзи в середине
        
        3. Хештеги:
           - Гарантировать 3-5 релевантных хештегов
           - Размещать ТОЛЬКО в конце
           - С 2 пустыми строками перед ними
        
        4. Проверка пунктуации:
           - Каждое предложение заканчивается .!?
           - Нет множественных знаков препинания
        """
        if not text:
            return text
        ...
        # 1. Форматирование переносов строк
        lines = [line.strip() for line in result.split('\n')]
        formatted_lines = []
        
        for i, line in enumerate(lines):
            if not line:
                # Не добавляем несколько пустых строк подряд
            ...
            # Добавляем пустую строку после шапки (первая строка с контентом)
            if i == 0 and line and len(line) > 10:
                formatted_lines.append('')
            
            # Добавляем пустую строку перед практическими блоками (TG)
            if self.post_type == 'telegram' and i < len(lines) - 1:
                next_line = lines[i + 1] if i + 1 < len(lines) else ""
                if any(marker in next_line for marker in self.PRACTICE_MARKERS):
                    formatted_lines.append('')
            
            # Добавляем пустую строку перед блоками завершения (ZEN)
            if self.post_type == 'zen' and i < len(lines) - 1:
                next_line = lines[i + 1] if i + 1 < len(lines) else ""
                if any(marker in next_line for marker in self.CONCLUSION_MARKERS):
                    formatted_lines.append('')
        ...
        if self.post_type == 'telegram':
            ...
            if not any(e in result[:10] for e in ['🌅', '🌞', '🌙']) and 'emoji' in self.slot_style:
                result = f"{self.slot_style['emoji']} {result}"
            
            # Удаляем лишние эмодзи в середине (кроме практических маркеров)
            lines = result.split('\n')
            for i in range(1, len(lines)):
                line = lines[i]
                # Сохраняем эмодзи в практических маркерах
        ...
        if hashtag_match:
            # Убеждаемся, что перед хештегами 2 пустые строки
            hashtags = hashtag_match.group()
            text_without_hashtags = result[:hashtag_match.start()].strip()
            
            # Удаляем все переносы в конце и добавляем 2 пустые строки
            text_without_hashtags = text_without_hashtags.rstrip('\n')
            if not text_without_hashtags.endswith('\n\n'):
                text_without_hashtags = text_without_hashtags + '\n\n'
            
            result = text_without_hashtags + hashtags.strip()
        else:
            # Добавляем хештеги
            hashtags = self._get_relevant_hashtags(3)
        ...
        # Гарантируем, что каждый абзац начинается с заглавной буквы
        paragraphs = result.split('\n')
        for i, para in enumerate(paragraphs):
            if para and para[0].islower():
                paragraphs[i] = para[0].upper() + para[1:]
        result = '\n'.join(paragraphs)
    ...
    def _validate(self, text):
        ...
        if self.post_type == 'telegram':
            if not any(e in text[:50] for e in ['🌅', '🌞', '🌙']):
                warnings.append("Нет эмодзи в начале Telegram поста")
            if not any(marker in text for marker in self.PRACTICE_MARKERS):
                warnings.append("Нет практического блока в Telegram посте")
        else:  # ZEN
            if any(e in text for e in ['🌅', '🌞', '🌙']):
                warnings.append("Есть эмодзи в Zen посте (не должно быть)")
            if not any(marker in text for marker in self.CONCLUSION_MARKERS):
                warnings.append("Нет блока завершения в Zen посте")
        ...
        forbidden_patterns = [
            ...
            r'методикующей',
            r'юг\.\.\.',
            r'\w+\.\.\.\s*$'  # обрывы в конце
        ]
        
        for pattern in forbidden_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                warnings.append(f"Найден запрещенный паттерн: {pattern}")
        
        # ТЕСТ 4: Проверка связности
        sentences = re.split(r'[.!?]+', text)
        if len(sentences) < 3:
            warnings.append("Текст слишком фрагментарный (мало предложений)")
        
        # Проверка наличия хештегов
        hashtags = re.findall(r'#[\w\u0400-\u04FF]+', text)
        if len(hashtags) < 2:
            warnings.append(f"Мало хештегов: {len(hashtags)}")
...
class TelegramBot:
    ...
    def create_complete_remake_post(self, original_message_id, original_post_data, selected_theme):
        ...
        try:
            ...
            # ФИНАЛЬНАЯ ОБРАБОТКА с использованием нового процессора
            tg_processor = TextPostProcessor(selected_theme, slot_style, 'telegram')
            zen_processor = TextPostProcessor(selected_theme, slot_style, 'zen')
            
            tg_text = tg_processor.process(tg_text)
            zen_text = zen_processor.process(zen_text)
            ...
            if new_text:
                ...
                if '---' in new_text:
                    ...
                    if len(parts) == 2:
                        ...
                        # Используем новый процессор для обработки
    ...
    def generate_with_retry(self, prompt, tg_min, tg_max, zen_min, zen_max, max_attempts=3):
        ...
        for attempt in range(max_attempts):
            ...
            if tg_text and zen_text:
                ...
                # ГАРАНТИЯ: TextPostProcessor ДОЛЖЕН был обеспечить лимиты
...
"""
    ...
    def send_to_admin_for_moderation(self, slot_time, tg_text, zen_text, image_url, theme):
        ...
        # Используем новый процессор для финальной обработки
        tg_min, tg_max = self.current_style['tg_chars']
        zen_min, zen_max = self.current_style['zen_chars']
        
        tg_processor = TextPostProcessor(theme, self.current_style, 'telegram')
        zen_processor = TextPostProcessor(theme, self.current_style, 'zen')
