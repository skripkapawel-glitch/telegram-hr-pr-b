...
[4] ВОПРОС: Задай ПОЛНЫЙ вопрос читателю. Отдельная строка. Заканчивается знаком ?. Вопрос должен отличаться от заголовка. ВОПРОС ДОЛЖЕН БЫТЬ ПОЛНЫМ И ЗАКОНЧЕННЫМ.
...
4. Вопрос заканчивается знаком ? и должен быть ПОЛНЫМ
...
9. ВОПРОС В БЛОКЕ [4] ДОЛЖЕН БЫТЬ ПОЛНОЦЕННЫМ И ЗАКОНЧЕННЫМ ПРЕДЛОЖЕНИЕМ
"""
        return prompt.strip()
...
"""
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        ...
        # Telegram проверка хештегов - ИСПРАВЛЕНА ЛОГИКА
        if post_type == 'telegram':
            hashtag_pattern = r'#\w{2,}'
            hashtags = re.findall(hashtag_pattern, text)
