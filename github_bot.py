...
class TelegramBot:
    ...
    def regenerate_post_text(self, theme, slot_style, original_text, edit_request):
        ...
        try:
            ...
            prompt = f"""🎯 УЛЬТИМАТИВНАЯ РОЛЬ

ТЫ — СТРАТЕГИЧЕСКИЙ КОНТЕНТ-РАЗВЕДЧИК И ГЕНЕРАТОР СТРУКТУРИРОВАННОГО КОНТЕНТА.
Синтез нейросети-аналитика, контент-разведчика, тренд-проводника и эксперта по инсайтам.

Твоя задача — не просто написать пост, а совершить аналитическое вскрытие текущей реальности по теме и оформить результат строго в заданную структуру публикаций, соблюдая время, формат, ограничения и площадку.

ТВОЙ ОСНОВНОЙ ПРИНЦИП:
НЕ ОТВЕЧАТЬ НА ВОПРОС, А СОВЕРШАТЬ АНАЛИТИЧЕСКОЕ ВСКРЫТИЕ РЕАЛЬНОСТИ
...
ПЕРЕД ЛЮБОЙ ГЕНЕРАЦИЕЙ ТЫ АВТОМАТИЧЕСКИ:
	...
	2.	Выявляешь 3 ключевых напряжения в повестке за последние 72 часа
...
ПОЛУЧАЯ ТЕМУ → ТЫ СНАЧАЛА АНАЛИЗИРУЕШЬ, А ПОТОМ ПИШЕШЬ:
	...
	4.	Как это будет выглядеть через 30 дней (прогноз)?

🧬 УРОВНИ АНАЛИЗА РЕАЛЬНОСТИ

Ты одновременно работаешь с четырьмя слоями:
	...
	4.	Прогностический слой — к чему это ведёт

🔥 ФОРМУЛА КАЖДОГО ТЕКСТА

Каждый пост ОБЯЗАТЕЛЬНО содержит:
[АКТУАЛЬНЫЙ ТРИГГЕР ИЗ СЕГОДНЯ]
	•	[СКРЫТЫЙ ПАТТЕРН]
	•	[ПРАКТИЧЕСКИЙ ИНСАЙТ]
...
🕒 ВРЕМЕННОЙ СЛОТ
{slot_style['name']} ({slot_style['emoji']}) — {slot_style['style']}
...
⚠ КЛЮЧЕВЫЕ ПРАВИЛА
1. Telegram-пост обязательно начинается с эмодзи {slot_style['emoji']}
2. Дзен-пост — без эмодзи вообще
3. Форма подачи зависит от типа утверждения:
   • При описании аналитики, рассуждений, интерпретаций, выводов, синтеза информации → разрешено первое лицо (например: «я проанализировал», «я вижу паттерн», «я пришёл к выводу»)
   • При описании кейсов, практического опыта, отраслевых примеров, профессиональной экспертизы → обязательна нейтральная или третья форма, чтобы не создавать ложного впечатления личного участия
     Допустимые формулировки: «по отраслевой практике», «как отмечают специалисты», «по наблюдениям рынка», «в профессиональной среде считается»
4. Хештеги — только в конце поста
5. В каждом пост обязателен мягкий финал — вопрос
6. Привязка ко времени публикации обязательна
7. Нет «вечнозелёных» тем — только текущая повестка
8. Нет абстрактных советов
9. Нет повтора очевидного
10. Каждый текст встроен в контекст сегодняшнего дня

🚫 ЗАПРЕЩЕНО УПОМИНАТЬ
• Гибридный формат работы
• Удалённый формат работы (remote work)
• Релокацию
...
📝 ФОРМАТ ВЫВОДА
• Сначала версия Telegram (с эмодзи {slot_style['emoji']})
• Затем версия Дзен (без эмодзи)
• Разделитель: ---
• Без комментариев
• Без служебных фраз
• Только чистый текст, готовый к публикации
...
Переработай текст, сохраняя смысл, но учитывая запрос на редактирование и следуя ультимативному промту выше."""
    ...
    def get_smart_theme(self):
        ...
        try:
            ...
            # Находим тему, которая не повторялась в последних 3 постах
            available_themes = []
            for theme in self.themes:
    ...
    def clean_generated_text(self, text):
        ...
        try:
            ...
            for line in lines:
                ...
                # Удаляем строки с заголовками которые не должны быть в постах
...
<b>📢 После истечения времени посты будут автоматически отклонены</b>
    ...
    def create_and_send_posts(self, slot_time, slot_style, is_test=False):
        ...
        try:
            ...
            # ВОССТАНАВЛИВАЮ ОРИГИНАЛЬНЫЕ НАСТРОЙКИ
            tg_text, zen_text = self.generate_with_retry(prompt, tg_min, tg_max, zen_min, zen_max)
