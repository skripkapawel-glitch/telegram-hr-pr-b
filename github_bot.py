...
class TextPostProcessor:
    ...
    def _correct_structure(self, text: str, structure: Dict) -> str:
        ...
        if self.post_type == 'telegram':
            ...
            # Проверяем наличие хештегов и завершения
            if structure['hashtags'] is None:
                logger.warning("⚠️ Telegram пост не содержит хештегов")
                # Добавляем хештеги если их нет
                base_hashtags = {
                    "HR и управление персоналом": "#hr #персонал #управление #работа",
                    "PR и коммуникации": "#pr #коммуникации #маркетинг #связи",
                    "ремонт и строительство": "#ремонт #строительство #дом #дизайн"
                }
                hashtags = base_hashtags.get(self.theme, "#работа #бизнес #успех")
                result = f"{result.strip()}\n\n{hashtags}"
                logger.info("✅ Добавлены хештеги в Telegram пост")
            
            # Проверяем наличие практического блока или завершения
            if not structure['has_practice']:
                logger.warning("⚠️ Telegram пост не содержит практического блока")
        else:
            ...
            if structure['hashtags'] is None:
                ...
                hashtags = base_hashtags.get(self.theme, "#работа #бизнес #успех")
                result = f"{result.strip()}\n\n{hashtags}"
                logger.info("✅ Добавлены хештеги в Zen пост")
        
        return result
    
    def _generate_practical_block(self) -> str:
        """Генерирует практический блок"""
        if self.theme == "HR и управление персоналом":
            return "🎯 Важно: Определите ключевые компетенции для каждой должности.\n📋 Шаги: Проведите аудит навыков, разработайте план развития.\n🔧 Практика: Внедрите регулярные встречи один на один с сотрудниками."
        elif self.theme == "PR и коммуникации":
            return "🎯 Важно: Создайте единый месседж для всех каналов коммуникации.\n📋 Шаги: Определите целевую аудиторию, разработайте контент-план.\n🔧 Практика: Регулярно анализируйте отзывы и адаптируйте стратегию."
        elif self.theme == "ремонт и строительство":
            return "🎯 Важно: Составьте детальный план работ перед началом проекта.\n📋 Шаги: Проведите замеры, рассчитайте материалы, согласуйте сроки.\n🔧 Практика: Используйте чек-листы для контроля качества на каждом этапе."
        return ""
    
    def _generate_conclusion_block(self) -> str:
        """Генерирует блок завершения"""
        if self.theme == "HR и управление персоналом":
            return "Почему это важно: Компетентные сотрудники — главный актив компании. Инвестиции в развитие персонала окупаются многократно через повышение производительности и снижение текучки кадров."
        elif self.theme == "PR и коммуникации":
            return "Что из этого следует: Единая коммуникационная стратегия укрепляет репутацию бренда и повышает доверие аудитории. Последовательность в сообщениях создает узнаваемость и лояльность."
        elif self.theme == "ремонт и строительство":
            return "Мнение экспертов: Качественный ремонт требует тщательного планирования. Профессионалы рекомендуют уделять особое внимание подготовительному этапу — это экономит время и бюджет в дальнейшем."
        return ""
    ...
    def _apply_formatting(self, text: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            # Добавляем пустую строку после шапки (первого предложения с вопросом)
            if i == 0 and self.post_type == 'zen' and line.endswith('?'):
                formatted_lines.append('')
        
        result = '\n'.join(formatted_lines)
        
        # Обработка хештеги - сохраняем как есть
    ...
    def _validate(self, text: str) -> Dict:
        """Валидация обработанного текста - ВКЛЮЧАЯ ХЕШТЕГИ"""
        warnings = []
        text_length = len(text)  # Вся длина текста, включая хештеги
        
        if text_length < self.min_chars:
            warnings.append(f"Текст слишком короткий: {text_length} < {self.min_chars} (включая хештеги)")
        elif text_length > self.max_chars:
            warnings.append(f"Текст слишком длинный: {text_length} > {self.max_chars} (включая хештеги)")
...
Ты — опытный человек-писатель и редактор с более чем 20-летним стажем. Твоя задача — написать текст так, чтобы он был на 100% нераспознаваемым инструментами для выявления ИИ, такими как Grammarly, QuillBot, Turnitin и GPTZero.

ТРЕБОВАНИЯ К TELEGRAM ПОСТУ:
1. Начинай с эмодзи {slot_style['emoji']} и цепляющего заголовка
2. Основная часть: 2-3 абзаца с анализом и примерами
3. Практический блок с конкретными действиями (используй: 🎯 Важно:, 📋 Шаги:, 🔧 Практика:)
4. Вопрос для вовлечения аудитории
5. 3-5 релевантных хештегов в конце поста
6. ОБЯЗАТЕЛЬНО: Весь пост ВКЛЮЧАЯ ХЕШТЕГИ должен быть {tg_min}-{tg_max} символов
7. НЕ ДОБАВЛЯЙ ХЕШТЕГИ ОТДЕЛЬНО ОТ ЛИМИТА - ОНИ УЖЕ ВКЛЮЧЕНЫ В ПОДСЧЕТ!
8. ЗАПРЕЩЕНО начинать пост с символов ** или любых других форматирующих символов

ВАЖНОЕ ПРАВИЛО: Когда пишешь текст, сразу считай что хештеги займут ~50-80 символов. 
Значит основной текст должен быть примерно {tg_min}-{tg_max-80} символов, а с хештегами получится {tg_min}-{tg_max}.
...
Создай ПОЛНЫЙ Telegram пост по теме '{theme}' с хештегами в конце."""
...
1. Начало: провокационный вопрос или утверждение ("крючок-убийца")
2. Основная часть: глубина анализа, экспертные мнеты
3. Завершение: естественный вывод (можно использовать: 'Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:')
4. Вопрос для обсуждения
5. 3-5 релевантных хештегов в конце
6. ОБЯЗАТЕЛЬНО: Весь пост ВКЛЮЧАЯ ХЕШТЕГИ должен быть {zen_min}-{zen_max} символов
7. НИКАКИХ ЭМОДЗИ в посте!
8. НЕ ДОБАВЛЯЙ ХЕШТЕГИ ОТДЕЛЬНО ОТ ЛИМИТА - ОНИ УЖЕ ВКЛЮЧЕНЫ В ПОДСЧЕТ!
9. ЗАПРЕЩЕНО начинать пост с "##вечерний пост" или любой другой подобной разметки
10. После первого предложения с вопросом (крючка) должна быть ПУСТАЯ СТРОЧКА

ВАЖНОЕ ПРАВИЛО: Когда пишешь текст, сразу считай что хештеги займут ~50-80 символов. 
Значит основной текст должен быть примерно {zen_min}-{zen_max-80} символов, а с хештегами получится {zen_min}-{zen_max}.
...
Создай ПОЛНЫЙ Дзен пост по теме '{theme}' с хештегами в конце."""
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_tg:
                ...
                if tg_min <= len(tg_processed) <= tg_max:
                    tg_text = tg_processed
                    logger.info(f"✅ Telegram успех! {len(tg_text)} символов (включая хештеги)")
                    break
                else:
                    logger.warning(f"⚠️ Telegram не прошел валидацию: {len(tg_processed)} символов, нужно {tg_min}-{tg_max} (включая хештеги)")
            
            if attempt < max_attempts - 1:
                time.sleep(2 * (attempt + 1))
        
        # Генерируем Zen пост с увеличенным количеством попыток
        ...
        for attempt in range(max_attempts + 2):  # Увеличиваем количество попыток
            ...
            if generated_zen:
                ...
                if zen_min <= len(zen_processed) <= zen_max:
                    zen_text = zen_processed
                    logger.info(f"✅ Zen успех! {len(zen_text)} символов (включая хештеги)")
                    break
                else:
                    logger.warning(f"⚠️ Zen не прошел валидацию: {len(zen_processed)} символов, нужно {zen_min}-{zen_max} (включая хештеги)")
