...
class TelegramBot:
    ...
    def create_telegram_prompt(self, theme, slot_info, text_format):
        ...
        prompt = f"""ТОЛЬКО TELEGRAM ПОСТ - КРАТКИЙ ФОРМАТ

ТЕМА: {theme}
ФОРМАТ: {text_format}
ВРЕМЯ: {slot_time} МСК ({slot_info['emoji']})

🚨 КРИТИЧЕСКОЕ ТРЕБОВАНИЕ: ДЛИНА ТЕКСТА
Твой Telegram пост должен быть РОВНО от {tg_min} до {tg_max} символов.
Символы считаются ВСЕ: буквы, пробелы, знаки препинания, эмодзи.
ТЕКСТ БУДЕТ ПРОВЕРЕН И ОТБРАКОВАН если длина не в этом диапазоне.

✅ СТРУКТУРА (строго соблюдай):
1. {slot_info['emoji']} + цепляющая первая фраза
2. 1-2 коротких абзаца (по 2-3 предложения)
3. Мини-вывод (1 предложение)
...
- Не превышай {tg_max} символов
- Не пиши меньше {tg_min} символов

📝 ПРИМЕР НА {tg_min}-{tg_max} СИМВОЛОВ:
{slot_info['emoji']} Знаете ли вы, что простые решения часто самые эффективные?

Многие усложняют ремонт, начиная с масштабных проектов. Начните с малого: покраска стен или замена розеток уже преобразит пространство.

Простота — ключ к успеху в ремонте.

Какие простые изменения планируете в своем доме?
#ремонт #советы #интерьер

🔢 ВАЖНО: После написания ПОСЧИТАЙ символы (включая все знаки). 
Если больше {tg_max} — сократи текст. Если меньше {tg_min} — добавь деталей.
Только когда длина от {tg_min} до {tg_max} — отправляй текст.

ТВОЙ TELEGRAM ПОСТ (только текст, без пометок о длине):"""
    ...
    def create_zen_prompt(self, theme, slot_info, text_format):
        ...
        prompt = f"""ТОЛЬКО ДЗЕН ПОСТ - АНАЛИТИЧЕСКИЙ ФОРМАТ

ТЕМА: {theme}
ФОРМАТ: {text_format}

🚨 КРИТИЧЕСКОЕ ТРЕБОВАНИЕ: ДЛИНА ТЕКСТА
Твой Дзен пост должен быть РОВНО от {zen_min} до {zen_max} символов.
Символы считаются ВСЕ: буквы, пробелы, знаки препинания.
ТЕКСТ БУДЕТ ПРОВЕРЕН И ОТБРАКОВАН если длина не в этом диапазоне.

✅ СТРУКТУРА (строго соблюдай):
1. Заголовок (1 строка, без эмодзи)
2. 2-3 информативных абзаца (по 3-4 предложения)
3. Мини-вывод (1-2 предложения)
4. Вопрос для обсуждения
5. 2-3 хэштега

❌ ЧЕГО НЕ ДЕЛАТЬ:
- НИКАКИХ эмодзи
- Не делай введение длиннее одного абзаца
- Не повторяй одни и те же мысли
- Не превышай {zen_max} символов
- Не пиши меньше {zen_min} символов

📝 ПРИМЕР НА {zen_min}-{zen_max} СИМВОЛОВ:
Эффективное управление ремонтом: от планирования к реализации

Планирование ремонтных работ требует системного подхода. Важно заранее определить объемы, составить смету и утвердить сроки выполнения. Это позволяет избежать распространенных ошибок и незапланированных расходов.

Ключевой этап — подбор материалов и исполнителей. Качество используемых материалов напрямую влияет на долговечность результата, а профессионализм рабочих определяет скорость и аккуратность выполнения задач.

Регулярный контроль процесса позволяет вовремя вносить корректировки. Ежедневная проверка выполненных работ и общение с прорабом помогают поддерживать проект в рамках установленных параметров.

Системный подход к ремонту экономит время, ресурсы и нервы. Как вы организуете процесс ремонта в своем доме?
#ремонт #управлениепроектами #строительство

🔢 ВАЖНО: После написания ПОСЧИТАЙ символы (включая все знаки).
Если больше {zen_max} — сократи текст, убери лишние предложения.
Если меньше {zen_min} — добавь конкретных деталей, примеров.
Только когда длина от {zen_min} до {zen_max} — отправляй текст.

ТВОЙ ДЗЕН ПОСТ (только текст, без пометок о длине):"""

        return prompt

    def _estimate_max_tokens(self, target_chars):
        """Оценивает максимальное количество токенов на основе целевой длины"""
        # Русский: ~1 токен на 3.5 символа + запас 20%
        estimated = int(target_chars / 3.5 * 1.2)
        return min(max(estimated, 200), 1000)

    def generate_single_post(self, prompt, target_chars_min, target_chars_max, post_type, max_retries=3):
        """Генерирует ОДИН пост с жесткой проверкой длины"""
        max_tokens = self._estimate_max_tokens(target_chars_max)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.info(f"🤖 Генерация {post_type} (попытка {attempt}/{max_retries})")
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        # Проверяем длину
                        if target_chars_min <= length <= target_chars_max:
                            logger.info(f"✅ {post_type} прошел проверку длины")
                            return generated_text
                        else:
                            logger.warning(f"⚠️ {post_type} не прошел проверку длины")
                            
                            # Адаптируем промпт для следующей попытки
                            if length > target_chars_max:
                                prompt += f"\n\n⚠️ Твой текст был {length} символов — СЛИШКОМ ДЛИННЫЙ. Сократи до {target_chars_max} символов. Удали лишние предложения, упрости формулировки."
                                # Уменьшаем max_tokens
                                max_tokens = int(max_tokens * 0.8)
                            else:
                                prompt += f"\n\n⚠️ Твой текст был {length} символов — СЛИШКОМ КОРОТКИЙ. Добавь деталей, примеров, чтобы было от {target_chars_min} символов."
                ...
                logger.warning(f"⚠️ Ошибка при генерации {post_type}: {str(e)[:100]}")
        
        logger.error(f"❌ Не удалось сгенерировать {post_type} после {max_retries} попыток")
        return None
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            tg_text_raw = self.generate_single_post(tg_prompt, tg_min, tg_max, "Telegram пост", max_retries=3)
            ...
            zen_text_raw = self.generate_single_post(zen_prompt, zen_min, zen_max, "Дзен пост", max_retries=3)
    ...
    def run_once_mode(self):
        ...
        print(f"🚨 ПОШАГОВАЯ ГЕНЕРАЦИЯ: Сначала Telegram, потом Дзен")
        print(f"🔄 3 попытки для каждого поста с адаптацией промпта")
