...
print("=" * 80)
print("🚀 GITHUB BOT: ГЕНЕРАЦИЯ ПОСТОВ С УЛУЧШЕННЫМИ КОНЦОВКАМИ")
print("=" * 80)
...
class AIPostGenerator:
    def __init__(self):
        ...
        self.time_slots = {
            ...
            "14:00": {
                ...
                "name": "Дневной пост",
                "emoji": "🌞",
                "tg_chars": (800, 1000),     # Telegram: 800-1000
                "zen_chars": (700, 800),     # Дзен: 700-800
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
            ...
            "19:00": {
                ...
                "zen_chars": (800, 1000),    # Дзен: 800-1000
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
    ...
    def create_combined_prompt(self, theme, time_slot_info, time_key):
        """Создает промпт с УЛУЧШЕННЫМИ требованиями к концовкам"""
...
ВАЖНЫЕ ИСПРАВЛЕНИЯ ДЛЯ ФОРМАТА:

1. НЕ ИСПОЛЬЗУЙ символы "---" в конце постов! Они выглядят как обрыв текста.
2. Каждый пост должен иметь ЯСНУЮ и ЗАВЕРШЕННУЮ концовку.
...
• 3-6 хештегов # по тематике (ДОБАВЬ В КОНЦЕ ПОСТА)
• Форматирование: используй • для пунктов
• КОНЦОВКА для Telegram: должен быть четкий завершающий абзац с вовлечением и хештегами
• НЕ ИСПОЛЬЗУЙ "---" в Telegram посте!
...
• Без эмодзи
• Форматирование: используй • для пунктов
• КОНЦОВКА для Дзен: должен быть ясный итог и призыв к обсуждению
• В конце Дзен поста обязательно задай вопрос читателям для вовлечения
• НЕ ИСПОЛЬЗУЙ "---" в Дзен посте!
...
• КОНЕЦ ПОСТА ДОЛЖЕН БЫТЬ ЧЕТКИМ И ЯСНЫМ, без обрывов
...
ВАЖНО: ГЕНЕРИРУЙ ПОСТЫ ТОЧНОЙ ДЛИНЫ И С ЧЕТКИМИ КОНЦОВКАМИ!
...
• Каждый пост должен иметь ЗАВЕРШЕННУЮ структуру: начало → развитие → вывод → вопрос
• НЕ ОБРЫВАЙ текст! Делай ясные концовки!
...
Telegram-пост:
[здесь текст для Telegram с эмодзи и хештегами в конце]

Дзен-пост:
[здесь текст для Дзен без эмодзи, с четкой концовкой и вопросом]
...
ОБРАТИ ВНИМАНИЕ: 
• Не используй "---" внутри постов
• Сделай концовки ясными и завершенными
• Telegram: добавь хештеги в конце
• Дзен: добавь вопрос для обсуждения в конце
• Посты не должны выглядеть обрывисто!
...
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС. УЧТИ ВСЕ ТРЕБОВАНИЯ ВЫШЕ."""
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        if "Telegram-пост:" in generated_text and "Дзен-пост:" in generated_text:
                            ...
                            if "---" in generated_text and "---" not in ["Telegram-пост:", "Дзен-пост:", "Поисковый запрос для изображения:"]:
                                logger.warning("⚠️ Обнаружены лишние '---' в тексте")
    ...
    def split_telegram_and_zen_text(self, combined_text):
        ...
        # Убираем поисковый запрос из основного текста
        for marker in query_markers:
        ...
        if tg_start != -1 and zen_start != -1:
            ...
            tg_text = self.clean_post_text(tg_text, is_telegram=True)
            zen_text = self.clean_post_text(zen_text, is_telegram=False)
        ...
        for separator in separators:
            if separator in combined_text:
                ...
                if len(parts) == 2:
                    tg_text = self.clean_post_text(parts[0].strip(), is_telegram=True)
                    zen_text = self.clean_post_text(parts[1].strip(), is_telegram=False)
                    return tg_text, zen_text, image_query
        
        # Если ничего не нашли, ищем по меткам
        if "Telegram:" in combined_text and "Дзен:" in combined_text:
            tg_part = combined_text.split("Дзен:")[0]
            zen_part = combined_text.split("Дзен:")[1]
            tg_text = tg_part.replace("Telegram:", "").strip()
        ...
        if text_length > 500:
            ...
            tg_text = self.clean_post_text(combined_text[:split_point].strip(), is_telegram=True)
            zen_text = self.clean_post_text(combined_text[split_point:].strip(), is_telegram=False)
            return tg_text, zen_text, image_query
        
        tg_text = self.clean_post_text(combined_text, is_telegram=True)
        return tg_text, tg_text, image_query

    def clean_post_text(self, text, is_telegram=True):
        """Очищает текст поста от лишних символов и улучшает концовку"""
        if not text:
            return ""
        
        # Убираем лишние разделители
        text = re.sub(r'-{3,}', '', text)  # Убираем ---
        text = re.sub(r'_{3,}', '', text)  # Убираем ___
        text = re.sub(r'={3,}', '', text)  # Убираем ===
        
        # Убираем обрывки в конце
        endings_to_remove = [
            r'\s*\.\.\.\s*$',  # Троеточие в конце
            r'\s*---\s*$',     # Дефисы в конце
            r'\s*–\s*$',       # Длинное тире в конце
            r'\s*-\s*$',       # Короткое тире в конце
        ]
        
        for pattern in endings_to_remove:
            text = re.sub(pattern, '', text)
        
        # Проверяем и улучшаем концовку
        text = self.improve_post_ending(text, is_telegram)
        
        return text.strip()

    def improve_post_ending(self, text, is_telegram=True):
        """Улучшает концовку поста"""
        # Разбиваем на строки
        lines = text.split('\n')
        
        # Если последняя строка пустая или очень короткая, ищем последнюю содержательную строку
        last_content_line = None
        for i in range(len(lines) - 1, -1, -1):
            line = lines[i].strip()
            if line and len(line) > 10:  # Считаем строку содержательной если >10 символов
                last_content_line = i
                break
        ...
        if last_content_line is not None:
            ...
            # Проверяем, есть ли хорошая концовка
            ...
            # Если нет хорошей концовки, добавляем
            if not has_good_ending:
                if is_telegram:
                    # Для Telegram
                    endings = [
                        "А что думаете вы? Пишите в комментариях! 💬",
                        "Как вы считаете, это работает? Обсудим! 👇",
                        "А у вас был похожий опыт? Расскажите! ✨",
                        "Сталкивались с таким? Поделитесь в комментариях! 🔥",
                        "Какой подход ближе вам? Давайте обсудим! 💭"
                    ]
                else:
                    # Для Дзен
                    endings = [
                        "Что думаете по этому поводу? Ваше мнение важно для обсуждения.",
                        "А как вы решаете подобные проблемы? Поделитесь опытом в комментариях.",
                        "Сталкивались ли вы с такой ситуацией? Расскажите, как выходили из нее.",
                        "Какой подход кажется вам более эффективным? Давайте обсудим в комментариях.",
                        "А в вашей практике было нечто подобное? Поделитесь в комментариях."
                    ]
    ...
    def generate_image_search_query(self, text, theme):
        ...
        try:
            ...
            theme_keywords = {
                "HR и управление персоналом": ["офис", "команда", "встреча", "переговоры", "бизнес", "управление", "персонал"],
                "PR и коммуникации": ["коммуникация", "медиа", "презентация", "аудитория", "брендинг", "маркетинг", "общение"],
                "ремонт и строительство": ["ремонт", "строительство", "инструменты", "дизайн", "интерьер", "дом", "квартира"]
            }
            
            keywords = theme_keywords.get(theme, ["бизнес", "профессия", "развитие", "успех"])
            ...
            contexts = ["профессиональный", "деловой", "рабочий", "современный", "успешный", "качественный"]
    ...
    def get_fresh_image(self, text, theme, width=1200, height=630):
        ...
        try:
            ...
            pexels_api_key = "563492ad6f91700001000001d15a5e2d6a9d4b5c8c0e6f5b8c1a9b7c"  # Примерный ключ
    ...
    def ensure_closing_hook(self, text, is_telegram=True):
        ...
        # Убираем возможные обрывки в конце
    ...
    def add_telegram_hashtags(self, text, theme):
        ...
        theme_hashtags = {
            "HR и управление персоналом": ["#HR", "#управление", "#персонал", "#карьера", "#работа", "#бизнес", "#команда"],
            "PR и коммуникации": ["#PR", "#коммуникации", "#маркетинг", "#медиа", "#бренд", "#продвижение", "#общение"],
            "ремонт и строительство": ["#ремонт", "#стройка", "#дизайн", "#интерьер", "#дом", "#квартира", "#строительство"]
        }
        
        # Выбираем хештеги по теме
        base_hashtags = theme_hashtags.get(theme, ["#контент", "#эксперт", "#советы", "#развитие"])
        
        # Добавляем случайные общие хештеги
        general_hashtags = ["#инсайты", "#лайфхак", "#профессия", "#развитие", "#успех", "#тренды", "#советы"]
    ...
    def generate_and_send_posts(self):
        ...
        try:
            ...
            # Проверяем концовки
            if not self.has_good_ending(tg_text, is_telegram=True):
                logger.warning("⚠️ Telegram пост может иметь слабую концовку")
            
            if not self.has_good_ending(zen_text, is_telegram=False):
                logger.warning("⚠️ Дзен пост может иметь слабую концовку")
            ...
            if success_count == 2:
                ...
                logger.info("\n" + "=" * 60)
                logger.info("🎉 УСПЕХ! Посты отправлены с улучшенными концовками!")
                logger.info("=" * 60)
    ...
    def has_good_ending(self, text, is_telegram=True):
        """Проверяет, есть ли хорошая концовка у текста"""
        if not text:
            return False
        
        # Берем последние 200 символов текста
        last_part = text[-200:].lower()
        
        # Паттерны хороших концовок
        patterns = [
            r'как вы считаете[^?.!]*[?.!]',
            r'что думаете[^?.!]*[?.!]',
            r'ваше мнение[^?.!]*[?.!]',
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🤖 GITHUB TELEGRAM HR BOT - УЛУЧШЕННЫЕ КОНЦОВКИ")
    print("=" * 80)
    print("📋 Исправления:")
    print("   • Убраны лишние '---' в постах")
    print("   • Добавлены четкие завершающие абзацы")
    print("   • Улучшены призывы к действию")
    print("   • Telegram: хештеги в конце, вовлекающие вопросы")
    print("   • Дзен: ясные итоги и вопросы для обсуждения")
    print("=" * 80)
    ...
    if success:
        ...
        print("   Посты с улучшенными концовками")
        print("=" * 50)
        sys.exit(0)
    else:
        print("\n" + "=" * 50)
