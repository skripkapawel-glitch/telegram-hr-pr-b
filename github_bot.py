...
Смысловой якорь:
(один отдельный абзац, фиксирующий значение или последствия без морали и клише)
...
3. Смысловой якорь должен быть отдельным абзацем после строки «Смысловой якорь:»
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                # Ищем признаки смыслового якоря (расширенный список маркеров)
                anchor_markers = ['Смысловой якорь:', 'Смысловой якорь', 'Значит', 'Поэтому', 'Таким образом', 'Следовательно', 
                                 'В итоге', 'Итак', 'Важно понимать', 'Ключевой момент', 'Суть в том',
                                 'Это означает', 'Последствия', 'Итог', 'Вывод', 'Что это значит',
                                 'Фиксируем', 'Закрепляем', 'Таким образом', 'Следовательно']
                has_anchor = any(marker in generated_zen for marker in anchor_markers)
                
                # ОСЛАБЛЕННАЯ ПРОВЕРКА: если нет явного маркера, но есть достаточно содержательный текст
                if not has_anchor and zen_length > 500:
                    # Проверяем последние 2 абзаца перед хештегами
                    ...
                    if hashtag_index > 1:
                        # Берем 2 абзаца перед хештегами
                        anchor_paragraphs = paragraphs[max(0, hashtag_index-2):hashtag_index]
                        anchor_text = ' '.join(anchor_paragraphs)
                        # Если суммарно больше 80 символов - считаем, что есть смысловой якорь
