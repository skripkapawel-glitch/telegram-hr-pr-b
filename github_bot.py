...
class TelegramBot:
    ...
    def generate_with_gemini(self, prompt: str) -> Optional[str]:
        ...
        try:
            # Определяем токены в зависимости от типа поста
            if hasattr(self, 'current_post_type'):
                if self.current_post_type == 'telegram':
                    max_tokens = self.current_style.get('tg_tokens', 400) if self.current_style else 400
                else:
                    max_tokens = self.current_style.get('zen_tokens', 400) if self.current_style else 400
            else:
                max_tokens = 400
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in result and result['candidates']:
                    ...
                    logger.info(f"✅ Текст получен, длина: {len(generated_text)} символов, токены: {max_tokens}")
    ...
    def create_detailed_prompt(self, theme: str, slot_style: Dict, text_format: str, image_description: str) -> str:
        ...
        # Полный промпт с информацией об авторе (как вы предоставили)
        prompt = f"""
Тебе нужно сгенерировать пост для каждой соц сети строго по колличеству символов указанному тут:

Утренний слот (11:00)
Telegram: 400–600 символов (максимум {slot_style['tg_tokens']} токенов)
Дзен: 600–700 символов (максимум {slot_style['zen_tokens']} токенов)

Дневной слот (15:00)
Telegram: 700–900 символов (максимум {slot_style['tg_tokens']} токенов)
Дзен: 700–900 символов (максимум {slot_style['zen_tokens']} токенов)

Вечерний слот (20:00)
Telegram: 600–900 символов (максимум {slot_style['tg_tokens']} токенов)
Дзен: 700–800 символов (максимум {slot_style['zen_tokens']} токенов)

Ты обязан строго соблюдать ограничения по количеству символов и токенов.
Если итоговый текст выходит за пределы диапазона — это считается ошибкой.
Приоритет №1 — длина текста, приоритет №2 — смысл и структура.
...
Telegram пост ДОЛЖЕН быть строго {tg_min}-{tg_max} символов (максимум {slot_style['tg_tokens']} токенов).
Дзен пост ДОЛЖЕН быть строго {zen_min}-{zen_max} символов (максимум {slot_style['zen_tokens']} токенов).
...
• Объём: {tg_min}-{tg_max} символов (максимум {slot_style['tg_tokens']} токенов) (ОБЯЗАТЕЛЬНО!)

ТРЕБОВАНИЯ К ZEN ПОСТУ:
• Начало: провокационный вопрос или утверждение ("крючок-убийца")
• Основная часть: глубина анализа, экспертные мнеты
• Завершение: естественный вывод (можно использовать: 'Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:')
• Вопрос для обсуждения
• 3-5 релевантных хештегов в конце
• Объём: {zen_min}-{zen_max} символов (максимум {slot_style['zen_tokens']} токенов) (ОБЯЗАТЕЛЬНО!)
...
• Соблюдение лимитов символов и токенов
        ...
        if tg_message_id or zen_message_id:
            try:
                instruction = (f"<b>✅ ПОСТЫ ОТПРАВЛЕНЫ НА МОДЕРАЦИЮ</b>\n\n"
                              ...
                              f"   Символов: {len(tg_text)} (макс {self.current_style['tg_tokens']} токенов)\n\n"
                              f"<b>📝 Дзен пост</b>\n"
                              f"   Канал: {ZEN_CHANNEL}\n"
                              f"   Время: {slot_time} МСК\n"
                              f"   Символов: {len(zen_text)} (макс {self.current_style['zen_tokens']} токенов)\n\n"
...
Дзен: 600–700 символов

Дневной слот (15:00)
Telegram: 700–900 символов
Дзен: 700–900 символов

Вечерний слот (20:00)
Telegram: 600–900 символов
Дзен: 700–800 символов

Ты обязан строго соблюдать ограничения по количеству символов.
Если итоговый текст выходит за пределы диапазона — это считается ошибкой.
Приоритет №1 — длина текста, приоритет №2 — смысл и структура.
