...
🎯 ВАЖНО: ОБЯЗАТЕЛЬНО добавь СМЫСЛОВОЙ ЯКОРЬ! Это отдельный абзац, который фиксирует значение, последствия или итог темы без морали и клише. Начинай этот абзац с маркера "🎯 Важно:" или "🎯 Важно". Это критический элемент поста.
...
Правила:
1. ЗАПРЕЩЕНО использовать эмодзи (кроме маркера 🎯 для якоря)
2. Вопрос должен быть отдельной строкой и заканчиваться знаком ?
3. ОБЯЗАТЕЛЬНО добавь смысловой якорь с маркером 🎯 Важно: как отдельный абзац
    ...
    def generate_with_retry(self, theme: str, slot_style: Dict, text_format: str, image_description: str,
        ...
        for attempt in range(max_attempts):
            ...
            if generated_zen:
                ...
                # Ищем признаки смыслового якоря с маркером 🎯 Важно:
                anchor_markers = [
                    # Основные маркеры с эмодзи
                    '🎯 Важно:', '🎯 Важно', '🎯:', 
                    # Русские формулировки
                    'Смысловой якорь', 'Значит', 'Поэтому', 'Таким образом', 'Следовательно', 
                ...
                # УЛУЧШЕННАЯ ПРОВЕРКА: если нет явного маркера, ищем смысловой якорь по содержанию
