...
class TelegramBot:
    ...
    def _clean_metadata(self, text: str, post_type: str) -> str:
        ...
        for line in text.split('\n'):
            ...
            # Удаляем нумерацию типа [1], [2], [3], [4] и т.д. - ИСПРАВЛЕНО
...
[4] ВОПРОС: Задай ПОЛНЫЙ, ЗАКОНЧЕННЫЙ вопрос читателю. Отдельная строка. Заканчивается знаком ?. Вопрос должен отличаться от заголовка и быть полностью сформулированным. Вопрос должен быть законченным предложением с четкой формулировкой.
...
4. Вопрос заканчивается знаком ? и должен быть ПОЛНЫМ И ЗАКОНЧЕННЫМ
...
9. ВОПРОС В БЛОКЕ [4] ДОЛЖЕН БЫТЬ ПОЛНОЦЕННЫМ, ЗАКОНЧЕННЫМ ПРЕДЛОЖЕНИЕМ С ЗНАКОМ ? В КОНЦЕ
10. УБЕДИСЬ, ЧТО ВСЕ БЛОКИ ПОЛНЫЕ И ЗАКОНЧЕННЫЕ
11. ВОПРОС НЕ ДОЛЖЕН ОБРЫВАТЬСЯ - ДОВЕДИ МЫСЛЬ ДО КОНЦА
12. ДЛЯ ТЕЛЕГРАМ ПОСТА ВОПРОС ДОЛЖЕН БЫТЬ ПОЛНОСТЬЮ СФОРМУЛИРОВАННЫМ И ЗАВЕРШЕННЫМ
"""
        return prompt.strip()
...
[2] АБЗАЦ 1: Разверни тему. 1-3 предложения. Начни с разных подходы: статистика, пример, вопрос к читателю.

[3] КЛЮЧЕВАЯ МЫСЛЬ: одна явная и конкретная мысль по теме. 1-2 предложения. Сформулируй уникально.

[4] ВОПРОС: Задай новый вопрос для обсуждения, отличный от заголовка. Отдельная строка. Заканчивается знаком ?.
...
3. Вопрос в блоке [4] заканчивается знаком ? и отличается от вопроса в заголовке
4. Хештеги в последней строке
5. НЕ ПРОПУСКАЙ ни один блок
6. Следуй строго порядку [1]-[5]
...
9. ОБЯЗАТЕЛЬНО соблюдай лимит в {zen_max_chars} символов - не превышай его ни на один символ
"""
        return prompt.strip()
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        ...
        # Telegram проверка - ИСПРАВЛЕНА ЛОГИКА ДОБАВЛЕНИЯ ВОПРОСА
        if post_type == 'telegram':
            ...
            # 3. ПРОВЕРКА ВОПРОСА - УСИЛЕННАЯ ЛОГИКА
            # Ищем строки с вопросами
            question_lines = [i for i, line in enumerate(lines) if '?' in line]
            
            if not question_lines:
                # Если вопросов нет вообще
                logger.warning("⚠️ Telegram пост не содержит вопроса! Добавляю вопрос...")
                
                if self.current_theme:
                    theme_questions = {
                        "HR и управление персоналом": "Как вы считаете, стоит ли компаниям инвестировать в культуру заботы, чтобы повысить продуктивность сотрудников?",
                        "PR и коммуникации": "Как вы оцениваете эффективность PR-стратегий, основанных на открытости и честности в эпоху информационного шума?",
                    ...
                    default_question = theme_questions.get(self.current_theme, "Что думаете по этому поводу?")
                else:
                    default_question = "Что думаете по этому поводу?"
                
                # Ищем место для вставки вопроса (перед хештегами)
                hashtag_found = False
                for i, line in enumerate(lines):
                    if line.startswith('#'):
                ...
                # Проверяем, что хотя бы один вопрос полный (заканчивается на ?)
                complete_questions = [i for i in question_lines if lines[i].endswith('?')]
                
                if not complete_questions:
                    # Все вопросы обрезаны - исправляем первый найденный вопрос
                    first_q_index = question_lines[0]
                    incomplete_question = lines[first_q_index]
                    
                    # Проверяем, на что обрывается вопрос
                    if self.current_theme:
                        theme_completions = {
                            "HR и управление персоналом": {
                                "влияние инвестиций": "влияние инвестиций в развитие персонала на общую продуктивность компании?",
                                "культуру заботы": "культуру заботы в организации на уровень вовлеченности сотрудников?",
                                "методы оценки": "методы оценки эффективности HR-стратегий в вашей компании?"
                            },
                            "PR и коммуникации": {
                                "эффективность PR-стратегий": "эффективность PR-стратегий, основанных на прозрачности и честности?",
                                "управление репутацией": "управление репутацией в условиях кризиса доверия к брендам?",
                                "влияние социальных": "влияние социальных медиа на формирование общественного мнения?"
                            },
                            "ремонт и строительство": {
                                "стоимость качественных": "стоимость качественных материалов на долговечность ремонта?",
                                "влияние профессионального": "влияние профессионального подхода на результат строительных работ?",
                                "экономию на материалах": "экономию на материалах на частоту необходимых ремонтов?"
                            }
                        }
                        
                        # Ищем ключевые слова в обрезанном вопросе
                        completion_found = False
                        for key, completion in theme_completions.get(self.current_theme, {}).items():
                            if key in incomplete_question.lower():
                                lines[first_q_index] = incomplete_question + " " + completion
                                completion_found = True
                                break
                        
                        if not completion_found:
                            # Если не нашли конкретное завершение, добавляем общее
                            lines[first_q_index] = incomplete_question + " на результаты деятельности?"
                    else:
                        # Общее завершение
                        lines[first_q_index] = incomplete_question + " в вашей практике?"
                    
                    text = '\n'.join(lines)
                    logger.info("✅ Исправлен обрезанный вопрос в Telegram посте")
            
            # 4. Проверяем, что все блоки полные
            # Проверяем заголовок
            if lines and slot_style and 'emoji' in slot_style:
                if not lines[0].startswith(slot_style['emoji']):
                    lines[0] = f"{slot_style['emoji']} {lines[0]}"
            
            # Проверяем ключевую мысль
            target_found = any('🎯' in line for line in lines)
            if not target_found:
