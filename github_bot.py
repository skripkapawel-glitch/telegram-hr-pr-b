...
• Практический блок: конкретные действия или советы (НЕ используй шаблон "Практический совет:")
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше. Лаконично, без воды, строго в пределах лимита."""
    ...
    def format_post_text(self, text, slot_style, post_type):
        """Форматирует текст поста БЕЗ ШАБЛОНОВ"""
        import re
        ...
        # ТИПОСПЕЦИФИЧНЫЕ ПРАВИЛА (минимальные)
        if post_type == 'telegram':
            lines = text.split('\n')
...
• Сделать завершение УНИКАЛЬНЫМ для этого поста
