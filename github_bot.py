...
1. УТРЕННИЙ СЛОТ (09:00) — только утренние приветствия: "Доброе утро", "Начало дня", "Старт утра"
2. ДНЕВНОЙ СЛОТ (14:00) — ЗАПРЕЩЕНО: "Доброе утро", "Добрый вечер". Только нейтральный деловой тон
3. ВЕЧЕРНИЙ СЛОТ (19:00) — ЗАПРЕЩЕНО: "Доброе утро". Можно: "Добрый вечер", "В завершение дня", "Подводя итоги"

Пост должен начинаться СТРОГО с фразы, соответствующей временному слоту.
...
Автор — синтез экспертизы высшего уровня с совокупным опытом 30+ лет:
• Практик с опытом в HR, PR и строительстве — разбирается в темах изнутри
• Аналитик и стратег — видит системные связи и тренды
• Редактор смыслов — упаковывает сложное в доступное
• Бренд-стратег — понимает аудиторию и рынок
...
Переработай текст, сохраняя смысл, но учитывая запрос на редактирование."""
    ...
    def create_detailed_prompt(self, theme, slot_style, text_format, image_description):
        ...
        try:
            ...
            # Актуальные тренды по темам
            trends_by_theme = {
                "HR и управление персоналом": [
                    "HR-аналитика и data-driven решения в 2025",
                    "Персонализация employee experience",
                    "Геймификация в обучении и мотивации 2026",
                    "Ментальное здоровье как KPI",
                    "AI в рекрутинге и оценке персонала",
                    "Скиллы будущего для HR-специалистов",
                    "Корпоративная культура в распределённых командах",
                    "Управление поколением Z в офисах",
                    "Этика AI в кадровых процессах",
                    "Персональные траектории развития сотрудников",
                    "Метрики вовлечённости в реальном времени",
                    "Бренд работодателя в эпоху соцсетей 2025",
                    "Управление изменениями в VUCA-мире",
                    "Диверсификация и инклюзивность на практике",
                    "Эко-friendly HR-политики 2026",
                    "Нейронауки в управлении командой",
                    "Сокращение выгорания через дизайн рабочих процессов",
                    "Цифровой двойник сотрудника",
                    "Эмоциональный интеллект как must-have навык",
                    "HR-автоматизация без потери человечности",
                    "Сторителлинг во внутренних коммуникациях",
                    "Кросс-функциональные команды как норма",
                    "Управление талантами в условиях дефицита кадров",
                    "Этика сбора данных о сотрудниках",
                    "Менторинг и reverse mentoring",
                    "Создание learning organization",
                    "Управление performance в неопределённости",
                    "HR-метрики, влияющие на бизнес",
                    "Адаптация мигрантов на рынке труда 2025",
                    "Новые законы о труде 2025-2026"
                ],
                "PR и коммуникации": [
                    "AI-генерация контента и этика в 2025",
                    "Персонализация коммуникаций через big data",
                    "Иммерсивный сторителлинг (AR/VR в PR)",
                    "Микро-инфлюенсеры как тренд 2026",
                    "Кризисные коммуникации в эпоху cancel culture",
                    "Устойчивое развитие как часть бренда",
                    "Голосовые интерфейсы в коммуникациях",
                    "Интерактивные пресс-релизы 2025",
                    "Нейромаркетинг в PR-кампаниях",
                    "Локальные нарративы в глобальном мире",
                    "Прозрачность как конкурентное преимущество",
                    "Метавселенные как площадка для PR",
                    "Data-журналистика в корпоративных коммуникациях",
                    "Эмоциональная аналитика в соцсетях",
                    "Геймификация PR-активностей",
                    "Борьба с дезинформацией и deepfakes 2026",
                    "Партизанский маркетинг 2.0",
                    "Комьюнити-менеджмент как сервис",
                    "Персональный брендинг для CEO",
                    "Экосистемные коммуникации 2025",
                    "Репутационный менеджмент в реальном времени",
                    "Сторителлинг через данные",
                    "Инклюзивный язык в коммуникациях",
                    "Визуальный нарратив (TikTok, Reels, Shorts)",
                    "Партнёрства с purpose-driven брендами",
                    "Адаптация контента под разные поколения",
                ...
                "ремонт и строительство": [
                    ...
                    "Роботизация строительных процессов 2026",
                    "Эко-материалы нового поколения",
                    "Адаптивная архитектура",
                    "Вертикальное озеленение и фитостены",
                    "Реновация с сохранением исторического облика",
                    "Сейсмоустойчивые технологии",
                    "3D-печать в строительстве",
                    "Умные системы управления зданием",
                    "Бесшовные технологии отделки",
                    "Аккустический дизайн пространств",
                    "Биофильный дизайн интерьеров 2025",
                    "Цифровизация управления проектами",
                    "Локальные строительные материалы",
                    "Адаптация под изменение климата",
                    "Инклюзивная среда в строительстве",
                    "Микроклимат помещений как KPI",
                    "Ресайклинг в строительных материалах",
                    "Смарт-стекло и адаптивные фасады",
                    "Водосберегающие технологии 2026",
                    "Электрификация и зарядная инфраструктура",
                    "Сенсорные и тактильные поверхности",
                    "Минимализм и функциональность",
                    "Тренды в отделочных материалах 2025-2026",
                    "Управление строительными отходами",
                    "Цифровые платформы для подрядчиков",
                    "Новые строительные нормы и правила 2025"
                ]
            }
            
            trends = trends_by_theme.get(theme, [])
            selected_trends = random.sample(trends, min(5, len(trends)))
...
🎭 ИНФОРМАЦИЯ ОБ АВТОРЕ
Автор — синтез экспертизы высшего уровня с совокупным опытом 30+ лет:
• Практик с реальным опытом в HR, PR и строительстве — разбирается в темах изнутри
...
2. Синтезирует лучшие практики и кейсы
3. Даёт практические рекомендации, которые можно применить сразу
4. Прогнозирует тренды на 2025-2026 годы
5. Объясняет сложное простым языком, но без упрощения

🚀 АКТУАЛЬНОСТЬ И ТРЕНДЫ 2025-2026
Пост должен быть:
• Информационно насыщенным — факты, цифры, кейсы, исследования
• Практически полезным — готовые решения, шаблоны, чек-листы, алгоритмы
• Аналитическим — не просто информация, а анализ, выводы, инсайты
• Вовлекающим — чтобы хотелось читать дальше, комментировать, подписываться
• Актуальным — отсылки к современным реалиям, новым законам, трендам, технологиям

📈 КЛЮЧЕВЫЕ ТРЕНДЫ ПО ТЕМЕ НА 2025-2026:
{trends_text}

🎯 КРИТЕРИИ КАЧЕСТВА КОНТЕНТА:
1. Профессиональная глубина — автор действительно разбирается в теме
2. Практическая ценность — можно применить здесь и сейчас
3. Уникальность угла — неочевидные связи, инсайты, свежий взгляд
4. Читабельность — легко читается, хорошо структурировано, логично
5. Эмоциональный резонанс — вызывает отклик, желание обсудить, поделиться
6. Экспертность — чувствуется 30+ лет опыта автора в смежных областях
...
• Начало с {slot_style['emoji']} и хука (1-2 предложения, цепляющих внимание)
• Основная часть: анализ явления, кейсы, данные, исследования
...
• Заголовок БЕЗ эмодзи, но цепляющий, провокационный или интригующий
• Введение: почему эта тема важна именно сейчас (2025-2026)
• Аналитическая часть: глубокий разбор с примерами, цифрами, кейсами
• Практические рекомендации: конкретные шаги, инструкции, советы
• Кейсы или примеры из реальной практики (можно вымышленные, но правдоподобные)
• Что будет дальше (тренды, перспективы, прогнозы)
• Мягкий финал: "{soft_final}"
• Хештеги: {hashtags_str}
...
Автор — эксперт с 30+ лет совокупного опыта. Текст должен это отражать:
• Глубина анализа, а не поверхностные советы
• Синтез информации из разных областей (HR + психология + менеджмент и т.д.)
• Практическая применимость в реальных российских условиях
• Акцент на современных вызовах и актуальных решениях
• Баланс между профессиональной терминологией и доступностью изложения

При упоминании опыта или кейсов использовать формулировки:
• «Анализ практики ведущих специалистов показывает...»
• «Современные исследования в области {theme} указывают на...»
• «По данным отраслевых отчётов за 2025 год...»
• «Эксперты с опытом внедрения отмечают следующие тенденции...»
• «В профессиональной среде сложился consensus относительно...»
...
• Офисную работу
• Работу в офисе
• Новые законы и нормативные акты 2025-2026
...
4. Telegram пост начинается с {slot_style['emoji']} и СТРОГО соответствует временному слоту
5. Дзен пост — БЕЗ ЭМОДЗИ ВООБЩЕ, но с профессиональным заголовком
6. Хештеги ТОЛЬКО В КОНЦЕ каждого поста
7. Мягкий финал — вопрос к аудитории, приглашение к дискуссии
8. Сохранять ограничения по символам
9. Оба поста должны быть РАЗНЫМИ по структуре, но об одном смысле
...
Текст должен быть:
- Информационно насыщенным (факты, цифры, кейсы)
- Практически полезным (что делать, как применять) 
- Аналитически глубоким (почему, зачем, какие выводы)
- Соответствующим трендам 2025-2026 (актуально, современно)
- Вовлекающим и профессиональным (экспертный уровень)
- Соответствующим временному слоту (утро/день/вечер)

Создай два РАЗНЫХ текста по одной теме. Оба должны быть уникальными по структуре, но об одном смысле."""
