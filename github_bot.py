...
class TelegramBot:
    ...
    def create_prompt(self, theme, slot_info, text_format):
        ...
        format_instructions = {
            ...
            "анализ поведения аудитории": "Проанализируй поведение людей (сотрудников, клиентов, аудитории) в контексте теме. Почему они так поступают?",
...
ХУК (ПЕРВАЯ СТРОКА) — ДОЛЖЕН БЫТЬ МОЩНЫЙ:
...
• Примеры сильных хуков (НЕ КОПИРОВАТЬ, только стиль):
  ...
  "Есть одна ошибка, которая стоит компаниям миллионы, и её делают почти все..."
...
1. Хук должен быть ИНТРИГУЮЩИЙ, МОЩНЫЙ, ЦЕПЛЯЮЩИЙ
...
НИКАКИХ КОММЕНТАРИЕВ, НИКАКИХ ОПИСАНИЙ, ТОЛЬКО ДВА ТЕКСТА!"""
    ...
    def generate_with_gemini(self, prompt):
        ...
        try:
            # Используем доступные модели из твоего списка
            available_models = [
                "gemini-2.5-flash-preview-04-17",
                "gemini-2.5-pro-exp-03-25", 
                "gemma-3-27b-it",
                "gemini-1.5-flash-002",  # Попробуем с версией
                "gemini-1.5-pro-002"     # Попробуем с версией
            ]
    ...
    def validate_and_fix_structure(self, text, is_telegram=True):
        ...
        for line in lines:
            if line.strip().startswith('#') and not line.strip().endswith('#'):
                # Это хэштег в середине текста - пропускаем
                continue
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            if success_count >= 1:
                ...
                # Исправленная строка без синтаксических ошибок
                lines_for_hook = tg_text.split('\n')
                first_line_for_hook = lines_for_hook[0] if lines_for_hook else ""
                has_hook = '?' in first_line_for_hook or '!' in first_line_for_hook
