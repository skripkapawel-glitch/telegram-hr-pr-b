...
{hashtags_str}"""
    ...
    def _add_telegram_practice_block(self, text, theme):
        ...
        practice_blocks = {
            "ремонт и строительство": [
                ...
                "📊 Конкретные действия: Перед началом ремонта составьте смету и прибавьте к ней 20% на непредвиденные расходы.",
                "🚀 Что делать дальше: Найдите надежного подрядчика с отзывами и портфолио реальных объектов."
            ],
            "HR и управление персоналом": [
                "🎯 Практический совет: Регулярно проводите one-on-one встречи с сотрудниками. Это лучший способ узнать реальные проблемы.",
                "📊 Конкретные действия: Внедрите систему обратной связи 360 градусов для объективной оценки.",
                "🚀 Что делать дальше: Создайте программу адаптации для новых сотрудников - это снизит текучку на 30%."
            ],
            "PR и коммуникации": [
                "🎯 Практический совет: Всегда имейте готовый кризисный план коммуникаций.",
                "📊 Конкретные действия: Мониторьте упоминания бренда в соцсетях ежедневно.",
                "🚀 Что делать дальше: Создайте базу часто задаваемых вопросов и ответов для сотрудников."
            ]
        }
        
        blocks = practice_blocks.get(theme, ["🎯 Практический совет: Начните с малого и масштабируйте успешные практики."])
        
        if random.random() < 0.7:  # 70% вероятность добавить
    ...
    def _add_telegram_insight(self, text, theme):
        """Добавляет инсайт/ключевую мысль в Telegram пост"""
        insights = {
            "ремонт и строительство": [
                "💡 Главный инсайт: Качество материалов определяет 80% успеха ремонта.",
                "✨ Ключевая мысль: Хороший ремонт - это не дорого, а грамотно спланировано.",
                "🎯 Самое важное: Функциональность важнее моды в дизайне интерьера."
            ],
            "HR и управление персоналом": [
                "💡 Главный инсайт: Лояльность сотрудника начинается с первого рабочего дня.",
                "✨ Ключевая мысль: Деньги мотивируют только до определенного предела.",
                "🎯 Самое важное: Корпоративная культура - это не пиар, а ежедневная практика."
            ],
            "PR и коммуникации": [
                "💡 Главный инсайт: Репутация строится годами, а разрушается за минуту.",
                "✨ Ключевая мысль: Лучшая реклама - это довольный клиент.",
                "🎯 Самое важное: Искренность в коммуникациях ценится выше идеальных формулировок."
            ]
        }
        
        insight_list = insights.get(theme, ["💡 Главный инсайт: Детали решают всё."])
        
        if random.random() < 0.6:  # 60% вероятность добавить
            insight = random.choice(insight_list)
            
            lines = text.split('\n')
            # Ищем место перед мягким финалом или хештегами
            soft_final_pos = -1
            for i, line in enumerate(lines):
                if any(q in line for q in ['А как вы считаете?', 'Было ли у вас так?', 'Что думаете?']):
                    soft_final_pos = i
                    break
            
            if soft_final_pos > 0:
                lines.insert(soft_final_pos, '')
                lines.insert(soft_final_pos, insight)
            else:
                for i, line in enumerate(lines):
                    if '#' in line:
                        lines.insert(i, '')
                        lines.insert(i, insight)
                        break
                else:
                    lines.append('')
    ...
    def _add_zen_case_study(self, text, theme):
        """Добавляет кейс из практики в Zen пост"""
        case_studies = {
            "ремонт и строительство": [
                "📊 Кейс из практики: При ремонте квартиры в новостройке, применение BIM-моделирования позволило выявить и устранить 15 коллизий еще до начала работ, что сэкономило 3 недели времени и 200 тыс. рублей бюджета.",
                "🏢 Пример с объекта: На строительстве бизнес-центра внедрение системы цифрового контроля качества снизило количество дефектов на 40% по сравнению с аналогичными проектами.",
                "📈 Реальный опыт: Девелоперская компания, использовавшая префабрицированные конструкции, сократила сроки строительства жилого комплекса на 25% при сохранении качества."
            ],
            "HR и управление персоналом": [
                "📊 Кейс из практики: Внедрение системы геймификации в обучении продавцов увеличило completion rate курсов с 45% до 85% за 3 месяца.",
                "🏢 Пример компании: Организация, внедрившая программу mental health support, снизила количество больничных по причине стресса на 35% за полгода.",
                "📈 Реальный опыт: IT-компания, перешедшая на гибкую систему оценки эффективности (OKRs вместо KPI), увидела рост удовлетворенности сотрудников на 28%."
            ],
            "PR и коммуникации": [
                "📊 Кейс из практики: Бренд косметики, использовавший UGC-контент в соцсетях, увеличил вовлеченность аудитории на 300% за квартал.",
                "🏢 Пример компании: Производитель продуктов питания, прозрачно рассказавший о проблемах в цепочке поставок во время кризиса, не потерял, а увеличил доверие клиентов на 25%.",
                "📈 Реальный опыт: Технологический стартап, внедривший AI-анализ тональности упоминаний, смог предотвратить 3 потенциальных кризиса репутации."
            ]
        }
        
        cases = case_studies.get(theme, ["Кейс из практики показывает эффективность этого подхода."])
        
        if random.random() < 0.5:  # 50% вероятность добавить
            case = random.choice(cases)
            
            lines = text.split('\n')
            # Ищем место перед блоком завершения
            conclusion_pos = -1
            for i, line in enumerate(lines):
                if any(marker in line for marker in ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']):
                    conclusion_pos = i
                    break
            
            if conclusion_pos > 0:
                lines.insert(conclusion_pos, '')
                lines.insert(conclusion_pos, case)
            else:
                # Ищем место перед хештегами
                for i, line in enumerate(lines):
                    if '#' in line:
                        lines.insert(i, '')
                        lines.insert(i, case)
    ...
    def _add_telegram_statistics(self, text, theme):
        """Добавляет статистику в Telegram пост"""
        return self.add_statistical_data(text, theme)

    def _add_zen_expert_opinion(self, text, theme):
        """Добавляет экспертное мнение в Zen пост"""
        return self.add_expert_quote(text, theme)

    def _add_zen_statistics(self, text, theme):
        """Добавляет статистику в Zen пост"""
        return self.add_statistical_data(text, theme)
    ...
    def _ensure_zen_hook(self, zen_text, theme):
        ...
        try:
            ...
            # Критерии крючка-убийцы:
            is_valid_hook = (
                len(first_line) >= 40 and 
                len(first_line) <= 250 and
                ('?' in first_line or '!' in first_line or '...' in first_line) and
                not any(marker in first_line for marker in ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']) and
                not any(e in first_line for e in ['🌅', '🌞', '🌙', '🎯', '📊', '🚀', '💡', '✨'])
            )
            
            if not is_valid_hook:
                logger.warning("⚠️ В Zen посте нет валидного крючка-убийцы, создаю...")
                
                # Готовые крючки по темам
                hooks_by_theme = {
                    "ремонт и строительство": [
                        "Задумывались ли вы, почему одни квартиры становятся домом, а другие остаются просто жилплощадью?",
                        "Что, если ваш ремонт может повысить стоимость квартиры на 25% уже через год?",
                        "Почему 70% ремонтов превышают первоначальный бюджет в 2-3 раза?",
                        "Знаете ли вы, что неправильный дизайн интерьера может повысить уровень стресса на 40%?",
                        "Как превратить ремонт из кошмара в осознанный и приятный процесс?",
                        "Что общего у успешного ремонта и хорошего инвестиционного портфеля?"
                    ],
                    "HR и управление персоналом": [
                        "Почему лучшие сотрудники уходят даже при высокой зарплате и отличных условиях?",
                        "Что, если одна HR-ошибка может стоить компании миллионы рублей?",
                        "Задумывались ли вы, что настоящая мотивация — это не про деньги?",
                        "Почему корпоративная культура сегодня важнее бизнес-стратегии?",
                        "Как превратить текучку кадров из проблемы в источник роста?",
                        "Что на самом деле хотят от работы сотрудники поколения Z?"
                    ],
                    "PR и коммуникации": [
                        "Что, если один неосторожный пост в соцсетях может уничтожить репутацию бренда за 24 часа?",
                        "Почему 90% рекламных бюджетов тратятся впустую?",
                        "Задумывались ли вы, что ваш бренд говорит о вас, когда вы молчите?",
                        "Что важнее в кризис: говорить правду или говорить красиво?",
                        "Как превратить негативные отзывы в инструмент роста лояльности?",
                        "Почему искренность в коммуникациях стала новым золотым стандартом?"
                    ]
                }
                
                hooks = hooks_by_theme.get(theme, [
                    "Задумывались ли вы, что ключ к успеху лежит в мелочах, которые все игнорируют?",
                    "Что, если самый простой способ улучшить результат — перестать делать очевидные ошибки?",
                    "Почему эксперты годами повторяют одно и то же, но никто их не слушает?",
                    "Как превратить проблему в точку роста за три простых шага?"
                ])
                
                hook = random.choice(hooks)
                
                # Заменяем первую строку или добавляем перед ней
                lines[0] = hook
                zen_text = '\n'.join(lines)
                
                logger.info(f"✅ Добавлен крючок-убийца: {hook[:60]}...")
    ...
    def add_case_study(self, text, theme):
        """Добавляет кейс из практики"""
        case_studies = {
            "HR и управление персоналом": [
                "Кейс из практики: одна компания внедрила этот подход и увидела рост вовлеченности сотрудников на 40% за полгода.",
                "Пример из опыта: в организации, применившей эту методику, текучесть кадров снизилась на 35%.",
                "Реальный случай: команда, использовавшая эти принципы, достигла показателей продуктивности на 25% выше среднего."
            ],
            "PR и коммуникации": [
                "Практический пример: бренд, применивший эту стратегию, увеличил узнаваемость на 60% за квартал.",
                "Кейс: компания, использовавшая этот подход в кризисной ситуации, не только сохранила репутацию, но и укрепила доверие клиентов.",
                "Пример из индустрии: кампания, построенная на этих принципах, привлекла на 45% больше лидов при том же бюджете."
            ],
            "ремонт и строительство": [
                "Пример с объекта: применение этой технологии позволило сократить сроки сдачи на 20% без потери качества.",
                "Кейс из практики: на проекте, где использовали этот подход, удалось сэкономить 15% бюджета на материалах.",
                "Реальный опыт: застройщик, внедривший эту методику, снизил количество рекламаций на 30%."
            ]
        }
        
        cases = case_studies.get(theme, ["Практический опыт показывает эффективность этого подхода."])
        
        if random.random() < 0.5:  # 50% вероятность добавить кейс
            case_text = random.choice(cases)
            ...
            if hashtag_start > 0:
                ...
                lines.insert(hashtag_start, case_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{case_text}"
    ...
    def add_statistical_data(self, text, theme):
        """Добавляет статистические данные"""
        stats_by_theme = {
            "HR и управление персоналом": [
                "Статистика показывает: компании с внедренными подобными практиками имеют на 30% более высокие показатели удержания персонала.",
                "По данным исследований: применение этих методов увеличивает удовлетворенность сотрудников на 25-40%.",
                "Цифры говорят сами за себя: организации, использующие такой подход, достигают на 35% лучших результатов в развитии лидерства."
            ],
            "PR и коммуникации": [
                "Согласно аналитике: бренды с продуманной коммуникационной стратегией имеют на 50% более высокую лояльность аудитории.",
                "Статистические данные: кампании, построенные на этих принципах, показывают на 40% большую конверсию.",
                "Исследования подтверждают: правильный подход к PR увеличивает капитализацию бренда на 20-30%."
            ],
            "ремонт и строительство": [
                "Цифры из практики: применение современных технологий сокращает сроки строительства на 25% в среднем.",
                "Статистика проектов: качественное планирование позволяет сэкономить до 20% бюджета на непредвиденные расходы.",
                "Данные исследований: правильный выбор материалов увеличивает срок службы конструкций на 30-40%."
            ]
        }
        
        stats = stats_by_theme.get(theme, ["Статистические исследования подтверждают эффективность этого подхода."])
        
        if random.random() < 0.4:  # 40% вероятность добавить статистику
            stat_text = random.choice(stats)
            ...
            if hashtag_start > 0:
                lines.insert(hashtag_start, '')
                lines.insert(hashtag_start, stat_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{stat_text}"
    ...
    def add_industry_example(self, text, theme):
        ...
        examples_by_theme = {
            "HR и управление персоналом": [
                "Пример из индустрии: ведущие IT-компании уже несколько лет успешно применяют подобные методики управления.",
                "Отраслевая практика: в финансовом секторе этот подход доказал свою эффективность в управлении высококвалифицированными специалистами.",
                "Международный опыт: глобальные корпорации используют эти принципы для построения кросс-культурных команд."
            ],
            "PR и коммуникации": [
                "Опыт индустрии: крупнейшие медиа1холдинги строят свои коммуникации на схожих принципах.",
                "Пример из сферы: технологические стартапы успешно применяют этот подход для быстрого роста узнаваемости.",
                "Международная практика: глобальные бренды адаптируют эти стратегии для разных культурных рынков."
            ],
            "ремонт и строительство": [
                "Отраслевой пример: ведущие девелоперские компании давно внедрили подобные подходы в свои стандарты.",
                "Международный опыт: в странах Европы эти технологии применяются уже более десяти лет.",
                "Пример из практики: крупные строительные холдинги используют эти методы для управления сложными проектами."
            ]
        }
        
        examples = examples_by_theme.get(theme, ["Отраслевая практика подтверждает эффективность этого подхода."])
        
        if random.random() < 0.3:  # 30% вероятность добавить пример
            example_text = random.choice(examples)
            ...
            if hashtag_start > 0:
                lines.insert(hashtag_start, '')
                lines.insert(hashtag_start, example_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{example_text}"
    ...
    def add_expert_recommendation(self, text, theme):
        ...
        recommendations_by_theme = {
            "HR и управление персоналом": [
                "Эксперты рекомендуют: начинать с пилотного проекта, чтобы оценить эффективность подхода.",
                "Совет от профессионалов: важно обеспечить поддержку руководства при внедрении таких изменений.",
                "Рекомендация специалистов: сочетать этот метод с существующими практиками для лучшего результата."
            ],
            "PR и коммуникации": [
                "Совет экспертов: тестировать разные варианты коммуникации на фокус-группах перед запуском.",
                "Рекомендация профессионалов: постоянно мониторить обратную связь и адаптировать стратегию.",
                "Экспертный совет: интегрировать этот подход в общую маркетинговую стратегию компании."
            ],
            "ремонт и строительство": [
                "Рекомендация специалистов: привлекать независимых экспертов для оценки качества работ.",
                "Совет профессионалов: использовать проверенные материалы от надежных поставщиков.",
                "Экспертный совет: тщательно планировать каждый этап работ для минимизации рисков."
            ]
        }
        
        recommendations = recommendations_by_theme.get(theme, ["Эксперты рекомендуют внимательно изучить все аспекты перед внедрением."])
        
        if random.random() < 0.4:  # 40% вероятность добавить рекомендацию
            ...
            if hashtag_start > 0:
                lines.insert(hashtag_start, '')
                lines.insert(hashtag_start, recommendation_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{recommendation_text}"
    ...
    def add_expert_quote(self, text, theme):
        ...
        quotes_by_theme = {
            "HR и управление персоналом": [
                "Как отмечает HR-директор крупной компании: «Этот подход изменил наше отношение к управлению талантами».",
                "Эксперт в области управления персоналом подчеркивает: «Методика доказала свою эффективность в разных отраслях».",
                "По словам специалиста по организационному развитию: «Внедрение таких практик требует системного подхода»."
            ],
            "PR и коммуникации": [
                "PR-директор известного бренда отмечает: «Эта стратегия стала ключом к нашему успеху на рынке».",
                "Эксперт по медиа-коммуникациям утверждает: «Подход позволяет выстраивать долгосрочные отношения с аудиторией».",
                "Специалист по брендингу подчеркивает: «Методика особенно эффективна в условиях цифровой трансформации»."
            ],
            "ремонт и строительство": [
                "Главный инженер строительной компании отмечает: «Технология значительно повысила качество наших объектов».",
                "Эксперт по строительным материалам утверждает: «Этот подход соответствует самым строгим международным стандартам».",
                "Архитектор с 20-летним стажем подчеркивает: «Методика позволяет создавать более комфортные и безопасные пространства»."
            ]
        }
        
        quotes = quotes_by_theme.get(theme, ["Эксперты отмечают важность этого подхода для современной практики."])
        
        if random.random() < 0.3:  # 30% вероятность добавить цитату
            ...
            if hashtag_start > 0:
                lines.insert(hashtag_start, '')
                lines.insert(hashtag_start, quote_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{quote_text}"
    ...
    def add_practical_advice(self, text, theme):
        ...
        advice_by_theme = {
            "HR и управление персоналом": [
                "Практический совет: начните с небольшого пилотного проекта в одном отделе.",
                "Что можно сделать уже сейчас: проведите опрос сотрудников об их потребностях.",
                "Конкретный шаг: назначьте ответственного за внедрение и мониторинг процесса."
            ],
            "PR и коммуникации": [
                "Практическая рекомендация: создайте шаблоны коммуникаций для разных ситуаций.",
                "Что сделать в первую очередь: проанализируйте текущее восприятие бренда аудиторией.",
                "Конкретное действие: разработайте план коммуникаций на ближайший квартал."
            ],
            "ремонт и строительство": [
                "Практический совет: составьте детальный план работ с указанием сроков и ответственных.",
                "Что важно учесть: заранее проверьте все необходимые разрешения и документы.",
                "Конкретный шаг: создайте систему контроля качества на каждом этапе работ."
            ]
        }
        
        advice = advice_by_theme.get(theme, ["Практический совет: начните с малого и постепенно масштабируйте подход."])
        
        if random.random() < 0.5:  # 50% вероятность добавить совет
            ...
            if hashtag_start > 0:
                lines.insert(hashtag_start, '')
                lines.insert(hashtag_start, advice_text)
                return '\n'.join(lines)
            else:
                return f"{text}\n\n{advice_text}"
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше. Лаконично, без воды, строго в пределах лимита."""
        ...
        try:
            ...
            if '---' in processed_text:
                ...
                if len(parts) == 2:
                    ...
                    logger.info(f"✅ Разделение по --- | TG: {len(tg_text)}, ZEN: {len(zen_text)}")
    ...
    def format_post_text(self, text, slot_style, post_type):
        """Форматирует текст поста с ГАРАНТИЕЙ структуры"""
        import re
        ...
        if post_type == 'telegram':
            # 1. ГАРАНТИЯ: Есть эмодзи в начале
            lines = text.split('\n')
            if lines and not any(e in lines[0] for e in ['🌅', '🌞', '🌙']):
            ...
            # 2. ГАРАНТИЯ: После заголовка пустая строка
            lines = text.split('\n')
            if len(lines) > 1 and lines[1].strip() != '':
                lines.insert(1, '')
                text = '\n'.join(lines)
            
            # 3. ГАРАНТИЯ: Есть практический блок
            has_practice = any(
                marker in text for marker in 
                ['Практический совет:', 'Что делать дальше:', 'Конкретные действия:', '🎯']
            )
            if not has_practice:
                text = self._add_telegram_practice_block(text, self.current_theme)
            
            # 4. ГАРАНТИЯ: Есть инсайт/ключевая мысль
            has_insight = any(
                marker in text for marker in 
                ['Главный инсайт:', 'Ключевая мысль:', 'Самое важное:', '💡', '✨', '🎯']
            )
            if not has_insight:
                text = self._add_telegram_insight(text, self.current_theme)
                
        elif post_type == 'zen':
            # 1. ГАРАНТИЯ: НЕТ эмодзи вообще
            emoji_pattern = re.compile("["
                u"\U0001F600-\U0001F64F"
                u"\U0001F300-\U0001F5FF" 
                u"\U0001F680-\U0001F6FF"
                u"\U0001F900-\U0001F9FF"  # добавил больше диапазонов
                "]+", flags=re.UNICODE)
            text = emoji_pattern.sub(r'', text).strip()
            
            # 2. ГАРАНТИЯ: Есть крючок-убийца
            text = self._ensure_zen_hook(text, self.current_theme)
            
            # 3. ГАРАНТИЯ: Есть блок завершения
            ...
            # 4. ГАРАНТИЯ: Форматирование абзацев
            lines = [line.strip() for line in text.split('\n') if line.strip()]
            
            # Восстанавливаем пустые строки между абзацами
            formatted_lines = []
            for i, line in enumerate(lines):
        ...
        if hashtag_lines:
            clean_text = '\n'.join(content_lines).strip()
            hashtags = ' '.join(hashtag_lines[0].split())  # Нормализуем пробелы
            return f"{clean_text}\n\n{hashtags}"
    ...
    def send_to_admin_for_moderation(self, slot_time, tg_text, zen_text, image_url, theme):
        ...
        def validate_post_structure(text, post_type):
            ...
            if post_type == 'telegram':
                ...
                # 3. Проверка блока завершения
                if not any(marker in text for marker in 
                          ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']):
                    errors.append("❌ Нет блока завершения")
