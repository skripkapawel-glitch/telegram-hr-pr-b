...
• Стиль: ЖИВОЙ, динамичный, человеческий, много эмодзи {slot_info['emoji']}
• Структура:
  1. СИЛЬНЫЙ ХУК: интрига или провокационный факт с первых слов
  2. ЯСНАЯ ЛОГИКА: факт → мини-кейс/наблюдение → вывод
  3. КОРОТКИЕ ФРАЗЫ с отступами
  4. 1-2 сильных тезиса, которые читатель сразу "схватит"
  5. МЯГКИЙ ФИНАЛ: вовлекающий вопрос ("Как вы считаете?", "А у вас было такое?")
  6. Хештеги: 5-7 релевантных
...
• Структура:
  ...
  2. ОСНОВНАЯ ЧАСТЬ: глубокий разбор в выбранном формате ({text_format})
  3. АНАЛИЗ: факты, мини-исследования, выводы
  4. ЧЕТКАЯ СТРУКТУРА с отступами
  5. ЗАКРЫВАШКА: мини-итог + приглашение поделиться мнением
• Требования:
  - Ощущение мини-статьи, но читается легко
  - Экспертность через реальные ситуации (если опыт — через 3-е лицо)
  - Глубина важнее объема

🎯 ТИП ПОСТА: {slot_info['name']} ({slot_info['type']})
🕒 ВРЕМЯ СУТОК: {slot_info['emoji']} {slot_info['name'].lower()}

ВАЖНО:
1. Telegram — быстро, ярко, живо. Дзен — глубина и разборы.
2. ИСПОЛЬЗУЙ КОНКРЕТИКУ: цифры, примеры, кейсы, когда уместно.
3. ИЗБЕГАЙ клише и шаблонных фраз.
4. УНИКАЛЬНЫЙ, НЕШАБЛОННЫЙ текст.

Формат вывода (соблюдай строго!):
TG: [текст Telegram-поста с эмодзи]
---
DZEN: [текст Дзен-поста без эмодзи]"""
