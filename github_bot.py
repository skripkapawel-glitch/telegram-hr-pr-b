...
print("=" * 80)
print("🚀 GITHUB BOT: ГЕНЕРАЦИЯ ПОСТОВ С ОБНОВЛЕННОЙ ДЛИНОЙ")
print("=" * 80)
...
class AIPostGenerator:
    def __init__(self):
        ...
        # Временные слоты с ОБНОВЛЕННОЙ длиной
        self.time_slots = {
            "09:00": {
                "type": "morning",
                "name": "Утренний пост",
                ...
                "zen_chars": (600, 700),     # Дзен: 600-700 (уменьшено)
            ...
            "14:00": {
                ...
                "name": "Дневной пост",
                "emoji": "🌞",
                "tg_chars": (800, 1000),     # Telegram: 800-1000 (уменьшено)
                "zen_chars": (700, 800),     # Дзен: 700-800 (уменьшено)
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
            ...
            "19:00": {
                ...
                "name": "Вечерний пост",
                "emoji": "🌙",
                "tg_chars": (600, 1000),     # Telegram: 600-1000
                "zen_chars": (800, 1000),    # Дзен: 800-1000 (уменьшено)
                "tg_style": "живой, динамичный, человеческий, много эмодзи",
                "zen_style": "глубже, аналитичнее, как мини-статья. Без эмодзи",
...
ВАЖНО: ГЕНЕРИРУЙ ПОСТЫ ТОЧНОЙ ДЛИНЫ!
• Telegram пост должен быть от {tg_chars_min} до {tg_chars_max} символов
• Дзен пост должен быть от {zen_chars_min} до {zen_chars_max} символов
• Не делай посты слишком длинными! Соблюдай ограничения!
...
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС. УЧТИ ВСЕ ТРЕБОВАНИЯ ВЫШЕ."""
    ...
    def split_telegram_and_zen_text(self, combined_text):
        """Разделяет текст на Telegram и Zen посты и извлекает поисковый запрос"""
        if not combined_text:
            return None, None, None
        
        # Ищем поисковый запрос для изображения
        image_query = None
        ...
        for marker in query_markers:
            if marker in combined_text:
                ...
                # Берем первую строку после маркера
        ...
        if tg_start != -1 and zen_start != -1:
            ...
            # Убираем поисковый запрос если он есть в этой части
            for marker in query_markers:
                if marker in zen_part:
                    zen_part = zen_part.split(marker)[0]
        ...
        for separator in separators:
            if separator in combined_text:
                ...
                if len(parts) == 2:
                    # Убираем поисковый запрос если он есть
                    for marker in query_markers:
                        if marker in parts[1]:
                            parts[1] = parts[1].split(marker)[0]
                    return parts[0].strip(), parts[1].strip(), image_query
        
        # Если ничего не нашли, делим пополам
        text_length = len(combined_text)
        if text_length > 500:
            split_point = text_length // 2
            return combined_text[:split_point].strip(), combined_text[split_point:].strip(), image_query
        
        return combined_text, combined_text, image_query

    def generate_image_search_query(self, text, theme):
        """Генерирует поисковый запрос для изображения на основе текста"""
        try:
            # Если уже есть запрос от Gemini, используем его
            ...
            # Иначе создаем запрос на основе темы и ключевых слов
            theme_keywords = {
                "HR и управление персоналом": ["офис", "команда", "встреча", "переговоры", "бизнес", "управление"],
                "PR и коммуникации": ["коммуникация", "медиа", "презентация", "аудитория", "брендинг", "маркетинг"],
                "ремонт и строительство": ["ремонт", "строительство", "инструменты", "дизайн", "интерьер", "дом"]
            }
            ...
            # Выбираем основное ключевое слово
            if found_keywords:
            ...
            contexts = ["профессиональный", "деловой", "рабочий", "современный", "успешный"]
    ...
    def get_fresh_image(self, text, theme, width=1200, height=630):
        """Находит свежую картинку по поисковому запросу"""
        try:
            # Генерируем поисковый запрос
            search_query = self.generate_image_search_query(text, theme)
            
            # Используем Pexels API (бесплатный, хорошее качество)
            pexels_api_key = "563492ad6f91700001000001d15a5e2d6a9d4b5c8c0e6f5b8c1a9b7c"  # Примерный ключ, можно заменить
            ...
            if response.status_code == 200:
                ...
                if data.get('photos') and len(data['photos']) > 0:
                    ...
                    logger.info(f"✅ Найдено изображение по запросу: '{search_query}'")
                    return image_url
            
            # Если Pexels не сработал, используем Picsum с темой
            logger.info(f"⚠️ Pexels не нашел изображение, используем Picsum")
    ...
    def generate_and_send_posts(self):
        ...
        try:
            ...
            # Сохраняем поисковый запрос для использования в get_fresh_image
            if image_query:
                self.last_image_query = image_query
                logger.info(f"🔍 Получен поисковый запрос от Gemini: {image_query}")
            ...
            logger.info(f"📊 Длина Telegram поста: {tg_len} символов (требуется: {tg_min}-{tg_max})")
            logger.info(f"📊 Длина Дзен поста: {zen_len} символов (требуется: {zen_min}-{zen_max})")
            
            if tg_len < tg_min or tg_len > tg_max * 1.2:
                logger.warning(f"⚠️ Длина Telegram поста не соответствует требованиям")
            
            if zen_len < zen_min or zen_len > zen_max * 1.2:
                logger.warning(f"⚠️ Длина Дзен поста не соответствует требованиям")
            
            logger.info("🖼️ Ищем свежие картинки...")
            # Используем оба текста для генерации изображения
            combined_post_text = f"{tg_text[:300]} {zen_text[:300]}"
            ...
            if success_count == 2:
                ...
                logger.info("\n" + "=" * 60)
                logger.info("🎉 УСПЕХ! Посты отправлены с обновленной длиной!")
                logger.info("=" * 60)
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🤖 GITHUB TELEGRAM HR BOT - ОБНОВЛЕННАЯ ДЛИНА ПОСТОВ")
    print("=" * 80)
    print("📋 Обновленная структура длин:")
    print("   Telegram: 09:00=400-600, 14:00=800-1000, 19:00=600-1000")
    print("   Яндекс Дзен: 09:00=600-700, 14:00=700-800, 19:00=800-1000")
    print("   Добавлена генерация поисковых запросов для изображений")
    print("=" * 80)
    ...
    if success:
        ...
        print("   Посты соответствуют обновленной длине")
        print("=" * 50)
        sys.exit(0)
    else:
        print("\n" + "=" * 50)
