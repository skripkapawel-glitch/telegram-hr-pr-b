...
class TelegramBot:
    def __init__(self):
        ...
        # ОБНОВЛЕННЫЕ объемы по временным слотам (согласно требованиям)
        self.schedule = {
            "09:00": {
                "name": "Утренний пост",
                "type": "morning",
                "emoji": "🌅",
                "tg_chars": (400, 600),
                "zen_chars": (600, 700)  # Обновлено: 600-700 вместо 1000-1500
            },
            "14:00": {
                "name": "Дневной пост",
                "type": "day",
                "emoji": "🌞",
                "tg_chars": (700, 900),
                "zen_chars": (700, 900)  # Обновлено: 700-900 вместо 700-850
            },
            "19:00": {
                "name": "Вечерний пост",
                "type": "evening",
                "emoji": "🌙",
                "tg_chars": (600, 900),
                "zen_chars": (700, 800)  # Обновлено: 700-800 вместо 800-900
            }
        }
    ...
    def create_prompt(self, theme, slot_info, text_format):
        """Создает промпт для Gemini с полным промптом из требований"""
        
        # ПРОМТ ИЗ ТРЕБОВАНИЙ (сохраняем полностью)
        ...
        # ОСНОВНОЙ ПРОМПТ с учетом всех требований
...
НИКАКИХ ДОПОЛНИТЕЛЬНЫХ ПОЯСНЕНИЙ, ТОЛЬКО ДВА ТЕКСТА В УКАЗАННОМ ФОРМАТЕ!"""
    ...
    def get_post_image(self, theme):
        """Находит подходящую картинку через Pexels API (бесплатный)"""
        try:
            # Параметры поиска по темам
            theme_queries = {
            ...
            # Fallback на Unsplash если Pexels не сработал
    ...
    def clean_text_for_posting(self, text):
        ...
        # Убираем предисловия типа "Вот пожалуйста держи"
    ...
    def create_and_send_posts(self, slot_time, slot_info, is_test=False, force_send=False):
        ...
        try:
            ...
            logger.info(f"📝 Пример Telegram текста: {tg_text[:100]}...")
            logger.info(f"📝 Пример Дзен текста: {zen_text[:100]}...")
