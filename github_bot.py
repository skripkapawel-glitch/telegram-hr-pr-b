...
• Структура:
  ...
  - Основной текст с пунктами (начинаются с •)
  - Вопрос для обсуждения
  - 3-6 хештегов
• Форматирование: пункты с отступом, обычный текст без отступа

2. ДЗЕН ПОСТ ({zen_chars_min}-{zen_chars_max} символов):
• Стиль: глубже, аналитичнее, как мини-статья
...
• Все посты должны быть уникальными и интересными
• Используй реальные ситуации и примеры
• Экспертность через опыт знакомых или личные наблюдения
...
Telegram-пост:
[Текст для Telegram с эмодзи, пунктами с отступом, вопросом и хештегами]

Дзен-пост:
[Текст для Дзен без эмодзи, абзацами без отступов, с вопросом и подписью]

Поисковый запрос для изображения:
[Один чёткий запрос на русском языке, основанный на анализе поста и его тематики]

⸻
НАЧИНАЙ ГЕНЕРАЦИЮ СЕЙЧАС. Создай по-настоящему цепляющие и уникальные посты!"""
    ...
    def generate_image_search_query(self, text, theme):
        ...
        try:
            ...
            # Fallback на тематические ключевые слова
            theme_keywords = {
                "HR и управление персоналом": ["офис", "команда", "бизнес", "управление", "персонал", "совещание"],
                "PR и коммуникации": ["коммуникация", "медиа", "маркетинг", "бренд", "презентация", "переговоры"],
                "ремонт и строительство": ["ремонт", "строительство", "инструменты", "дом", "интерьер", "отделка"]
            }
            
            keywords = theme_keywords.get(theme, ["бизнес", "профессия", "работа"])
            main_keyword = random.choice(keywords)
            
            image_query = f"{main_keyword} работа профессия"
            logger.info(f"🔍 Используем fallback запрос: {image_query}")
            return image_query
            
        except Exception as e:
            logger.error(f"❌ Ошибка генерации запроса: {e}")
            return "бизнес профессия работа"
    ...
    def get_fresh_image(self, text, theme, width=1200, height=630):
        ...
        try:
            ...
            # Fallback на Picsum
            logger.info(f"⚠️ Используем Picsum как fallback")
...
def main():
    ...
    print("=" * 80)
    print("📋 Возможности:")
    print("   • AI генерирует уникальные посты")
    print("   • Telegram: с эмодзи и пунктами")
    print("   • Яндекс.Дзен: аналитический стиль без эмодзи")
    print("   • Автоматический поиск картинок")
    print("   • Контроль длины постов")
