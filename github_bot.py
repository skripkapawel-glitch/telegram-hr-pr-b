...
⚠️ ⚠️ ⚠️ СТРОГОЕ ТРЕБОВАНИЕ К ЛИМИТАМ СИМВОЛОВ ⚠️ ⚠️ ⚠️

1. Telegram: ДОЛЖЕН БЫТЬ ОТ {tg_min} ДО {tg_max} СИМВОЛОВ
2. Дзен: ДОЛЖЕН БЫТЬ ОТ {zen_min} ДО {zen_max} СИМВОЛОВ

НИКАКИХ ИСКЛЮЧЕНИЙ! Если текст не соответствует - перепиши его!
...
Пример на 500 символов:
🌅 "Знаете ли вы, что эффективные советы по HR могут повысить продуктивность команды на 40%? 

Многие руководители допускают типичную ошибку: дают общие рекомендации вместо конкретных шагов. 

Вот 3 конкретных действия:
• Проводите регулярные 1-on-1 встречи
• Создайте систему обратной связи
• Развивайте корпоративную культуру

Начните с малого, но будьте последовательны.

Какие методы используете вы в своей команде?"

Дзен ({zen_min}-{zen_max} символов):
Пример на 650 символов:
"Эффективное управление персоналом: от теории к практике

Управление человеческими ресурсами давно перестало быть просто кадровым делопроизводством. Сегодня это стратегический инструмент развития компании, напрямую влияющий на бизнес-результаты.

Основная проблема многих организаций - разрыв между теорией HR и реальной практикой. Политики, написанные в корпоративных документах, часто не работают в повседневной деятельности.

Ключевой момент - адаптация общих принципов под конкретную команду. То, что работает в крупной IT-компании, может не подойти для небольшого производственного предприятия.

Системный подход к управлению персоналом позволяет не только повысить эффективность работы, но и создать устойчивую организационную культуру.

Какой опыт внедрения HR-практик есть в вашей организации?"

ВЫХОДНОЙ ФОРМАТ:

TG:
[Твой Telegram пост здесь]
---
DZEN:
[Твой Дзен пост здесь]

ПРОВЕРЬ ДЛИНУ КАЖДОГО ТЕКСТА!
len(text) должен быть: TG {tg_min}-{tg_max}, Дзен {zen_min}-{zen_max}"""

        logger.info(f"📝 Создан упрощенный строгий промпт для Gemini")
