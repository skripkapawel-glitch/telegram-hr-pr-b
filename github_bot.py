...
class TelegramBot:
    ...
    def create_prompt(self, theme, slot_info, text_format):
        ...
        slot_time = slot_info.get('name', '').split()[0]
        time_style = time_styles.get(slot_time, "аналитика, рациональность, польза")
...
ТОЧНЫЕ ОБЪЁМЫ СИМВОЛОВ:
• Telegram: ТОЧНО {tg_min}-{tg_max} символов
• Дзен: ТОЧНО {zen_min}-{zen_max} символов
...
ВАЖНО: Генерируй ДВА ПОЛНОСТЬЮ РАЗНЫХ текста для разных платформ!"""
    ...
    def format_telegram_text(self, text, slot_info):
        ...
        if text_length < max_length - 50:
            hashtags = f"\n\n#{theme.replace(' ', '').replace('и', '_').replace(' ', '_') if self.current_theme else 'бизнес'}"
