...
class TelegramBot:
    ...
    def create_telegram_prompt(self, theme, slot_info, text_format):
        """Создает промпт для Telegram поста с улучшенной эмодзи-поддержкой"""
...
ИСПОЛЬЗУЙ БОЛЬШЕ ЭМОДЗИ В ТЕКСТЕ! Добавляй релевантные эмодзи к ключевым фразам.
...
Второй абзац из 2-3 предложений с эмодзи для эмоций 🚀
...
ТВОЙ ПОСТ (только текст):"""
    ...
    def create_zen_prompt(self, theme, slot_info, text_format):
        """Создает промпт для Дзен поста с закрывающей фразой и релевантными хэштегами"""
...
НЕ ИСПОЛЬЗУЙ ЭМОДЗИ В ТЕКСТЕ! Только в конце — стандартная закрывающая фраза.

ВАЖНО ДЛЯ ДЗЕН: Используй релевантные хэштеги по теме "{theme}"
...
Отредактированный текст (только текст, без пояснений):"""
    ...
    def generate_single_post(self, prompt, target_chars_min, target_chars_max, post_type):
        ...
        try:
            ...
            max_tokens = int(target_chars_max / 3 * 1.5)
