...
4. Мини-вывод или ключевая мысль (инсайт).

5. Мягкий финал: {soft_final}

6. Хештеги: {hashtags_str}

📝 СТРУКТУРА ДЗЕН (СТРУКТУРА «КРЮЧОК-УБИЙЦА»):
1. КРЮЧОК-УБИЙЦА
Первая строка — провокационный вопрос, заявление или неочевидный факт, который ломает привычную картину. БЕЗ ЭМОДЗИ.

2. СУТЬ ЗА 15 СЕКУНД
Второй абзац — только сухие факты. Что произошло, что представлено или обнародовано, без оценок.

3. ПОЧЕМУ ЭТО ВАЖНО (2-3 пункта)
...
5. Дзен пост — КРЮЧОК-УБИЙЦА БЕЗ ЭМОДЗИ ВООБЩЕ
...
1. Строго соблюдать пустые строки между структурными блоками
2. Не сливать блоки в один текст
3. Каждый структурный блок начинать с новой строки
4. Telegram пост должен иметь пустые строки между 6 блоками
5. Дзен пост должен иметь строгую структуру с пустыми строками между блоками:
   - Крючок-убийца
   - ПУСТАЯ СТРОКА
   - Суть за 15 секунд
   - ПУСТАЯ СТРОКА
...
• Потом Дзен версия (с шапкой без эмодзи, структура «Крючок-убийца»)
• Разделитель: три дефиса (---)
...
Создай два РАЗНЫХ текста по одной теме. Оба должны быть уникальными по структуре, но об одном смысле."""
    ...
    def format_zen_text(self, text, slot_style):
        """Форматирует Дзен текст (без эмодзи) - ГАРАНТИРУЕТ наличие хештегов и структуру «Крючок-убийца»"""
        if not text:
            return None
        ...
        while i < len(lines):
            ...
            if line:
                ...
                # Проверяем, нужно ли добавить пустую строку после этого блока
                if i < len(lines) - 1:
                    next_line = lines[i + 1].strip()
                    if next_line:
                        # Проверяем, является ли текущая строка концом структурного блока
        ...
        # Убедимся, что у нас есть правильная структура блоков с пустыми строками
        # Проверяем наличие ключевых структурных элементов
        has_hook = any('?' in line or '!' in line for line in structured_lines[:3])
        has_bullets = any(line.startswith('•') for line in structured_lines)
        
        # Если структура нарушена, пытаемся восстановить
        if not has_hook or not has_bullets:
            logger.warning("⚠️ Структура Дзен поста нарушена. Восстанавливаю...")
            ...
            if len(paragraphs) >= 4:
                ...
                if len(paragraphs) > 2:
                    # Проверяем, есть ли маркированный список
                    if '•' in paragraphs[2]:
                        restored_text += f"{paragraphs[2]}\n\n"  # Почему это важно
                    elif len(paragraphs) > 3 and '•' in paragraphs[3]:
                        restored_text += f"{paragraphs[3]}\n\n"  # Почему это важно
                        # Пропускаем один абзац
                        paragraphs.pop(3)
                ...
                for i in range(2, len(paragraphs)):
                    if i < len(paragraphs) - 1:
                        ...
                        restored_text += paragraphs[i]  # Последний абзац (включая хештеги)
