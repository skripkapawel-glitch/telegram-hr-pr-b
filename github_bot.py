...
[1] {slot_style['emoji']} ЗАГОЛОВОК: Создай провокационный вопрос или утверждение по теме "{theme}". Начни сразу с эмодзи {slot_style['emoji']}. Используй разнообразные формулировки, не повторяй предыдущие тексты. АБСОЛЮТНО НОВЫЙ УНИКАЛЬНЫЙ ЗАГОЛОВОК.

[2] АБЗАЦ 1: Свободный вход в мысль и небольшое развитие. 2-3 предложения. Используй разные подходы: можно начать с примера, вопроса, цитаты или личного опыта. УНИКАЛЬНЫЙ КОНТЕНТ, НЕ ПОВТОРЯЙ ПРЕДЫДУЩИЕ АБЗАЦЫ.

[3] 🎯 КЛЮЧЕВАЯ МЫСЛЬ: одна явная и конкретная мысль по теме. Начни с 🎯. 1-2 предложение. Сформулируй по-новому, избегая шаблонных фраз. АБСОЛЮТНО НОВАЯ ФОРМУЛИРОВКА.

[4] ВОПРОС: Задай вопрос читателю. Отдельная строка. Заканчивается знаком ?. Вопрос должен отличаться от заголовка. УНИКАЛЬНЫЙ ВОПРОС, НЕ ПОВТОРЯЙСЯ.
...
8. НИ В КОЕМ СЛУЧАЕ не повторяй предыдущие тексты, используй абсолютно разные формулировки, примеры и ракурсы
...
[1] ЗАГОЛОВОК: Создай провокационный вопрос или утверждение по теме "{theme}". Без эмодзи. Заголовок должен заканчиваться знаком ?. Используй разнообразные формулировки. АБСОЛЮТНО НОВЫЙ УНИКАЛЬНЫЙ ЗАГОЛОВОК.

[2] АБЗАЦ 1: Разверни тему. 1-3 предложения. Начни с разных подходы: статистика, пример, вопрос к читателю. УНИКАЛЬНЫЙ КОНТЕНТ, НЕ ПОВТОРЯЙ ПРЕДЫДУЩИЕ АБЗАЦЫ.

[3] КЛЮЧЕВАЯ МЫСЛЬ: одна явная и конкретная мысль по теме. 1-2 предложения. Сформулируй уникально. АБСОЛЮТНО НОВАЯ ФОРМУЛИРОВКА.

[4] ВОПРОС: Задай новый вопрос для обсуждения, отличный от заголовка. Отдельная строка. Заканчивается знаком ?. УНИКАЛЬНЫЙ ВОПРОС, НЕ ПОВТОРЯЙСЯ.
...
"""
    ...
    def validate_post_structure(self, text: str, post_type: str, slot_style: Dict = None) -> Tuple[bool, str]:
        """Проверяет структуру поста и принудительно исправляет - ИСПРАВЛЕНО"""
        if not text:
    ...
    def get_post_image_and_description(self, theme: str) -> Tuple[Optional[str], str]:
        ...
        try:
            theme_queries = {
                "ремонт и строительство": ["construction", "renovation", "architecture", "building", "home improvement", "interior design"],
                "HR и управление персоналом": ["office", "business", "teamwork", "leadership", "recruitment", "corporate"],
                "PR и коммуникации": ["communication", "marketing", "media", "social media", "public relations", "networking"]
            }
                ...
                if response.status_code == 200:
                    ...
                    if photos:
                        ...
                        if available:
                            ...
                            # Если все фото использованы, очищаем историю для этого запроса
            ...
            # Если Pexels не сработал, используем Unsplash с расширенным запросом
            encoded_query = quote_plus(f"{query} {random.randint(1, 1000)}")
