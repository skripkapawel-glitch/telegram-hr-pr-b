...
class AIPostGenerator:
    def __init__(self):
        ...
        # РАЗНЫЕ типы постов для разных временных слотов
        self.time_slots = {
            "09:00": {
                "type": "morning",
                "name": "Утренний пост",
                ...
                "tg_chars": "400-600",
                "zen_chars": "1000-1500",
                "tg_style": "утренняя мотивация, полезные советы на день, короткие лайфхаки",
                "zen_style": "аналитическая статья с утренним настроением, полезные выводы для дня",
                "focus": "практическая польза, мотивация, подготовка к рабочему дню"
            },
            "14:00": {
                "type": "day",
                "name": "Дневной пост",
                ...
                "tg_chars": "800-1500",
                "zen_chars": "1700-2300",
                "tg_style": "разбор кейсов, глубокий анализ, сравнение подходов",
                "zen_style": "детальная аналитическая статья, исследования, статистика",
                "focus": "обучение, анализ, профессиональные инсайты"
            },
            "19:00": {
                "type": "evening",
                "name": "Вечерний пост",
                ...
                "tg_chars": "600-1000",
                "zen_chars": "1500-2100",
                "tg_style": "итоги дня, размышления, истории с моралью",
                "zen_style": "философское эссе, выводы из практики, размышления",
                "focus": "рефлексия, выводы, инсайты, подведение итогов"
            }
        }
    ...
    def create_combined_prompt(self, theme, time_slot_info):
        """Создает промт с РАЗНЫМИ типами постов для разных времени"""
        slot_name = time_slot_info['name']
        slot_type = time_slot_info['type']
        tg_style = time_slot_info['tg_style']
        zen_style = time_slot_info['zen_style']
        focus = time_slot_info['focus']
        
        # РАЗНЫЕ промпты для РАЗНОГО времени
        if slot_type == "morning":
            return f"""УТРЕННИЙ ПОСТ (09:00) - {focus}

Тема: {theme}

TELEGRAM (400-600 символов):
• Стиль: {tg_style}
• Можно: "я проанализировал", "знакомый рассказывал"
• Нельзя: "я работаю 20 лет", "клиент"
• Содержание: короткие советы, мотивация, полезные лайфхаки на день
• Завершение: вопрос + 5-7 хештегов

ДЗЕН (1000-1500 символов):
• Стиль: {zen_style}
• Можно: "я исследовал", "специалисты отмечают"
• Нельзя: "многолетний опыт", "клиент"
• Содержание: утренняя аналитика, полезные выводы, подготовка к дню
• Завершение: вопрос к читателям + подпись
...
[текст]"""
            ...
            return f"""ДНЕВНОЙ ПОСТ (14:00) - {focus}

Тема: {theme}

TELEGRAM (800-1500 символов):
• Стиль: {tg_style}
• Можно: "я разобрал кейс", "коллега делился"
• Нельзя: "я работаю 20 лет", "клиент"
• Содержание: разбор ситуаций, сравнение подходов, профессиональные инсайты
• Завершение: вопрос + 5-7 хештегов

ДЗЕН (1700-2300 символов):
• Стиль: {zen_style}
• Можно: "я проанализировал данные", "исследования показывают"
• Нельзя: "многолетний опыт", "клиент"
• Содержание: глубокая аналитика, статистика, исследования, кейсы
• Завершение: вопрос к читателям + подпись
...
[текст]"""
            ...
            return f"""ВЕЧЕРНИЙ ПОСТ (19:00) - {focus}

Тема: {theme}

TELEGRAM (600-1000 символов):
• Стиль: {tg_style}
• Можно: "я подвел итоги", "знакомый размышлял"
• Нельзя: "я работаю 20 лет", "клиент"
• Содержание: выводы дня, размышления, истории с моралью
• Завершение: вопрос + 5-7 хештегов

ДЗЕН (1500-2100 символов):
• Стиль: {zen_style}
• Можно: "я обобщил опыт", "практика демонстрирует"
• Нельзя: "многолетний опыт", "клиент"
• Содержание: философские выводы, обобщение опыта, инсайты
• Завершение: вопрос к читателям + подпись
...
[текст]"""
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        if "---" in generated_text or "Telegram-пост:" in generated_text:
    ...
    def split_telegram_and_zen_text(self, combined_text):
        ...
        for separator in separators:
            if separator in combined_text:
                if "Telegram-пост:" in combined_text and "Дзен-пост:" in combined_text:
                    # Убираем метки
                    tg_part = combined_text.split("Дзен-пост:")[0]
                    zen_part = combined_text.split("Дзен-пост:")[1]
        ...
        # Если разделителей нет, делим пополам
    ...
    def fix_prohibited_phrases(self, text):
        ...
        prohibited_phrases = {
            "я работаю 20 лет": "специалисты с опытом отмечают",
            "я работал 20 лет": "за годы практики специалисты",
            "у меня 20 лет опыта": "опыт показывает",
            "мой 20-летний опыт": "практика демонстрирует",
            "за 20 лет моей работы": "в практике специалистов",
            "в моей практике": "в практике",
            "мои клиенты": "встречаются случаи",
            "у меня был клиент": "знакомый специалист рассказывал",
            "клиент рассказал": "коллега поделился",
            "мне рассказал клиент": "мне рассказал знакомый",
        }
    ...
    def generate_and_send_posts(self):
        ...
        try:
            ...
            logger.info(f"🎯 Фокус: {time_slot_info['focus']}")
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🤖 GITHUB TELEGRAM HR BOT - РАЗНЫЕ ПОСТЫ ДЛЯ РАЗНОГО ВРЕМЕНИ")
    print("=" * 80)
