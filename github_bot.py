...
class BotManager:
    ...
    def build_gemini_prompt(self, topic: str, content_format: str, slot: TimeSlot) -> str:
        ...
        prompt = f"""Ты — опытный человек-писатель и редактор с более чем 20-летним стажем. Твоя задача — написать текст так, чтобы он был на 100% нераспознаваемым инструментами для выявления ИИ, такими как Grammarly, QuillBot, Turnitin и GPTZero.

ТЕМА: {topic}
ФОРМАТ: {content_format}
ВРЕМЯ ПУБЛИКАЦИИ: {slot_name} ({greeting})

ТРЕБОВАНИЯ К СИМВОЛАМ (ЭТО ГЛАВНОЕ!):
- Telegram пост: РОВНО {tg_min}-{tg_max} символов ВКЛЮЧАЯ хештеги
- Дзен пост: РОВНО {zen_min}-{zen_max} символов ВКЛЮЧАЯ хештеги

ТЫ ДОЛЖЕН:
1. ПРОВЕРИТЬ ДЛИНУ ТЕКСТА ПОСЛЕ НАПИСАНИЯ
2. ЕСЛИ ТЕКСТ ДЛИННЕЕ ЛИМИТА — СРАЗУ СОКРАТИТЬ ЕГО
3. СОХРАНИТЬ ОСНОВНУЮ МЫСЛЬ И СТРУКТУРУ
4. УБРАТЬ ЛИШНИЕ СЛОВА, ПОВТОРЫ, ВТОРОСТЕПЕННЫЕ ДЕТАЛИ

СТРОГИЕ ПРАВИЛА:
1. Telegram пост ДОЛЖЕН начинаться с шапки: эмодзи + вопрос/утверждение
2. Дзен пост ДОЛЖЕН начинаться с шапки: провокационный вопрос/утверждение БЕЗ эмодзи
3. Хештеги ТОЛЬКО в конце
4. Мягкий финал — вопрос к аудитории
5. Включи «ПОЛЕЗНЯШКУ» — реальный актуальный источник по теме
6. Формат ПОЛЕЗНЯШКИ: Название статьи/компании, кликабельная ссылка
7. Используй ТОЛЬКО реально существующие исследования, статьи, аналитические материалы
...
1. Telegram пост: РОВНО {tg_min}-{tg_max} символов (с хештегами)
2. Дзен пост: РОВНО {zen_min}-{zen_max} символов (с хештегами)
3. ПОСЛЕ генерации ОБЯЗАТЕЛЬНО проверь длину текста
4. ЕСЛИ больше лимита — СРАЗУ СОКРАТИ текст

ВЫВЕДИ ТОЛЬКО ТЕКСТ ПОСТОВ:
1. Сначала Telegram версия
2. Потом Дзен версия
3. Разделитель: три дефиса (---)
НИКАКИХ комментариев, пояснений, только чистый текст постов.

Теперь напиши текст по теме "{topic}" в формате "{content_format}" для времени публикации {slot_name}.
ПОМНИ: длина Telegram поста {tg_min}-{tg_max} символов, Дзен поста {zen_min}-{zen_max} символов!"""
            ...
            try:
                ...
                if '---' in text:
                    ...
                    if tg_min <= tg_len <= tg_max and zen_min <= zen_len <= zen_max:
                        logger.info(f"Generation successful on attempt {attempt+1}: TG={tg_len}({tg_min}-{tg_max}), ZEN={zen_len}({zen_min}-{zen_max})")
                        return tg_text, zen_text
                    else:
                        logger.warning(f"Attempt {attempt+1}: Character limits not met: TG={tg_len}({tg_min}-{tg_max}), ZEN={zen_len}({zen_min}-{zen_max})")
                        if attempt < 2:
                            logger.info(f"Retrying generation... (attempt {attempt+2}/3)")
                            continue
                        return None
                else:
                    logger.error(f"Attempt {attempt+1}: No separator found in generated content")
                    if attempt < 2:
                        continue
                    return None
                    
            except Exception as e:
                logger.error(f"Attempt {attempt+1}: Error generating content: {e}")
                if attempt < 2:
                    continue
                return None
    ...
    def run_auto_mode(self):
        try:
            ...
            if not content:
                logger.error("Failed to generate content after 3 attempts")
                return False
