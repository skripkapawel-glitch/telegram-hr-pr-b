...
ТЕМА: {theme}
ФОРМАТ: {text_format}
ЦЕЛЕВАЯ ДЛИНА: {target_length} символов (диапазон: {tg_min}-{tg_max})
...
ВАЖНЫЕ ПРАВИЛА:
• Держи длину около {target_length} символов
• Не пиши "Длина:" или счетчик символов в конце
• Не используй ** вокруг заголовка или текста
• Каждый абзац - законченная мысль
• Используй эмодзи умеренно и релевантно
...
ТЕМА: {theme}
ФОРМАТ: {text_format}
ЦЕЛЕВАЯ ДЛИНА: {target_length} символов (диапазон: {zen_min}-{zen_max})

СТРУКТУРА:
1. Заголовок: 1 строка, отражающая суть
2. Введение: 3-4 предложения, вводят в тему
3. Основная часть: 3-4 предложения, раскрывают тему
4. Практическая часть: 3-4 предложения с примерами
5. Заключение: 1-2 предложения, подводят итог
6. Вопрос для обсуждения: 1 вопрос читателям
7. {closing}
8. Хэштеги: {hashtags_str}

ВАЖНЫЕ ПРАВИЛА:
• Держи длину около {target_length} символов
• НИКАКИХ эмодзи в тексте (только в закрывающей фразе)
• Не пиши "Длина:" или счетчик символов в конце
• Не используй ** вокруг заголовка или текста
• Заголовок пиши обычным текстом, без выделения
...
ТВОЙ ПОСТ:"""
    ...
    def clean_generated_text(self, text):
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in ['длина:', 'символов', 'символы:', 'количество символов']):
                continue
            
            # Удаляем ** с начала и конца строки (если они окружают всю строку)
        ...
        # Удаляем возможные артефакты в конце (типа "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        cleaned_text = re.sub(r'━+$', '', cleaned_text, flags=re.MULTILINE)
        cleaned_text = re.sub(r'=+$', '', cleaned_text, flags=re.MULTILINE)
        
        # Удаляем возможные фразы типа "Текст готов" и т.д.
        unwanted_endings = [
            'текст готов', 'пост готов', 'готово', 'создано', 
            'вот пост:', 'вот текст:', 'результат:'
        ]
    ...
    def _intelligent_rewrite(self, text, target_min, target_max, text_type, original_prompt, current_length):
        ...
        # Очищаем текст перед перезаписью
...
ВАЖНЫЕ ПРАВИЛА ДЛЯ ПЕРЕЗАПИСИ:
1. Сохрани основную мысль и структуру
2. Не обрезай, а ПЕРЕПИШИ текст
3. Не пиши "Длина:" или счетчик символов
4. Не используй ** вокруг текста
5. Если длинный - сделай формулировки лаконичными
6. Если короткий - добавь ценного содержания

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ПЕРЕПИСАННЫЙ ТЕКСТ (только текст, без лишней информации):"""
        ...
        try:
            ...
            data = {
                ...
                "generationConfig": {
                    ...
                    "topP": 0.8,
                    "topK": 50,
                    "maxOutputTokens": int(target_max * 1.5),
                }
            }
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in result and result['candidates']:
                    ...
                    if target_min <= new_len <= target_max:
                        ...
                        logger.warning(f"⚠️ Перезапись не достигла цели: {new_len} символов")
                        return None
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'candidates' in result and result['candidates']:
                        ...
                        # ОЧИЩАЕМ текст сразу после генерации
    ...
    def format_telegram_text(self, text, slot_info):
        ...
        # Удаляем ** с начала заголовка если есть
        lines = text.split('\n')
        if lines and lines[0].strip().startswith('**') and lines[0].strip().endswith('**'):
            lines[0] = lines[0].strip()[2:-2].strip()
        
        # Добавляем стартовый эмодзи слота если его нет
        if lines and not lines[0].startswith(slot_info['emoji']):
