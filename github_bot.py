...
6. Дзен пост должен начинаться с шапки БЕЗ эмодзи: вопрос/утверждение
...
Каждый пост должен иметь ОТДЕЛЬНУЮ ШАПКУ с вопросом/утверждением в самом начале.
Пример шапки Telegram: "{slot_style['emoji']} Часто ли ваши PR-кампании тонут в информационном шуме?"
Пример шапки Дзен: "Часто ли ваши PR-кампании тонут в информационном шуме?"
...
Пост должен начинаться СТРОГО с шапки (вопрос/утверждение), соответствующей временному слоту.
...
• ШАПКА: Вопрос/утверждение по теме с эмодзи {slot_style['emoji']}, цепляющий, провокационный или интригующий
...
Создай два РАЗНЫХ текста по одной теме. Оба должны быть уникальными по структуре, но об одном смысле."""
    ...
    def format_zen_text(self, text, slot_style):
        """Форматирует Дзен текст (без эмодзи) - ГАРАНТИРУЕТ наличие хештегов и отдельную шапку"""
        if not text:
            return None
