...
• Структура:
  ...
  3. Вывод / польза для читателя (обязательно полноценное завершение!)
  4. Вопрос или мягкий призыв к действию (без знака • перед вопросом!)
  5. Подпись в конце: "Главная Видео Статьи Новости Подписки"
• СМЕШАННЫЙ СТИЛЬ:
  - Анализ и размышления: можно от первого лица ("я считаю", "по моему опыту")
  - Конкретные кейсы и примеры: строго от третьего лица ("клиент обратился", "в практике встречается")
  - Можно использовать "мы" для обобщений ("мы видим", "как показывает практика")
• Без хештегов
...
[текст Дзен поста]"""
    ...
    def ensure_proper_completion(self, text):
        """Проверяет и добавляет полноценное завершение если нужно"""
        if not text:
            return text
        
        # Проверяем, есть ли уже вывод/заключение
        has_conclusion = any(phrase in text.lower() for phrase in [
            'в заключение', 'таким образом', 'исходя из', 'подводя итоги',
            'можно сказать', 'стоит отметить', 'важно понимать', 'следовательно',
            'итак', 'в итоге'
        ])
        
        if not has_conclusion:
            conclusion_options = [
                "\n\nВ заключение стоит отметить, что профессиональный подход и тщательное планирование являются залогом успеха в любом начинании.",
                "\n\nТаким образом, внимательное отношение к деталям и последовательность действий помогают избежать многих проблем.",
                "\n\nИсходя из вышесказанного, можно сделать вывод о важности комплексного подхода к решению подобных задач.",
                "\n\nПодводя итоги, отметим, что опыт и экспертиза играют ключевую роль в достижении качественного результата."
            ]
            
            # Вставляем перед подписью или в конец
            if "Главная Видео Статьи Новости Подписки" in text:
                parts = text.split("Главная Видео Статьи Новости Подписки")
                text = parts[0] + random.choice(conclusion_options) + "\n\nГлавная Видео Статьи Новости Подписки"
            else:
    ...
    def fix_final_question_format(self, text):
        ...
        for i, line in enumerate(lines):
            ...
            # Фильтруем строки, которые могут быть вопросами/призывами
    ...
    def check_schedule_time(self):
        ...
        for schedule_time in schedule_times:
            ...
            if time_diff <= 2:
                ...
                # Проверяем, не отправляли ли уже пост в это время сегодня
    ...
    def send_single_post(self, chat_id, text, image_url):
        ...
        try:
            ...
            if chat_id == ZEN_CHANNEL_ID:
                ...
                clean_text = self.ensure_proper_completion(clean_text)
...
def main():
    ...
    print(f"   • {ZEN_CHANNEL_ID} (Дзен стиль - смешанный стиль: анализ от первого лица, кейсы от третьего)")
    print("=" * 80)
