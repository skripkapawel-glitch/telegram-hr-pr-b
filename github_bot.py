    ...
    def __init__(self, target_slot=None, auto=False):
        ...
        self.time_styles = {
            "11:00": {
                ...
                "allowed_formats": [
                    "структурированные советы", "демонстрация польза", "объяснение простым языком", "мини-обобщение опыта", "сравнение подходов"
                ],
                "tg_chars": (400, 600),
                "zen_chars": (600, 700),
...
{hashtags_str}"""
    ...
    def _add_telegram_insight(self, text, theme):
        ...
        insights = {
            "ремонт и строительство": [
                "💡 Качество материалов определяет 80% успеха ремонта.",
                "✨ Хороший ремонт - это не дорого, а грамотно спланировано.",
                "🎯 Функциональность важнее моды в дизайне интерьера."
            ],
            "HR и управление персоналом": [
                "💡 Лояльность сотрудника начинается с первого рабочего дня.",
                "✨ Деньги мотивируют только до определенного предела.",
                "🎯 Корпоративная культура - это не пиар, а ежедневная практика."
            ],
            "PR и коммуникации": [
                "💡 Репутация строится годами, а разрушается за минуту.",
                "✨ Лучшая реклама - это довольный клиент.",
                "🎯 Искренность в коммуникациях ценится выше идеальных формулировок."
            ]
        }
        
        insight_list = insights.get(theme, ["💡 Детали решают всё."])
    ...
    def _add_zen_case_study(self, text, theme):
        ...
        case_studies = {
            ...
            "HR и управление персоналом": [
                ...
                "Организация, внедрившая программу mental health support, снизила количество больничных по причине стресса на 35% за полгода.",
                "IT-компания, перешедшая на гибкую систему оценки эффективности (OKRs вместо KPI), увидела рост удовлетворенности сотрудников на 28%."
            ],
            "PR и коммуникации": [
                "Бренд косметики, использовавший UGC-контент в соцсетях, увеличил вовлеченность аудитории на 300% за квартал.",
                "Производитель продуктов питания, прозрачно рассказавший о проблемах в цепочке поставок во время кризиса, не потерял, а увеличил доверие клиентов на 25%.",
                "Технологический стартап, внедривший AI-анализ тональности упоминаний, смог предотвратить 3 потенциальных кризиса репутации."
            ]
        }
    ...
    def add_industry_example(self, text, theme):
        ...
        examples_by_theme = {
            ...
            "PR и коммуникации": [
                "Опыт индустрии: крупнейшие медиахолдинги строят свои коммуникации на схожих принципах.",
    ...
    def add_expert_quote(self, text, theme):
        ...
        quotes_by_theme = {
            "HR и управление персоналом": [
                "«Этот подход изменил наше отношение к управлению талантами».",
                "«Методика доказала свою эффективность в разных отраслях».",
                "«Внедрение таких практик требует системного подхода»."
            ],
            "PR и коммуникации": [
                "«Эта стратегия стала ключом к нашему успеху на рынке».",
                "«Подход позволяет выстраивать долгосрочные отношения с аудиторией».",
                "«Методика особенно эффективна в условиях цифровой трансформации»."
            ],
            "ремонт и строительство": [
                "«Технология значительно повысила качество наших объектов».",
                "«Этот подход соответствует самым строгим международным стандартам».",
                "«Методика позволяет создавать более комфортные и безопасные пространства»."
            ]
        }
...
Создай два РАЗНЫХ текста по одной теме, СТРОГО следуя шаблонам выше. Лаконично, без воды, строго в пределах лимита."""
    ...
    def format_post_text(self, text, slot_style, post_type):
        ...
        if post_type == 'telegram':
            ...
            # 4. ГАРАНТИЯ: ВИЗУАЛЬНОЕ РАЗДЕЛЕНИЕ - добавляем пустые строки перед ключевыми элементами
            lines = [line.strip() for line in text.split('\n') if line.strip()]
            
            # Создаем новый список с визуальным разделением
            formatted_lines = []
            for i, line in enumerate(lines):
                formatted_lines.append(line)
                
                # Добавляем пустую строку перед блоками завершения
                if i < len(lines) - 1 and any(marker in lines[i+1] for marker in 
                                             ['Почему это важно:', 'Что из этого следует:', 'Мнение экспертов:']):
                    formatted_lines.append('')
                
                # Добавляем пустую строку перед вопросами к аудитории
                if i < len(lines) - 1 and any(q in lines[i+1] for q in 
                                             ['А как вы считаете?', 'Было ли у вас так?', 
                                              'Что думаете?', 'Как бы вы поступили?']):
                    formatted_lines.append('')
                
                # Добавляем пустую строку перед экспертными мнениями/цитатами
                if i < len(lines) - 1 and any(marker in lines[i+1].lower() for marker in 
                                             ['архитектор с', 'эксперт отмечает', 'специалист подчеркивает', 
                                              'по словам', 'как отмечает']):
                    formatted_lines.append('')
                
                # Добавляем пустую строку между абзацами (если это не последняя строка и следующий абзац длинный)
        ...
        for line in lines:
            if '#' in line and len(re.findall(r'#[\w\u0400-\u04FF]+', line)) >= 2:
    ...
    def send_moderation_instructions(self, post_ids, slot_time, theme, tg_text, zen_text, edit_timeout):
        ...
        try:
            ...
            instruction = (
                ...
                f"   {'✅' if zen_has_bullets else '⚠️'} Маркеры списка: {'Есть' if zen_has_bullets else 'НЕТ!'}\n"
                f"   {'✅' if zen_has_hook else '⚠️'} Крючок-убийца: {'Есть' if zen_has_hook else 'НЕТ!'}\n"
                f"   {'✅' if zen_has_conclusion else '⚠️'} Блок завершения: {'Есть' if zen_has_conclusion else 'НЕТ!'}\n"
                f"   📌 Используйте кнопки под постом для модерации\n\n"
