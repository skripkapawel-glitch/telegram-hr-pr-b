...
1–3 абзаца
Мини-вывод
Финал вопрос: {random.choice(self.soft_finals)}
...
🔒 СТРОГИЕ ПРАВИЛА
1. Эмодзи только в Telegram (начинается с {slot_style['emoji']})
2. В Дзене — НИ ОДНОГО эмодзи
3. Тексты разные по структуре
4. Хештеги — только в конце каждого поста
5. Мягкий финал — вопрос к аудитории

Переработай текст, сохраняя смысл, но учитывая запрос на редактирование и все вышеперечисленные правила."""
...
🔒 СТРОГИЕ ПРАВИЛА
• Эмодзи только в Telegram (начинается с {slot_style['emoji']})
• В Дзене — НИ ОДНОГО эмодзи
• Тексты разные по структуре
• Хештеги — только в конце каждого поста
• Мягкий финал — вопрос к аудитории
...
📊 СИСТЕМА РОТАЦИИ
• Темы не повторяются подряд
• Форматы чередуются
• Хештеги выбираются случайно
• Картинки не повторяются
...
🖼️ ИНФОРМАЦИЯ О КАРТИНКАХ
• Источники: Pexels, Unsplash
• Ориентация: landscape
• Размер: 1200x630
• Описание: {image_description}

🎨 ТЕМЫ ПОСТОВ
• HR и управление персоналом: управление командой, рекрутинг, мотивация, развитие персонала
• PR и коммуникации: кризисные коммуникации, управление репутацией, брендинг
• Ремонт и строительство: качественный ремонт, строительные технологии, дизайн интерьеров
