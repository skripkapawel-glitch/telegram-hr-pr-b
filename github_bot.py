...
• СТИЛЬ: Строго ОТ ТРЕТЬЕГО ЛИЦА ("специалисты", "эксперты", "опыт показывает")
• КЕЙСЫ: Конкретные примеры от третьего лица ("клиент", "специалист", "компания")
• НЕ ИСПОЛЬЗОВАТЬ: "я", "мне", "мой", "меня", "моё" - никаких от первого лица!
...
• СТИЛЬ: Строго ОТ ТРЕТЬЕГО ЛИЦА ("за 20+ лет практики специалисты пришли", "опыт показывает", "эксперты отмечают")
• НЕ ИСПОЛЬЗОВАТЬ: от первого лица ("я", "мне", "мой")
• Без хештегов
...
[текст Дзен поста]"""
    ...
    def format_text_with_indent(self, text):
        ...
        # Проверяем на первое лицо и исправляем
        text = self.ensure_third_person(text)
    ...
    def ensure_third_person(self, text):
        """Обеспечивает использование третьего лица"""
        # Слова от первого лица для замены
        first_person = {
            ' я ': ' специалисты ',
            ' мне ': ' специалистам ',
            ' мой ': ' их ',
        ...
        for word, replacement in first_person.items():
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🤖 GITHUB TELEGRAM HR BOT - ГЕНЕРАЦИЯ ПОСТОВ СО СВЕЖИМИ КАРТИНКАМИ")
    print("=" * 80)
