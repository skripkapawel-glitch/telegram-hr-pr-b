...
class TelegramBot:
    ...
    def handle_rejection_from_callback(self, message_id, post_data, call):
        ...
        try:
            ...
            # Завершаем workflow при отклонении
            logger.info("✅ Пост отклонен. Завершаем workflow.")
            self.workflow_complete = True
            sys.exit(0)  # Завершаем выполнение с успешным кодом
...
9. Пост должен иметь отдельную шапку с эмодзи и вопросом/утверждением (например: "{slot_style['emoji']} Часто ли ваши PR-кампании тонут в информационном шуме?")
        ...
        try:
            ...
            trends_by_theme = {
                ...
                "PR и коммуникации": [
                    ...
                    "Борьба с дезинформации и deepfakes 2026",
...
Каждый пост должен иметь ОТДЕЛЬНУЮ ШАПКУ с эмодзи и вопросом/утверждением в самом начале.
Пример шапки: "{slot_style['emoji']} Часто ли ваши PR-кампании тонут в информационном шуме?"

После шапки идет основное содержание.
...
Пост должен начинаться СТРОГО с шапки (эмодзи + вопрос/утверждение), соответствующей временному слоту.
...
• ШАПКА: {slot_style['emoji']} + вопрос/утверждение по теме (например: "{slot_style['emoji']} Часто ли ваши PR-кампании тонут в информационном шуме?")
...
• ШАПКА: Вопрос/утверждение по теме БЕЗ эмодзи, но цепляющий, провокационный или интригующий
...
4. Telegram пост начинается с ШАПКИ: {slot_style['emoji']} + вопрос/утверждение
5. Дзен пост — ШАПКА БЕЗ ЭМОДЗИ ВООБЩЕ, но с профессиональным вопросом/утверждением
...
📝 ФОРМАТ ВЫВОДА:
• Сначала Telegram версия (с шапкой и эмодзи {slot_style['emoji']})
• Потом Дзен версия (с шапкой без эмодзи)
• Разделитель: три дефиса (---)
...
Создай два РАЗНЫХ текста по одной теме. Оба должны быть уникальными по структуре, но об одном смысле."""
    ...
    def format_telegram_text(self, text, slot_style):
        ...
        # Проверяем, начинается ли текст с нужного эмодзи (шапка)
        if not text.startswith(slot_style['emoji']):
            lines = text.split('\n')
            if lines and lines[0].strip():
                # Добавляем эмодзи к первой строке (шапке)
    ...
    def format_zen_text(self, text, slot_style):
        ...
        # Удаляем все эмодзи из Дзен поста (но оставляем шапку)
        ...
        # Удаляем эмодзи, но сохраняем структуру
        lines = text.split('\n')
        cleaned_lines = []
        for line in lines:
            cleaned_line = emoji_pattern.sub(r'', line)
            cleaned_line = re.sub(r'[^\w\s#@.,!?;:"\'()\-—–«»\n]', '', cleaned_line)
