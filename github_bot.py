...
• Структура:
  ...
  4. Вопрос или мягкий призыв к действию (без знака • перед вопросом!)
...
• Структура:
  ...
  3. КОНЦОВКА (обязательно включай все 4 элемента):
     a) Ключевая мысль/резюме: кратко самое важное из поста
     b) Вопрос к читателю: чтобы вызвать обсуждение в комментариях
     c) Призыв к действию: подписаться, поставить лайк, поделиться
     d) Намёк на продолжение: что будет в следующем материале
  4. Подпись в конце: "Главная Видео Статьи Новости Подписки"
• СТРОГОЕ РАЗДЕЛЕНИЕ СТИЛЕЙ:
  - Анализ, выводы, наблюдения, исследования: ОТ ПЕРВОГО ЛИЦА ("я изучил", "я проанализировал", "я понял", "мой анализ показал")
  - Кейсы, примеры, истории, рассказы: ОТ ТРЕТЬЕГО ЛИЦА ("клиент обратился", "в практике встречается", "специалисты отмечают", "компания столкнулась")
  - 20+ лет опыта упоминать ОТ ТРЕТЬЕГО ЛИЦА ("За 20+ лет практики специалисты пришли", "Опыт показывает", "Как демонстрирует практика")
...
[текст Дзен поста]"""
    ...
    def ensure_zen_completion(self, text):
        ...
        # Проверяем наличие элементов концовки
        has_summary = any(phrase in text.lower() for phrase in [
            'таким образом', 'в итоге', 'главное', 'ключевое', 'суть',
            'можно сделать вывод', 'подводя итоги', 'итак'
        ])
        
        has_question = any(marker in text for marker in ['?', 'Как вы думаете', 'А что считаете вы', 'Ваше мнение'])
        
        has_call_to_action = any(phrase in text.lower() for phrase in [
            'подписывайтесь', 'ставьте лайк', 'делитесь', 'комментируйте',
            'обсуждайте', 'пишите', 'оставляйте'
        ])
        
        has_next_hint = any(phrase in text.lower() for phrase in [
            'в следующем', 'в продолжении', 'далее мы', 'в следующий раз',
            'в новых материалах', 'в будущих публикациях'
        ])
        
        # Если все элементы есть - возвращаем как есть
        if has_summary and has_question and has_call_to_action and has_next_hint:
            return text
        ...
        if not has_summary:
            summary_options = [
                "Таким образом, главное — действовать проактивно и не бояться диалога.",
                "В итоге, ключевое — это своевременная и честная коммуникация.",
                "Суть в том, что профессиональный подход всегда дает лучшие результаты."
            ]
            completion_parts.append(random.choice(summary_options))
        
        if not has_question:
            question_options = [
                "А как вы считаете? Сталкивались ли с подобными ситуациями?",
                "А что думаете вы? Какие стратегии использовали бы в такой ситуации?",
                "Ваше мнение? Как бы вы поступили на месте героев этого кейса?"
            ]
        ...
        if not has_call_to_action:
            call_to_action_options = [
                ...
                "Ставьте лайк, если статья была полезной, и делитесь в соцсетях!",
                "Оставляйте комментарии — ваше мнение важно для нас!"
            ]
            completion_parts.append("\n\n" + random.choice(call_to_action_options))
        
        if not has_next_hint:
            next_hint_options = [
                "В следующем материале мы разберем конкретные инструменты кризисных коммуникаций.",
                "В продолжении темы — как измерять эффективность PR-кампаний.",
                "В следующий раз поговорим о том, как избежать самых частых ошибок в коммуникациях."
            ]
            completion_parts.append("\n\n" + random.choice(next_hint_options))
    ...
    def fix_final_question_format(self, text):
        """Исправляет формат финального вопроса/призыва к действию - убирает • перед ним"""
        if not text:
            return text
        ...
        for i, line in enumerate(lines):
            # Проверяем, не является ли строка финальным вопросом/призывом
            is_likely_final = False
            
            # Признаки финального вопроса/призыва
            question_indicators = ['?', 'Почему', 'Зачем', 'Как', 'Что', 'Какой', 'Какие', 'Чей', 
                                  'Чем', 'Какой', 'Когда', 'Где', 'Куда', 'Откуда', 'Зачем', 'Сколько']
            call_to_action_indicators = ['Пишите', 'Делитесь', 'Расскажите', 'Обсудим', 'Обсуждаем',
                                        'Жду', 'Оставляйте', 'Комментируйте', 'Подписывайтесь']
            ...
            if any(indicator.lower() in line_lower for indicator in call_to_action_indicators):
                is_likely_final = True
            
            # Если строка начинается с • и это похоже на финальный вопрос/призыв
            if line.strip().startswith('•') and is_likely_final:
    ...
    def send_single_post(self, chat_id, text, image_url):
        ...
        try:
            ...
            if chat_id == ZEN_CHANNEL_ID:
                ...
                clean_text = self.ensure_zen_completion(clean_text)
            
            # Исправляем формат финального вопроса/призыва
            clean_text = self.fix_final_question_format(clean_text)
    ...
    def generate_and_send_posts(self, slot_type=None):
        ...
        try:
            ...
            # Применяем фикс формата финального вопроса
            tg_text = self.fix_final_question_format(tg_text)
            zen_text = self.fix_final_question_format(zen_text)
            
            # Обеспечиваем базовую структуру с • для основных пунктов
            if "•" not in tg_text and len(tg_text.split('\n')) > 2:
                lines = tg_text.split('\n')
                formatted_lines = []
                for i, line in enumerate(lines):
            ...
            if "•" not in zen_text and len(zen_text.split('\n')) > 2:
            ...
            zen_text = self.clean_telegram_text(zen_text)
